...
import (
	...
	"nudgebee/tickets-server/services/cache"
	ticketmgr "nudgebee/tickets-server/services/ticket"
	"nudgebee/tickets-server/services/tools"
)

// createMetaNamespace is the cache namespace for create-meta blobs, keyed by
// "<integration_id>:<project_key>". The blob holds all issue-type templates, so
// issue_type is intentionally not part of the key.
const createMetaNamespace = "ticket_create_meta"

func init() {
	cache.CreateNamespace(createMetaNamespace)
}
...
func updateMetadataConfigValues(integrationID string, metadata []map[string]interface{}, createdBy *string) error {
	...
	// Metadata just changed (save, test, or the bi-hourly sync), so any cached
	// create-meta for this integration is potentially stale — drop it now rather
	// than waiting out the TTL. Next request re-warms from fresh data.
	invalidateCreateMetaCache(integrationID)
	return nil
}
...
func FetchActiveIssueTemplates(ctx *gin.Context, request models.IssueTemplateMetaRequest) (interface{}, error) {
	...
	return fetchCreateMetaCached(ctx, config, request.Input.ProjectKey)
}

// fetchCreateMetaCached returns the create-meta for (integration, project), serving
// from Redis/in-memory cache on a hit and falling through to a live per-tool fetch on
// a miss (then best-effort caching the result). The cache degrades gracefully: a miss
// or any cache error simply triggers the live path. All six tools are reached through
// the registry, so adding a tool needs no change here.
func fetchCreateMetaCached(ctx *gin.Context, config models.TicketConfigurations, projectKey string) (interface{}, error) {
	// Use the request context so a cache op is cancelled if the client disconnects.
	reqCtx := context.Background()
	if ctx != nil && ctx.Request != nil {
		reqCtx = ctx.Request.Context()
	}

	key := config.ID + ":" + projectKey
	if blob, ok := cache.Get(reqCtx, createMetaNamespace, key); ok {
		var cached interface{}
		if err := json.Unmarshal(blob, &cached); err == nil {
			return cached, nil
		}
		// Corrupted / schema-drifted entry: drop it so the cache self-heals.
		_ = cache.Delete(reqCtx, createMetaNamespace, key)
	}

	mgr, ok := ticketmgr.GetTicketManager(config.Tool)
	if !ok {
		return nil, fmt.Errorf("issue templates not supported for tool %s (integration %s)", config.Tool, config.ID)
	}
	meta, err := mgr.GetCreateMeta(ctx, config, projectKey)
	if err != nil {
		return nil, err
	}

	if blob, mErr := json.Marshal(meta); mErr == nil {
		ttl := time.Duration(common.Config.CacheExpirationMinutes) * time.Minute
		if sErr := cache.Set(reqCtx, createMetaNamespace, key, blob, ttl, config.ID); sErr != nil {
			slog.Warn("create-meta cache set failed", "error", sErr, "integration", config.ID, "project", projectKey)
		}
	}
	return meta, nil
}

// invalidateCreateMetaCache drops all cached create-meta for an integration. Called on
// integration save/test and the bi-hourly sync (often a background goroutine, so it uses
// a detached context — invalidation should complete regardless of any request).
func invalidateCreateMetaCache(integrationID string) {
	if err := cache.DeleteByIntegration(context.Background(), createMetaNamespace, integrationID); err != nil {
		slog.Warn("create-meta cache invalidation failed", "error", err, "integration", integrationID)
	}
}
