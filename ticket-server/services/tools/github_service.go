...
func CreateGithubIssue(ctx *gin.Context, configuration models.TicketConfigurations, ticket models.Ticket) (models.Ticket, error) {
	var githubClinet *github.Client
	var err error

	if configuration.AuthType != "application" {
		githubClinet = clients.CreateGithubClient(configuration.Password)
	} else {
		...
		githubClinet, err = clients.CreateGithubClientWithInstallationToken(ctx, installationID)
		if err != nil {
			return ticket, err
		}
	}
	...
	// Create the issue
	createdIssue, _, err := githubClinet.Issues.Create(ctx, owner, repo, &issueRequest)
	if err != nil {
...
func FetchGitHubIssueCreateMeta(ctx *gin.Context, configuration models.TicketConfigurations, repoKey string) (any, error) {
	...
	assigneeOpts := &github.ListOptions{PerPage: 100}
	for {
		assignees, resp, err := githubClient.Issues.ListAssignees(ctx, owner, repo, assigneeOpts)
		if err != nil {
			return nil, err
		}
		...
		assigneeOpts.Page = resp.NextPage
	}
	...
	// Match the Jira shape: {data: [template]}. The Hasura action exposes this
