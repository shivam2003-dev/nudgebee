...
Configure the Slack App's redirect URL to `${BASE_URL}/api/integrations/slack/callback` and the event subscription URL to `${BASE_URL}/webhooks/slack/events`.

> The Next.js frontend exposes the callback at `/api/integrations/slack/callback` (since the PR that restructured `/api/*` to the 7-surface canonical model). The Next.js handler proxies to notifications-server's internal Bolt route at `/slack/oauth_redirect` — that internal path is the Slack Bolt library's default and is unchanged.
