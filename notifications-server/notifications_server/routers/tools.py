...
async def google_chat_oauth_redirect(request: Request, data: Dict[Any, Any]):
    tenant_id = data.pop("tenantId", None)
    if tenant_id is None:
        raise ValueError(400, Err.OS0010, ["tenantId"])
...
class SlackBase:
    ...
    @staticmethod
    def set_response(bolt_resp, response: Response):
        response.status_code = bolt_resp.status
        response.body = bolt_resp.body
        ...
        for cookie in bolt_resp.cookies():
            for name, c in cookie.items():
                ...
                response.set_cookie(
                    ...
                    expires=expire.replace(tzinfo=timezone.utc) if expire else None,
                    path=c.get("path"),
                    domain=c.get("domain"),
                    secure=True,
                    httponly=True,
...
@slack_router.get("/install_path")
async def get_slack_install_path(
    request: Request,
):
    ...
    if slack_base.slack_app.oauth_flow is not None:
        ...
        bolt_resp = await request.app.state.executor.submit(oauth_flow.handle_installation, bolt_request)
        return slack_base.set_response(bolt_resp, Response())
    else:
