...
class Events:
    ...
    def _process_event(self, channel_id, cleaned_string, team_id, thread_ts, _type):
        ...
        try:
            ...
            error_response = getattr(e.response, "json", lambda: None)() if hasattr(e, "response") else None
            if is_conversation_in_progress_error(error_response):
                self.common_service.slack_reply_in_thread(channel_id, team_id, thread_ts, get_llm_busy_message())
            else:
    ...
    def update_llm_chat_feedback(self, payload, channel_id, team_id, thread_ts):
        try:
            ...
            # (The earlier Hasura mutation passed feedback_object={data:{...}} as the
            # $object variable, which was a long-standing schema mismatch — that path
            # silently no-op'd because Hasura rejected it. Going direct gives us the
            ...
            # Post directly to services-server using the same envelope Hasura
            # would have forwarded. Matches the handler at api-server/services/
            # api/hasura_actions_ai_feedback.go (ai_feedback_create case), so we
            # don't need a parallel REST endpoint to be defined upstream.
            requests.post(
                settings.services.api_server + "/hasura/ai",
                json={
                    "action": {"name": "ai_feedback_create"},
                    "input": {"request": feedback_request},
                    "session_variables": {
                        "x-hasura-user-tenant-id": tenant_id,
