    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            if "decrypted_secret" not in value and "access_secret_v2" not in value:
                raise UnauthorizedError(INVALID_SECRET)
            if (
                "decrypted_secret" in value
                and len(value["decrypted_secret"]) > 0
                and value["decrypted_secret"] != api_secret
            ) or (value.get("access_secret_v2") and not validate_key(api_secret, value["access_secret_v2"])):
                raise UnauthorizedError(INVALID_SECRET)
