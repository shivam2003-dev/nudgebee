...
*   **API Endpoints:** Provides RESTful APIs for:
    ...
    *   Handling Hasura actions/events.
...
The service exposes several API endpoints. For detailed information on request/response formats, please refer to the API handler functions in the `api/` directory (e.g., `api/cloud.go`, `api/hasura.go`, `api/health.go`).
...
*   **Hasura Actions/Events:**
    *   Endpoints to handle Hasura actions or events. These require authentication.
...
*   **API Structure:** Defined in `api/` directory. Main routes in `api/api.go`. Cloud-specific handlers in `api/cloud.go`. Hasura handlers in `api/hasura.go`.
