...
func (s *optimizerService) GetRecommendations(ctx context.Context, sc *security.RequestContext, accountID uuid.UUID, ruleName *string, status []string, categories []string) (map[string][]string, error) {
	...
	if len(likeFilter) == 0 && len(inFilter) == 0 {
		return map[string][]string{"recommendation": {}}, nil
	}
...
func (s *optimizerService) UpdateAutoOptimize(ctx context.Context, sc *security.RequestContext, accountID uuid.UUID, req model.AutoOptimizeRequestModel) (*string, error) {
	...
	// Preserve existing frequency if the request omits one. Without this, an empty
	// frequency falls through to the category default and silently resets the cron.
	freq := oldAO.ScheduleTime
	if req.Schedule.Frequency != "" {
	...
	ao := *oldAO
	// Preserve existing rule if the request omits one, so a client that ships an
	// empty auto_optimize_config cannot silently wipe the saved configuration.
	if len(req.AutoOptimizeConfig) > 0 {
	...
	// Transition status with the dry-run toggle. Without the else branch a user can
	// turn Dry Run on but never off — toggling it off would leave Status=Dryrun untouched.
	// Disabled is left alone so disabling and toggling dry-run don't fight each other.
	if req.DryRun {
		ao.Status = model.AutoOptimizeStatusDryrun
	} else if ao.Status == model.AutoOptimizeStatusDryrun {
		ao.Status = model.AutoOptimizeStatusActive
	}
