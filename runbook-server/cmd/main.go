...
// @version         1.0
// @description     Runbook server — manages workflows, executions, configs, approvals, and Hasura action entry points for Nudgebee. Docs are auto-merged: handlers with godoc annotations get rich entries; everything else is stubbed from the gin route table.
// @BasePath        /
...
func main() {
	...
	// Load cron triggers from embedded YAML config (similar to Hasura cron_triggers.yaml)
