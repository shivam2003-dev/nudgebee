...
func (s *Server) handleHasuraListTemplates(c *gin.Context, sc *security.RequestContext, args map[string]any) {
	...
	if err != nil {
		s.logger.Error("failed to list templates via Hasura", "error", err)
...
func (s *Server) handleHasuraGetTemplate(c *gin.Context, sc *security.RequestContext, args map[string]any) {
	templateType, ok := args["type"].(string)
	if !ok || templateType == "" {
		c.JSON(http.StatusBadRequest, buildApiResponse(nil, []error{fmt.Errorf("type is required")}))
		return
	...
	if err != nil {
		s.logger.Error("failed to get template via Hasura", "templateID", templateID, "error", err)
