...
func (s *Service) ExecuteWorkflow(ctx *security.RequestContext, accountId, id string, triggerType model.WorkflowTrigger, inputs map[string]any) (string, error) {
	...
	// Override the draft definition on the in-memory workflow with the live
	// version's snapshot. This is what the executor receives and what Temporal
	// records in history.
	...
	// Stamp execution-to-version linkage in Temporal Memo. Memo (not
	// SearchAttributes) keeps zero coupling to per-namespace admin config —
	// the only consumer is GetDetailedWorkflowExecution for display, never a
	// filter. We deliberately store name as a string ("" when null) so the
	// reader has a single, type-stable shape regardless of metadata presence.
	memo := map[string]any{
...
func (s *Service) GetDetailedWorkflowExecution(ctx *security.RequestContext, accountId, workflowId, executionId string) (*model.WorkflowExecutionDetails, error) {
	...
	if workflowDetails.DefinitionSource == "" && !isNonPersistedWorkflow {
		// Either no Memo key, or version row missing — surface the draft and
		// flag the divergence so the UI can banner appropriately.
