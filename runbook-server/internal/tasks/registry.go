...
func NewInitializedTaskRegistry() *TaskRegistry {
	...
	// CallWorkflowTask is dispatched inline by the executor (see call_workflow_task.go),
	// but it must still be registered so workflow validation accepts `core.call-workflow`
	// as a known task type and so the frontend palette discovers it via
	// workflow_list_taskdefinitions instead of relying on a synthetic taskdef.
	tr.RegisterTask(&core.CallWorkflowTask{})
