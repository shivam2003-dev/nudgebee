...
import (
	"log/slog" // Import common package for DatabaseManager
...
func TestGithubTask_Execute(t *testing.T) {
	task := &GithubCliTask{}
	...
	testCases := []struct {
	...
	}{
		{
			...
			params: map[string]any{
				"command":        "gh help",
				"integration_id": os.Getenv("TEST_GITHUB_INTEGRATION_ID"),
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectErr {
				...
				// Updated assertion for 'gh help' output
				assert.True(t, strings.Contains(result.(map[string]any)["data"].(string), "USAGE"))
			}
		})
	}
}
