...
func (t *PVRightsizeTask) applyRateStorage(currentStorage resource.Quantity, changeByStr string) (resource.Quantity, bool, error) {
	...
	if strings.HasSuffix(changeByStr, "%") {
		changeByStr = strings.TrimSuffix(changeByStr, "%")
...
func buildPVCIssueBody(name, namespace, oldSize, newSize, maxSize string, taskCtx types.TaskContext) string {
	...
	b.WriteString(fmt.Sprintf("**Current storage:** `%s`  \n", oldSize))
	b.WriteString(fmt.Sprintf("**Target storage:** `%s`  \n", newSize))
	if maxSize != "" {
		b.WriteString(fmt.Sprintf("**Max allowed:** `%s`  \n", maxSize))
	}
	b.WriteString("\n## Suggested change\n\n")
	...
	if link := getWorkflowBaseLink(taskCtx); link != "" {
		b.WriteString(fmt.Sprintf(" [Workflow run](%s)", link))
	}
	b.WriteString("\n")
	return b.String()
}
