...
// hasuraError returns an error response in the format Hasura action webhooks expect: { "message": "..." }
func hasuraError(c *gin.Context, code int, msg string) {
...
func handleBudgetStatusApi(r *gin.Engine, tracer trace.Tracer, meter metric.Meter) {
	...
	group.POST("/status", func(c *gin.Context) {
		...
		if err != nil {
			...
			hasuraError(c, 400, "api: "+err.Error())
			return
		}
		...
		if hasuraRequestPayload["request"] != nil {
			if reqMap, ok := hasuraRequestPayload["request"].(map[string]any); ok {
				hasuraRequestPayload = reqMap
			}
		}
		...
		if err != nil {
			hasuraError(c, 401, err.Error())
			return
		}

		if request.AccountID == "" {
			hasuraError(c, 400, "api: account_id is required")
			return
		}

		if !agentContext.GetSecurityContext().HasAccountAccess(request.AccountID, security.SecurityAccessTypeRead) {
			hasuraError(c, 403, errorUserAccessMessage)
			return
		}
		...
		if err != nil {
			...
			hasuraError(c, 500, "api: failed to retrieve budget status")
			return
		}
	...
	group.POST("/config/list", func(c *gin.Context) {
		...
		if !sc.IsSuperAdmin() && !sc.IsTenantAdmin() {
			hasuraError(c, 403, "api: admin access required")
			return
		}

		var listReq budget.BudgetConfigListRequest
		_ = common.DecodeMapToStruct(hasuraRequestPayload, &listReq)
		...
		if err != nil {
			hasuraError(c, 500, "api: database error")
			return
		}
		...
		if err != nil {
			...
			hasuraError(c, 500, "api: failed to list budget configs")
			return
		}
	...
	group.POST("/config/upsert", func(c *gin.Context) {
		...
		if request.EntityType == "" || request.EntityID == "" || request.Module == "" {
			hasuraError(c, 400, "api: entity_type, entity_id, and module are required")
			return
		}
		...
		if request.BudgetDisabled != nil && *request.BudgetDisabled && !sc.IsSuperAdmin() {
			hasuraError(c, 403, "api: only super admins can disable budget checks")
			return
		}
		...
		if err != nil {
			...
			hasuraError(c, statusCode, err.Error())
			return
		}
	...
	group.POST("/config/delete", func(c *gin.Context) {
		...
		_ = common.DecodeMapToStruct(hasuraRequestPayload, &req)

		if req.ID == "" {
			hasuraError(c, 400, "api: id is required")
			return
		}
		...
		if err != nil {
			hasuraError(c, 500, "api: failed to fetch budget config")
			return
		}
		if cfg == nil {
			hasuraError(c, 404, "api: budget config not found")
			return
		}
