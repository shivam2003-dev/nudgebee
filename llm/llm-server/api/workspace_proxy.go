...
func handleWorkspaceApis(r *gin.Engine, tracer trace.Tracer, meter metric.Meter) {
	...
	// Unified Files API (Handles all Hasura Actions: list, get, save, delete, batch-read)
...
func handleWorkspaceFiles(c *gin.Context, tracer trace.Tracer, meter metric.Meter) {
	...
	default:
		c.JSON(http.StatusBadRequest, common.ErrorHasuraActionBadRequest(fmt.Sprintf("unsupported action: %s", actionName)))
	}
}
...
func extractWorkspacePayloadAndContext(c *gin.Context, tracer trace.Tracer, meter metric.Meter) (map[string]any, *security.RequestContext, string, error) {
	...
	if authHeader != "" && authHeader == config.Config.LlmServerToken {
		...
		_ = common.DecodeMapToStruct(requestMap, &hasuraRequest)

		payload := requestMap
		actionName := hasuraRequest.Action.Name
		if actionName != "" {
	...
	if actionName != "" {
		ctx, err = buildContextFromHasuraPayload(c.Request.Context(), c, &hasuraRequest, tracer, meter, slog.Default())
		payload = hasuraRequest.Input
		if rawRequest, ok := payload["request"]; ok {
			if castedRequest, castOk := rawRequest.(map[string]any); castOk {
				payload = castedRequest
...
func handleWorkspaceListFilesWithPayload(c *gin.Context, payload map[string]any, ctx *security.RequestContext) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, common.ErrorHasuraActionInternal(err.Error()))
		return
	}
...
func handleWorkspaceGetFileWithPayload(c *gin.Context, payload map[string]any, ctx *security.RequestContext) {
	...
	if err != nil {
		if strings.Contains(strings.ToLower(err.Error()), "not found") || strings.Contains(err.Error(), "404") {
			c.JSON(http.StatusNotFound, common.ErrorHasuraActionBadRequest("File not found"))
		} else {
			c.JSON(http.StatusInternalServerError, common.ErrorHasuraActionInternal(err.Error()))
		}
		return
	...
	if status != http.StatusOK {
		...
		if status >= http.StatusInternalServerError {
			c.JSON(status, common.ErrorHasuraActionInternal("failed to read workspace file"))
		} else {
			c.JSON(status, common.ErrorHasuraActionBadRequest(errMsg))
		}
		return
	}

	// The Hasura action `ai_get_workspace_file` returns `jsonb`, so the webhook
...
// suitable for returning from the ai_get_workspace_file Hasura action. It
// enforces two invariants that the Hasura path requires:
//
...
func handleWorkspaceExecute(c *gin.Context, tracer trace.Tracer, meter metric.Meter) {
	...
	default:
		c.JSON(http.StatusBadRequest, common.ErrorHasuraActionBadRequest(fmt.Sprintf("unsupported tool: %s", req.Tool)))
		return
	}
	...
	if registeredToolName != "" {
		...
		if !found {
			...
			if !found {
				c.JSON(http.StatusNotFound, common.ErrorHasuraActionBadRequest(fmt.Sprintf("tool not found: %s", req.Tool)))
				return
			}
		}
	} else {
		c.JSON(http.StatusBadRequest, common.ErrorHasuraActionBadRequest(fmt.Sprintf("tool %s is not fully supported via proxy yet", req.Tool)))
		return
	}
