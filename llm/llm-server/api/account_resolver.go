...
//  2. Exact match: query contains full account name (e.g. "check civo-dev" matches "civo-dev")
//  3. Token match: a query word matches a token in account name (e.g. "check civo" matches "civo-dev")
...
func resolveAccountFromQuery(query string, accounts []CloudAccountInfo) ResolveAccountResult {
	...
	// e.g. query "list pods civo" -> word "civo" matches token "civo" in "civo-dev"
