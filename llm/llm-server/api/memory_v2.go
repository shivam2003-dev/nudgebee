...
// Wired into api.go as handleMemoryV2Apis. Follows the Hasura Action envelope
...
func handleMemoryV2Apis(r *gin.Engine, tracer trace.Tracer, meter metric.Meter) {
	...
	group.POST("", func(c *gin.Context) {
		...
		if err := c.ShouldBindJSON(&hasuraRequest); err != nil {
			slog.Error("memory_v2: bind hasura request", "error", err)
		...
		ctx, err := buildContextFromHasuraPayload(c.Request.Context(), c, &hasuraRequest, tracer, meter, slog.With())
		if err != nil {
