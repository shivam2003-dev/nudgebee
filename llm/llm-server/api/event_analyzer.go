...
func handleAnalysisApis(r *gin.Engine, tracer trace.Tracer, meter metric.Meter) {
	...
	groupV2.POST("/event/rca", func(c *gin.Context) {
		...
		if v, ok := hasuraRequestPayload["request"].(map[string]any); ok {
			hasuraRequestPayload = v
		}
		...
		context, err := buildContextFromHasuraPayload(c.Request.Context(), c, &hasuraRequest, tracer, meter, logger)
		if err != nil {
	...
	groupV2.POST("/event/rca/format_get", func(c *gin.Context) {
		...
		var hasuraRequest HasuraActionRequest
		if err := c.ShouldBindJSON(&hasuraRequest); err != nil {
...
func processEventAnalysis(c *gin.Context, tracer trace.Tracer, meter metric.Meter) {
	...
	if request.UserId == "" || request.AccountId == "" || request.EventId == "" {
		slog.Error("analyzer: userId, accountId and eventId are required", "payload", slog.AnyValue(hasuraRequest), "headers", slog.AnyValue(c.Request.Header))
		c.JSON(400, buildApiResponse(nil, []error{
			common.Error{
