...
func TestUnmarshalJsonAsMap(t *testing.T) {
	...
	t.Run("prod gcp cluster payload preserves all clusters", func(t *testing.T) {
		data := `[{"id":"nudgebee-dev","name":"nudgebee-dev","type":"container.googleapis.com/Cluster"},{"id":"nudgebee-prod","name":"nudgebee-prod","type":"container.googleapis.com/Cluster"}]`
		out, err := UnmarshalJsonAsMap([]byte(data))
		assert.NoError(t, err)
		items, ok := out["items"].([]any)
		assert.True(t, ok)
