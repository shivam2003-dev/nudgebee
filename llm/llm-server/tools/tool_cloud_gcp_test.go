	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				Query:      "gcloud compute instances list --project nudgebee-dev --format=\"table(name, zone, status, machineType, networkInterfaces[0].networkIP, networkInterfaces[0].accessConfigs[0].natIP)\"",
				ToolConfig: "gcp-dev - nudgebee-dev",
			},
		}
