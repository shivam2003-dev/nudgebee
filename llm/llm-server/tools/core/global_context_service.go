...
const (
	...
	errGCSizeLimitExceeded       = "content exceeds the maximum allowed size. Please reduce the content size"
...
// gcCacheTTL controls how long the active-GC-per-account loader cache holds
// an entry before re-reading from the database. Matches the 1-hour TTL used
// by the prompts package for account-scoped config.
const gcCacheTTL = time.Hour
...
var gcActiveCache sync.Map // cacheKey (string, tenantId:accountId) -> gcCacheEntry

// gcCacheKey returns the composite key used by the active-GC cache. Keying on
// tenant_id + account_id keeps the cache aligned with the SQL queries which
// also gate on tenant_id, so the cache cannot serve another tenant's data even
// if account_id values were ever reused across tenants.
func gcCacheKey(tenantId, accountId string) string {
	return tenantId + ":" + accountId
}

// InvalidateActiveGCCache drops the cached active-GC entry for an account
// under a specific tenant. Called from Create/Update/Delete so the next chat
// turn sees fresh data.
func InvalidateActiveGCCache(tenantId, accountId string) {
	gcActiveCache.Delete(gcCacheKey(tenantId, accountId))
}

// LoadActiveGlobalContext returns the data string of the active global context
// for the account, or "" if none is configured. The function is designed for
// the chat hot path: it never returns an error that would block a turn —
// database failures are logged and reported as "no GC", and an empty data
// string is treated as no GC. A 1-hour in-memory cache absorbs repeated reads.
// Tenant is derived from the authenticated security context and is enforced in
// both the cache key and the SQL WHERE clause.
func LoadActiveGlobalContext(sc *security.RequestContext, accountId string) string {
	if accountId == "" {
		return ""
	}
	...
	cacheKey := gcCacheKey(tenantId, accountId)

	if v, ok := gcActiveCache.Load(cacheKey); ok {
		if entry, ok := v.(gcCacheEntry); ok && time.Now().Before(entry.expiresAt) {
			return entry.data
		}
	}
	...
	if err != nil {
		sc.GetLogger().Warn("gc: LoadActiveGlobalContext: failed to get db manager", "error", err, "account_id", accountId)
		return ""
	}

	var data sql.NullString
	err = dbms.Db.Get(&data, `
		SELECT data
		FROM llm_global_contexts
		WHERE account_id = $1 AND tenant_id = $2 AND status = $3
		ORDER BY updated_at DESC
		LIMIT 1`, accountId, tenantId, string(GCStatusActive))
	if err != nil {
		// Only cache the negative result for sql.ErrNoRows (no GC configured).
		// Caching transient DB errors (timeouts, connection drops) for the full
		// 1-hour TTL would lock the account out of its GC even after the DB
		// recovers — re-querying on the next turn is the safer behavior.
		if errors.Is(err, sql.ErrNoRows) {
			gcActiveCache.Store(cacheKey, gcCacheEntry{data: "", expiresAt: time.Now().Add(gcCacheTTL)})
		} else {
			sc.GetLogger().Warn("gc: LoadActiveGlobalContext: query failed", "error", err, "account_id", accountId)
		}
		return ""
	}

	out := strings.TrimSpace(data.String)
	gcActiveCache.Store(cacheKey, gcCacheEntry{data: out, expiresAt: time.Now().Add(gcCacheTTL)})
	return out
}
...
// tenant_id is always derived from the authenticated security context.
// Any tenant_id on the input struct is ignored.
func CreateGlobalContext(sc *security.RequestContext, gc GlobalContext) (GlobalContext, error) {
	tenantId := sc.GetSecurityContext().GetTenantId()
	if tenantId == "" {
	...
	if maxBytes := config.Config.LlmServerMaxGCBytes; maxBytes > 0 && gc.DataSizeBytes > int64(maxBytes) {
		return GlobalContext{}, fmt.Errorf("%s: got %d bytes (max: %d)", errGCSizeLimitExceeded, gc.DataSizeBytes, maxBytes)
	}
	...
	InvalidateActiveGCCache(gc.TenantId, gc.AccountId)
...
// tenant_id is always derived from the authenticated security context.
func GetGlobalContext(sc *security.RequestContext, accountId, gcId string) (GlobalContext, error) {
	tenantId := sc.GetSecurityContext().GetTenantId()
	if tenantId == "" {
		return GlobalContext{}, errors.New(errGCTenantIDRequired)
	}
	...
	err = dbms.Db.Get(&gc, `
		...
		LEFT JOIN users cu ON gc.created_by = cu.id
		LEFT JOIN users uu ON gc.updated_by = uu.id
		WHERE gc.id = $1 AND gc.account_id = $2 AND gc.tenant_id = $3`, gcId, accountId, tenantId)
	if err != nil {
...
// tenant_id is always derived from the authenticated security context.
func ListGlobalContexts(sc *security.RequestContext, accountId string) ([]GlobalContext, error) {
	tenantId := sc.GetSecurityContext().GetTenantId()
	if tenantId == "" {
	...
	err = dbms.Db.Select(&gcs, `
		...
		LEFT JOIN users cu ON gc.created_by = cu.id
		LEFT JOIN users uu ON gc.updated_by = uu.id
		WHERE gc.account_id = $1 AND gc.tenant_id = $2
		ORDER BY gc.created_at DESC`, accountId, tenantId)
	if err != nil {
...
// tenant_id is always derived from the authenticated security context.
func UpdateGlobalContext(sc *security.RequestContext, accountId, gcId string, updates GlobalContext) error {
	tenantId := sc.GetSecurityContext().GetTenantId()
	if tenantId == "" {
	...
	if updates.Data != "" && updates.Data != existingGC.Data {
		...
		if maxBytes := config.Config.LlmServerMaxGCBytes; maxBytes > 0 && updates.DataSizeBytes > int64(maxBytes) {
			return fmt.Errorf("%s: got %d bytes (max: %d)", errGCSizeLimitExceeded, updates.DataSizeBytes, maxBytes)
		}
	} else {
	...
	_, err = dbms.DoInTransaction(func(tx *sqlx.Tx) (any, error) {
		_, err = tx.Exec(`
			...
			updates.DataSizeBytes, nullableUpdatedBy, gcId, accountId, tenantId)
		if err != nil {
	...
	InvalidateActiveGCCache(tenantId, accountId)
...
// tenant_id is always derived from the authenticated security context.
func DeleteGlobalContext(sc *security.RequestContext, accountId, gcId string) error {
	tenantId := sc.GetSecurityContext().GetTenantId()
	if tenantId == "" {
		return errors.New(errGCTenantIDRequired)
	}
	...
	var gc GlobalContext
	err = dbms.Db.Get(&gc, "SELECT id, name, tenant_id, account_id FROM llm_global_contexts WHERE id = $1 AND account_id = $2 AND tenant_id = $3", gcId, accountId, tenantId)
	if err != nil {
		if err == sql.ErrNoRows {
			return errors.New(errGCNotFound)
		}
	...
	_, err = dbms.DoInTransaction(func(tx *sqlx.Tx) (any, error) {
		result, err := tx.Exec("DELETE FROM llm_global_contexts WHERE id = $1 AND account_id = $2 AND tenant_id = $3", gcId, accountId, tenantId)
		if err != nil {
