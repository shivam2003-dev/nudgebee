...
func RetriggerKnowledgebase(sc *security.RequestContext, accountId, kbId string) error {
	...
	if existingKB.KBType == KBTypeIntegration {
		...
		if err := triggerIntegrationKBSync(accountId, *existingKB.IntegrationID, *existingKB.KBSource, sc.GetSecurityContext().GetUserId()); err != nil {
			_ = updateKBStatus(dbms, kbId, string(KBStatusError))
...
func triggerIntegrationKBSync(accountId, integrationID, kbSource, triggeredBy string) error {
	ragServerURL := config.Config.RAGServerUrl
	if ragServerURL == "" {
	...
	req, err := http.NewRequest("POST", ragServerURL+"/api/v1/kb/retrigger_integration", bytes.NewBuffer(payloadBytes))
	if err != nil {
		return fmt.Errorf("kb: failed to prepare integration sync request: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")
