...
func DeleteKnowledgebase(sc *security.RequestContext, accountId, kbId string) error {
	...
	var kb Knowledgebase
	err = dbms.Db.Get(&kb, "SELECT id, name, account_id, tenant_id FROM llm_knowledgebases WHERE id = $1 AND account_id = $2", kbId, accountId)
	if err != nil {
		if err == sql.ErrNoRows {
			return errors.New(errKBNotFound)
		}
...
func GetKBLoadHistory(sc *security.RequestContext, accountId, kbId string) ([]KBLoadHistoryEntry, error) {
	...
	err = dbms.Db.Select(&entries, `
		SELECT id::text, document_count, total_tokens, embedding_provider, embedding_model,
		       request_status, error_message, created_at
		FROM rag_embedding_token_usage
		WHERE account_id = $1 AND collection_name = $2
		  AND operation_type = 'batch_embedding'
		ORDER BY created_at DESC
		LIMIT 50`, accountId, collectionName)
	if err != nil {
...
func RetriggerKnowledgebase(sc *security.RequestContext, accountId, kbId string) error {
	...
	err = dbms.Db.Get(&existingKB, "SELECT data, data_format, status FROM llm_knowledgebases WHERE id = $1 AND account_id = $2", kbId, accountId)
	if err != nil {
		if err == sql.ErrNoRows {
			return errors.New("knowledgebase not found")
		}
	...
	// Submit async re-embedding (reuses the update flow: delete old → create new)
	submissionCtx, cancel := context.WithTimeout(context.Background(), time.Duration(config.Config.AsyncOperationTimeoutSeconds)*time.Second)
	defer cancel()
...
func processKBEmbeddingsAsync(sc *security.RequestContext, accountId, kbId, data, format string) {
	...
	// Update status to active — document_count and last_loaded_at are updated
	// by the rag-server after successful embedding generation.
	err = updateKBStatus(dbms, kbId, string(KBStatusActive))
	if err != nil {
		slog.Error("kb: async - failed to update status to active", "error", err, "kb_id", kbId)
		return
	}
...
func processKBUpdateEmbeddingsAsync(sc *security.RequestContext, accountId, kbId, data, format, triggerType string) {
	...
	err = updateKBStatus(dbms, kbId, string(KBStatusActive))
	if err != nil {
		slog.Error("kb: async update - failed to update status to active", "error", err, "kb_id", kbId)
		return
	}
