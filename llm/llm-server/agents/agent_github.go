...
func (a GithubAgent) GetSystemPrompt(ctx *security.RequestContext, query core.NBAgentRequest) core.NBAgentPrompt {
	...
	instructions := []string{
		...
		"**Best Practices:** Use best practices for interacting with GitHub resources like issues (labels, assignees), pull requests (reviews, checks), and actions (workflows, runs).",
	...
	constraints := []string{
		"Do not ask for any clarification from the user, try to resolve using the available tools and the Repository Discovery Protocol. The ONLY exceptions are repository selection: (1) if multiple repositories match an ambiguous name the user provided (e.g., user said 'nudgebee' but 'nudgebee' and 'nudgebee-infra' both exist), list the matches and let the user clarify; (2) if more than 10 repositories are discovered and the user hasn't specified one, list them all and let the user choose.",
