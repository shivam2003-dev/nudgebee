...
func (a GithubAgent) GetSystemPrompt(ctx *security.RequestContext, query core.NBAgentRequest) core.NBAgentPrompt {
	...
	examples := []core.NBAgentPromptExample{
		...
		{
			Question: "check_ci_failure user: Migrations-Test-Civo CI has failed",
			AnswerSteps: []core.NBAgentPromptExampleAnswerStep{
				{
					Tool:        tools.ToolExecuteGithubCliCommand,
					Input:       "gh repo view --json nameWithOwner --jq '.nameWithOwner'",
				...
				{
					Tool:        tools.ToolExecuteGithubCliCommand,
					Input:       "gh run list --repo <repo_from_previous_step> --workflow 'Migrations-Test-Civo CI' --status failure --limit 1 --json id --jq '.[0].id'",
		...
		{
			Question: "Find all merged pull requests created by 'shiv' in the '<owner-name>/<repo-name>' repository.",
			AnswerSteps: []core.NBAgentPromptExampleAnswerStep{
				{
					Tool:  tools.ToolExecuteGithubCliCommand,
					Input: "gh search prs is:merged --repo <owner-name>/<repo-name> --author shiv",
				},
			},
		...
		},
		{
			...
			AnswerSteps: []core.NBAgentPromptExampleAnswerStep{
				...
				},
				{
					...
					Explanation: "Match the requested name against BOTH the login and name fields from the previous result (case-insensitive, tolerating separator/suffix and whitespace differences) so casual inputs like 'kankshit' resolve to 'Kankshit-02' and display-name inputs like 'Mayank Pande' resolve to login 'mayankpande88'. Use the canonical login from that match. The final response MUST explicitly note any non-trivial substitution (e.g. 'Assignee resolved from <requested-name> to <resolved-login>'). If the previous step yielded zero or multiple plausible matches, do NOT run this step — list the candidates (login + name) and ask the user to pick first.",
				},
			},
