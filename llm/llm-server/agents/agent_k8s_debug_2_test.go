	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     `My llm-server deployment in nudgebee namespace restarted recently, can you investigate why?`,
			},
		}
	for _, tc := range testCases {
		sc := security.NewRequestContextForTenantAccountAdmin(os.Getenv("TEST_TENANT"), tc.UserId, []string{tc.AccountId})
	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     `Can you investigate recent restarts with llm-server pod in nudgebee namespace`,
			},
		}
	for _, tc := range testCases {
		sc := security.NewRequestContextForTenantAccountAdmin(os.Getenv("TEST_TENANT"), tc.UserId, []string{tc.AccountId})
