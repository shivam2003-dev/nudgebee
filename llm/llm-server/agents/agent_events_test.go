...
func TestAgentAgentExecuteWithId(t *testing.T) {
	eventsAgent := AgentEvents{}
	sc := security.NewRequestContextForSuperAdmin()
	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId:      os.Getenv("TEST_ACCOUNT"),
				UserId:         os.Getenv("TEST_USER"),
				Query:          "Summarize event with Id 79501ed6-3580-4bf5-91f1-d18bf08d78cf",
			},
		}
	for _, tc := range testCases {
	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     "Get RCA of Event with Id 16f67e25-ec98-40e4-91b1-adaf2771d2f2",
			},
		}
	for _, tc := range testCases {
...
func TestEventAgentRecentEvent2(t *testing.T) {
	sc := security.NewRequestContextForSuperAdmin()
	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     "Get the details of Event with id - ba7fe88b-4578-412a-a7e4-a3bd97f7cecd",
			},
		}
	for _, tc := range testCases {
