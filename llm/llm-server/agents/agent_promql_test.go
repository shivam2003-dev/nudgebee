	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     "What is the CPU utilization for node k3s-nudgebee-dev-e606-2246bc-node-pool-4196-xet14?",
			},
		}
	for _, tc := range testCases {
	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     "{\"command\":\"rate(node_disk_io_time_seconds_total{namespace=\"nudgebee-agent\", pod=~\"nudgebee-agent-clickhouse.*\"}[5m])\",\"args\":null,\"context\":\"\"}",
			},
		}
	for _, tc := range testCases {
