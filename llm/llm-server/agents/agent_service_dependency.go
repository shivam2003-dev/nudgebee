...
func init() {
	...
	if config.Config.KGToolsEnabled {
		toolDescription = `Explores service dependencies, infrastructure topology, connectivity, and call chains. ` +
			...
			`Example: User says "What does webapp in the nudgebee namespace call? Account k8s-dev." → Correct command: "What does webapp in the nudgebee namespace call in account k8s-dev?" (keeps both scope qualifiers). Wrong command: "What does webapp in the nudgebee namespace call?" (drops the account — tool will then ask "which account?" because webapp exists in multiple). ` +
			`` +
	...
	toolInput := "Provide question in natural language to get the dependencies of the service"
