...
func executeAgent(ctx *security.RequestContext, agent NBAgent, request NBAgentRequest) (NBAgentResponse, error) {
	...
	kbChan := make(chan NBAgentPrompt, 1)
	go func(prompt NBAgentPrompt, selected []string) {
		userQuery := request.OriginalQuery
		if userQuery == "" {
			userQuery = request.Query
		}
		kbChan <- injectKBContext(ctx, request.AccountId, skillAgentNames, selected, agent.GetPlannerType(), prompt, userQuery)
	...
	if len(kbPrompt.Instructions) > 0 {
		basePrompt = kbPrompt
	}
...
func injectKBContext(ctx *security.RequestContext, accountId string, agentNames []string, selectedIds []string, plannerType AgentPlannerType, prompt NBAgentPrompt, userQuery string) NBAgentPrompt {
	...
	// selectedSet is nil when selection is disabled or skipped. When non-nil
	// (even empty) it represents the question-aware short-list and inherited
	// KBs are filtered against it.
	...
	// Aggregate KBs across the agent's own name plus any inherited ancestor names.
	// De-dup by id since the same KB could in principle be mapped to multiple
	// agents in the inheritance chain.
	...
	for i, name := range agentNames {
		...
		for _, kb := range fetched {
			...
			// Apply the question-aware filter only to inherited KBs. KBs mapped
			// directly to this agent's own name are always shown so a sub-agent's
			// scoped expertise is never hidden by the parent's selection.
