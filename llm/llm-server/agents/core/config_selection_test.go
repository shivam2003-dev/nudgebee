...
func TestFindConfigInQuery(t *testing.T) {
	configs := []toolcore.ToolConfig{
		...
		{
			Name: "gcp-dev - my-project-dev",
			Values: []toolcore.ToolConfigValue{
				{Name: "id", Value: "my-gcp-project-dev"},
	...
	testCases := []struct {
		...
		{
			name:     "Value match - my-gcp-project-dev",
			query:    "Get metrics for project my-gcp-project-dev",
			expected: "gcp-dev - my-project-dev",
		},
		{
		...
		},
		{
			...
			query:    "use project",
			expected: "", // "project" is not a whole config name or value
		},
		{
		...
		},
		{
			...
			query:    `gcloud logging read ... --project=my-gcp-project-dev using 'gcp-dev - my-project-dev' account`,
			expected: "gcp-dev - my-project-dev",
		},
	}
