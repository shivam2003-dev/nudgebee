...
func TestGetNameToToolIncludesAliases(t *testing.T) {
	...
	assert.NotNil(t, nameToTool["KUBECTL_EXECUTE"])
	assert.NotNil(t, nameToTool["EVENTS_EXECUTE"])

	// Aliases (uppercased)
	assert.NotNil(t, nameToTool["KUBECTL"])
	assert.NotNil(t, nameToTool["EVENTS"])
	assert.NotNil(t, nameToTool["EVENTS EXECUTE"])
