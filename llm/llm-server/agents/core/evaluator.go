...
func EvaluateAgentResponse(ctx *security.RequestContext, agentResponse NBAgentResponse, accountId string, availableTools []toolcore.NBTool, userId string) (AgentResponseEvaluationResult, error) {
	...
	completion, err := GenerateAndTrackLLMContent(ctx, userId, accountId, agentResponse.ConversationId, agentResponse.MessageId, "evaluation_agent", false, messageContent, true)
	if err != nil {
	...
	if len(agentResponse.AgentStepResponse) > 0 {
		...
		toolCompletion, err := GenerateAndTrackLLMContent(ctx, userId, accountId, agentResponse.ConversationId, agentResponse.MessageId, "evaluation_agent", false, toolMsg, true)
		if err == nil {
