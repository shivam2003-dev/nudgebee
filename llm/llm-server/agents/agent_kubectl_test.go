	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     "get the details for pod with IPs 10.42.158.223, 10.42.175.113, 10.42.158.160 across all namespaces",
			},
		}
	for _, tc := range testCases {
		sc := security.NewRequestContextForTenantAccountAdmin(os.Getenv("TEST_TENANT"), tc.UserId, []string{tc.AccountId})
	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     "kubectl exec deployment/nudgebee-agent-runner -n nudgebee-agent -- ls",
			},
		}
	for _, tc := range testCases {
		sc := security.NewRequestContextForTenantAccountAdmin(os.Getenv("TEST_TENANT"), tc.UserId, []string{tc.AccountId})
	...
	testCases :=
		[]struct {
		...
		}{
			{
				...
				AccountId: os.Getenv("TEST_ACCOUNT"),
				UserId:    os.Getenv("TEST_USER"),
				Query:     "Can you check network connectivity to google.com by launching new pod using ubuntu image in nudgebee-agent namespace ?",
			},
		}
	for _, tc := range testCases {
		sc := security.NewRequestContextForTenantAccountAdmin(os.Getenv("TEST_TENANT"), tc.UserId, []string{tc.AccountId})
