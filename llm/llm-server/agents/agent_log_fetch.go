...
`
	...
	if orig := strings.TrimSpace(request.OriginalQuery); orig != "" && orig != strings.TrimSpace(request.Query) {
		messages = append(messages, llms.TextParts(llms.ChatMessageTypeSystem, fmt.Sprintf("Original user question: %s", orig)))
	}
...
func generateLogQuery(ctx *security.RequestContext, request core.NBAgentRequest, provider string, fields []string, indices map[string]string, defaultIndex string) (string, error) {
	...
	if len(indices) > 0 {
		...
		if defaultIndex != "" {
			b.WriteString(fmt.Sprintf("  Account default index (used when `index` is omitted): %s\n", defaultIndex))
		}
		...
		b.WriteString(fmt.Sprintf("Account default log index (used when `index` is omitted): %s. Omit the `index` field unless the user's question implies a different source.\n", defaultIndex))
	}
