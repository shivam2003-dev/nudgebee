...
func getProvider(modelID string) string {
	// If ARN, extract the actual model ID at the end
	...
	// Remove version suffix if any (e.g., :0)
	modelID = strings.Split(modelID, ":")[0]

	// Extract provider name
	parts := strings.Split(modelID, ".")
	if len(parts) >= 2 && isRegion(parts[0]) {
		return parts[1]
	}
	return parts[0]
}

func isRegion(part string) bool {
	// Expand as needed
...
func (c *Client) CreateCompletion(ctx context.Context,
	...
	case "nudgebee":
		// Remove the provider prefix as the custom model from AWS are ARN names
		modelID = strings.Split(modelID, ".")[1]
		return createMetaCompletion(ctx, c.client, modelID, messages, options)
	default:
