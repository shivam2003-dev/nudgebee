...
func (g *GoogleAI) GenerateContent(
	...
	// dispatch is a closure so we can re-invoke after stripping ThinkingConfig
	// on a thinking-level-unsupported 400 (defensive backstop for the gate
	// above: if the allowlist falls behind a newly-restricted model, the gate
	// misses and the API surfaces the 400 — we recover by stripping the option
	// and retrying once rather than failing the user-facing call).
	var response *llms.ContentResponse
	dispatch := func() (*llms.ContentResponse, error) {
		if len(messages) == 1 {
			...
			return generateFromSingleMessage(ctx, g.client, opts.Model, theMessage.Parts, cfg, &opts)
		}
		return generateFromMessages(ctx, g.client, opts.Model, messages, cfg, &opts)
	}

	response, err = dispatch()
	if err != nil && cfg.ThinkingConfig != nil && isThinkingLevelUnsupportedError(err) {
		slog.Warn("googleai: model rejected ThinkingLevel; retrying without it",
			"model", opts.Model, "error", err.Error())
...
// supportsThinkingLevelOption returns true only for models that accept the
// string-based ThinkingLevel API (gemini-3.x). Gemini 2.5 thinking models
// (e.g. gemini-2.5-flash, gemini-2.5-pro) reason via the older numeric
// ThinkingBudget field — passing them ThinkingLevel returns HTTP 400
// "Thinking level is not supported for this model" and aborts the call.
// Narrower than isThinkingModel by design: that helper gates cost-accounting
// (must include 2.5), this one gates the request-config option (must not).
func supportsThinkingLevelOption(modelName string) bool {
	if modelName == "" {
		return false
	}
	m := strings.ToLower(modelName)
...
// isThinkingLevelUnsupportedError matches the specific Gemini API rejection
// for ThinkingLevel on a model that doesn't accept it. The full server message
// is "Thinking level is not supported for this model." surfaced inside a
// 400 INVALID_ARGUMENT — either as the raw error or wrapped through the
// streaming path ("error in stream mode: Error 400, Message: ...").
func isThinkingLevelUnsupportedError(err error) bool {
	if err == nil {
		return false
	}
