...
@pytest.fixture
def hasura_jwt_factory():
    """Mint a Hasura-style JWT with overridable claims."""
    ...
    def _mint(**overrides):
        ...
        now = datetime.now(timezone.utc)
        claims = {"x-hasura-user-id": "user-123", "x-hasura-default-role": "user"}
        payload = {
            "iat": now,
            "exp": now + timedelta(hours=1),
            "iss": HASURA_JWT_ISSUER,
            "aud": HASURA_JWT_AUDIENCE,
            "email": "test@example.com",
            HASURA_CLAIMS_NS: claims,
        }
        # ``hasura_claims`` is a fixture-level convenience for overriding
        # the inner Hasura claim map without rebuilding the whole payload.
        if "hasura_claims" in overrides:
            payload[HASURA_CLAIMS_NS] = overrides.pop("hasura_claims")
        payload.update(overrides)
        return pyjwt.encode(payload, PRIVATE_PEM, algorithm="RS256")
