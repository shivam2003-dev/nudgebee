"""JWT validation CVE-class matrix tests.
...
the validation to ``benchmark-server`` / ``benchmark`` (session) and
``urn:pollux:issuer`` / ``urn:pollux:hasura`` (Hasura).
"""
...
def test_session_rejects_hasura_audience_token(hasura_jwt_factory):
    """A real Hasura JWT replayed at the session-cookie path must NOT pass.
    This is the exact cross-service replay attack the iss/aud check stops."""
    from benchmark_server.middleware.auth import _extract_user_from_session

    token = hasura_jwt_factory()  # iss/aud = urn:pollux:*
    with pytest.raises((pyjwt.InvalidIssuerError, pyjwt.InvalidAudienceError)):
        _extract_user_from_session(token)


# ---------- Hasura JWT (Bearer header) ----------


def test_hasura_valid_token_extracts_user(hasura_jwt_factory):
    from benchmark_server.middleware.auth import _extract_user_from_hasura_jwt

    user = _extract_user_from_hasura_jwt(hasura_jwt_factory())
    assert user.user_id == "user-123"


def test_hasura_session_iss_rejected(hasura_jwt_factory):
    """A benchmark session JWT replayed at the Hasura path must NOT pass."""
    from benchmark_server.middleware.auth import _extract_user_from_hasura_jwt

    token = hasura_jwt_factory(iss="benchmark-server")
    with pytest.raises(HTTPException) as exc:
        _extract_user_from_hasura_jwt(token)
    assert exc.value.status_code == 401


def test_hasura_session_aud_rejected(hasura_jwt_factory):
    from benchmark_server.middleware.auth import _extract_user_from_hasura_jwt

    token = hasura_jwt_factory(aud="benchmark")
    with pytest.raises(HTTPException) as exc:
        _extract_user_from_hasura_jwt(token)
    assert exc.value.status_code == 401


def test_hasura_expired_rejected(hasura_jwt_factory):
    from benchmark_server.middleware.auth import _extract_user_from_hasura_jwt

    token = hasura_jwt_factory(
        iat=datetime.now(timezone.utc) - timedelta(hours=2),
        exp=datetime.now(timezone.utc) - timedelta(hours=1),
    )
    with pytest.raises(HTTPException) as exc:
...
def test_hasura_missing_user_id_rejected(hasura_jwt_factory):
    """A Hasura JWT without ``x-hasura-user-id`` is unusable for benchmark."""
    from benchmark_server.middleware.auth import _extract_user_from_hasura_jwt

    token = hasura_jwt_factory(hasura_claims={"x-hasura-default-role": "user"})
    with pytest.raises(HTTPException) as exc:
        _extract_user_from_hasura_jwt(token)
    assert exc.value.status_code == 401
