...
def _agents_root() -> Path:
    """Absolute path to llm/benchmark/llm/agents/ — the root of the agent tree."""
    # controllers/ -> benchmark_server/ -> llm/benchmark/ -> + llm/agents/
    return Path(__file__).resolve().parent.parent.parent / "llm" / "agents"


def _resolve_agent_dir(agent_name: str) -> Optional[Path]:
    """Resolve an agent name to its on-disk directory.

    Handles both the flat layout (e.g. llm/agents/logs/) and one level of
    sub-grouping (e.g. llm/agents/cloud/aws/) introduced by the
    capability-tier restructure. The public agent name stays flat — only
    the filesystem layout has the sub-group.
    """
    root = _agents_root()
    direct = root / agent_name
    if (direct / "config.yaml").exists():
        return direct
    for sub in sorted(root.iterdir()):
        if not sub.is_dir() or (sub / "config.yaml").exists():
            continue
        nested = sub / agent_name
...
def _iter_agent_config_files() -> List[Path]:
    """Return every agent's config.yaml — top level plus one level of sub-grouping."""
    root = _agents_root()
    configs: List[Path] = []
    if not root.exists():
        return configs
    configs.extend(root.glob("*/config.yaml"))
    configs.extend(root.glob("*/*/config.yaml"))
...
async def run_agent_benchmark_and_notify(  # noqa: C901
    ...
    # Use the root agents/ dir (not agent_dir.parent) — with the cloud/ sub-grouping
    # introduced by the capability-tier restructure, agent_dir.parent for cloud/aws
    # is llm/agents/cloud/, which would hide the shared llm/agents/conftest.py from
    # pytest's confcutdir.
    ...
    if infra_state and infra_state.get("status") == "deployed":
        ...
        try:
            ...
            if agent_dir is None:
                raise FileNotFoundError(f"agent dir not found: {agent}")
            needed = set(
...
async def trigger_agent_benchmark(
    ...
    Supported agents are discovered at runtime from llm/agents/. Today they
    include: logs, metrics, traces, events, finops, nubi, aws, azure, gcp.
    The exact list is available via GET /agents.
...
async def list_available_agents():
    ...
    project_root = Path(__file__).resolve().parent.parent.parent
    agents = []
    # Picks up both flat agents (llm/agents/logs/) and sub-grouped ones
    # (llm/agents/cloud/aws/) from the capability-tier restructure.
    for config_file in _iter_agent_config_files():
...
@router.get("/runs/users")
async def list_benchmark_run_users(authz=Depends(get_authz)):
    """Distinct users who own runs, for the runs-page "All Users" filter.

    Scoped exactly like ``/runs/list`` (same ``_scope_tenant_ids``), so a
    non-admin sees every contributor in their tenant(s) and super_admins see
    all — independent of which runs happen to be on the visible page.
    """
    return {"users": run_manager.list_run_users(tenant_ids=_scope_tenant_ids(authz))}
...
async def _run_single_test_task(run_id: str, test_index: int, config: dict):
    ...
    agent_dir = _resolve_agent_dir(agent)
    if agent_dir is None:
        run_manager.fail_run(run_id, f"agent dir not found: {agent}")
        return
    # Use the root agents/ dir (not agent_dir.parent) so pytest's --confcutdir
    # finds the shared llm/agents/conftest.py even for cloud-nested agents.
    agents_dir = _agents_root()
