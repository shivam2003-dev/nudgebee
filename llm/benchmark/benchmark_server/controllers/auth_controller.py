...
Programmatic clients are unaffected: they continue to authenticate via
Bearer JWT (the existing Hasura RS256 path in middleware/auth.py).
...
# ---------------------------------------------------------------------------
NOTIFICATION_SERVICE_URL = os.environ.get("NOTIFICATION_SERVICE_URL", "http://notifications:80")
...
def _send_magic_link_email(email: str, magic_link_url: str) -> bool:
    ...
    try:
        resp = requests.post(
            f"{NOTIFICATION_SERVICE_URL}/api/send_email",
            json={
                "recipients": email,
...
async def email_login(request: Request, email: str = Form("")):
    ...
    if user_data is None:
        ...
        logger.info("magic-link request for unregistered email: %s (silent)", normalized)
        return RedirectResponse("/signin?sent=1", status_code=302)
