...
def _update_kb_document_count(knowledgebase_id: str, document_count: int) -> None:
    """Update llm_knowledgebases with the latest doc count and load timestamp on success."""
    try:
        ...
        with engine.connect() as connection:
            with connection.begin():
                connection.execute(
                    sql_text(
                        "UPDATE llm_knowledgebases SET document_count = :doc_count, "
                        "last_loaded_at = NOW(), updated_at = NOW() WHERE id = :kb_id"
                    ),
                    {"doc_count": document_count, "kb_id": knowledgebase_id},
                )
        logger.info(f"[KB] Updated document_count={document_count} for KB {knowledgebase_id}")
    except Exception as e:
        logger.error(f"[KB] Failed to update document_count for KB {knowledgebase_id}: {e}")
...
def process_documents(
    ...
    # Update llm_knowledgebases with latest doc count (only on success, only for KBs)
    if knowledgebase_id:
        _update_kb_document_count(knowledgebase_id, tracker.expected_document_count or total_documents)
