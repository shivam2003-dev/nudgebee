...
This project was bootstrapped with [Create Next App](https://nextjs.org/). Major libraries the project uses [Material UI](https://mui.com/material-ui/getting-started/), Typescript, [Graphql](https://hasura.io/docs/latest/queries/overview/), [next-auth](https://next-auth.js.org/getting-started/introduction) and [vis-network](https://github.com/visjs/vis-network).
...
- **GraphQL API Layer**: All backend communication uses GraphQL via Hasura. Modules follow the `queryGraphQL` pattern in `HttpService` with TypeScript interfaces for requests.
