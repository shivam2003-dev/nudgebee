...
const CloudAccountTile = ({ cloudProvider, title, AddAccountModalComponent, addAccountButtonText, AddOrgModalComponent, addOrgButtonText }) => {
  ...
  const handleOpenEventGridModal = (account) => {
    ...
    apiKubernetes1
      ...
      .then((res) => {
        const data = res?.data?.data?.azure_get_onboard_eventgrid_url;
        if (data?.url) {
  ...
  const handleOpenEventBridgeModal = (account) => {
    ...
    apiKubernetes1
      ...
      .then((res) => {
        const data = res?.data?.data?.aws_get_onboard_eventbridge_url;
        if (data?.url) {
  ...
  const listCloudAccounts = () => {
    ...
    apiKubernetes1
      ...
      .then((res) => {
        const accounts = res?.data?.data?.accounts_list?.rows || [];
  ...
  return (
    <>
      ...
      <ListingLayout id={`${cloudProvider?.toLowerCase()}-integrations`}>
        ...
        >
          <FilterDropdown
            id={`${cloudProvider?.toLowerCase()}-status-filter`}
            label='Status'
