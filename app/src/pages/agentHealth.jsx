      ...
      {activeTab === 1 && (
        <>
          {!proxyLoading && proxyAgentHealthData.length === 0 ? (
            ...
            >
              ...
              {hasWriteAccess() ? (
                <CustomButton
                  variant='primary'
                  size='Medium'
                  text='Connect VM Agent'
                  onClick={() => router.push('/accounts/account-form?cloudProvider=VM_AGENT')}
                />
              ) : (
            ...
            <>
              <BoxLayout2
                id='proxy-agent-health'
                heading='Proxy Agent Health'
                sharingOptions={{
                  download: { enabled: false },
                  sharing: { enabled: false },
                }}
                marginTop={3}
              >
                {proxyData[0]?.status === 'NOT_CONNECTED' && <Typography color='red'>The Proxy Agent is not connected</Typography>}
                <CustomTable headers={HEADERS_PROXY} tableData={proxyAgentHealthData} loading={proxyLoading} />
              </BoxLayout2>
              {proxyDatasourcesData.length > 0 && (
                <BoxLayout2
                  id='proxy-datasources'
                  heading='Datasources'
                  sharingOptions={{
                    download: { enabled: false },
                    sharing: { enabled: false },
