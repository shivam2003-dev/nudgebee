...
export default function SignInV2({ providers, isOnPrem, samlEnabled }: any) {
  ...
  const renderMainView = () => (
    ...
    >
      ...
      {/* Divider between OAuth and SAML SSO */}
      {otherIconProviders.length > 0 && samlEnabled && (
        <Box sx={{ my: 3 }}>
          <Divider
      ...
      {/* SAML SSO Button - separate from OAuth grid */}
      {samlEnabled && (
        <Button
          variant='outlined'
          fullWidth
        ...
        >
          <LockIcon sx={{ fontSize: 18, color: colors.text.secondary }} />
          Single Sign-On
        </Button>
      )}
