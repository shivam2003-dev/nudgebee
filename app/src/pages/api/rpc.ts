...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  await context.with(trace.setSpan(context.active(), span), async () => {
    ...
    try {
      ...
      // --- session_variables require a NextAuth-decodable JWT. Bearer-only
      //     callers (token decrypted from Authorization but no JWT) hit the
      //     501 below. /api/graphql no longer accepts them either; the
      //     gateway needs a JWT to build session_variables. If a real
      //     non-browser caller surfaces, add bearer-payload decode here.
      if (!auth.jwt) {
        res.status(501).json(
          rpcError(rpcId, RPC_AUTH_ERROR, 'bearer_unsupported_on_rpc', {
            hint: 'NextAuth session required; bearer-only flow unsupported',
          })
        );
        return;
      }
      ...
      const result = await forwardAction({
        ...
        tenantId,
        userId,
        // Match the existing /api/graphql proxy: forward the decrypted token
        // (not the raw, possibly-encrypted Authorization header) so handlers
        // with forward_client_headers see the same Bearer value they'd see
        // through Hasura.
        clientAuthorization: `Bearer ${auth.token}`,
