...
export default function SignIn({ providers, selfSignupEnabled, samlEnabled }: any) {
  ...
  return (
    <BaseTemplate>
      ...
      {/* SSO providers slot (SAML in EE; empty in OSS) */}
...
export async function getServerSideProps(_context: any) {
  ...
  // Self-signup is a SaaS-only flow (creates a new tenant per sign-up).
  // EE pre-provisions the tenant via license; OSS is singleton-tenant. Fail
  // closed if license can't be resolved — hiding a link that would 404 is
  // better UX than showing it. The signup pages themselves are stripped
  // from the OSS build via .oss-exclude.
  let selfSignupEnabled = false;
  try {
    const license = await getLicenseDetails();
    selfSignupEnabled = license.tier === 'saas';
  } catch {
  ...
  return {
    props: { providers, selfSignupEnabled, samlEnabled },
  };
}
