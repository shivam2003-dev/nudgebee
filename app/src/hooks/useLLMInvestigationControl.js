...
function investigationReducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_MODELS':
      return { ...state, availableModels: action.availableModels, defaultModel: action.defaultModel };
    ...
    case 'RESET':
      return {
        ...initialState,
        // Preserve loaded model list across resets
...
export const useLLMInvestigationControl = (accountId) => {
  ...
  useEffect(() => {
    ...
    apiAskNudgebee
      ...
      .then((res) => {
        ...
        dispatch({ type: 'SET_MODELS', availableModels, defaultModel });
      })
  ...
  const stopInvestigation = useCallback(
    async (conversationId, currentStatus, onStopSuccess) => {
      ...
      try {
        ...
        const response = res?.data?.data?.ai_stop_investigation || {};
  ...
  const handleInvestigationGeneration = useCallback(
    async (finalQuery, llmSessionId, onSuccess, categorySource) => {
      const sessionKey = currentSessionRef.current;
      ...
      if (!response?.data?.query) {
        const status = res?.data?.errors?.[0]?.extensions?.internal?.response?.status;
        const budgetErrorMessage = res?.data?.errors?.[0]?.extensions?.internal?.response?.body?.errors?.[0]?.message;
        const isBudgetError = status === 429 || budgetErrorMessage?.includes('budget');
        snackbar.error(
          isBudgetError
            ? `Monthly Budget Limit exceeded for this account. Contact ${getBrandTitle()} Support team.`
            : parseHttpResponseBodyMessage(res) || 'Failed to start investigation.'
        );
    ...
    async ({
      ...
      try {
        if (apiMode === 'workflow') {
          ...
          await handleInvestigationGeneration(finalQuery, llmSessionId, onSuccess, categorySource);
        }
      } catch (error) {
        console.error(error);
