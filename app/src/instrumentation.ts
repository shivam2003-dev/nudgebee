...
export async function register() {
  ...
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    // Dynamic import — keeps EE init out of the Edge runtime bundle.
    // EE init transitively imports HttpService which uses Node's `crypto`,
    // which the Edge runtime can't resolve. Wrapped in try/catch the same
    // way as the other registrations below so a missing/broken EE module
    // doesn't crash the whole instrumentation pass.
    try {
