...
query GetCloudAccounts {
    cloud_accounts: get_cloud_accounts_v2(where: __WHERE__) {
      rows {
        cloud_provider
        account_type
        id
...
const apiDashboard = {
  ...
  getInsights: async function (accountId: string) {
    ...
    query InsightSummary {
      insight_v2(where: __WHERE__, order_by: [{column: "created_at", order: desc}]) {
        rows {
          account_id
          unique_id
          title
    ...
    if (response?.data?.data?.insight_v2?.rows) {
      response.data.data.insight_v2.rows = parseInsightJsonFields(response.data.data.insight_v2.rows);
    }

    // Sort by severity: Critical > High > Medium > Low
    const insights = response?.data?.data?.insight_v2?.rows;
