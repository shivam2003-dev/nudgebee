...
export function getRelayServerEndpoint() {
  return isServer() ? process.env.RELAY_SERVER_ENDPOINT || 'http://localhost:52832' : '/api/relay';
}
...
function getGQLDefaultHeaders(existingHeaders?: Record<string, string>) {
  ...
  // Merge any additional custom headers (e.g. x-hasura-user-tenant-id) that aren't trace or default headers
  if (existingHeaders) {
