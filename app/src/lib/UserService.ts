...
const USER_BY_PROVIDER_ACCOUNT_QUERY = `query UserByProviderAccount($where: UserByProviderAccountWhereRequest) {
  user_by_provider_account_v2(where: $where) {
...
query GetUserByProvider($provider: String!, $id: String!) {
  user_auth_by_username_v2(where: {provider: {_eq: $provider}, user_id: {_eq: $id}}) {
    rows {
      id
      username
    }
...
query GetUserByProviderAndAccountId($username: String!, $provider: String!) {
  user_auth_by_username_v2(where: {provider: {_eq: $provider}, username: {_eq: $username}}) {
    rows {
      auth_id
      credential
      auth_tenant_id
...
query GetUsers($offset: Int, $limit: Int) {
  users: admin_get_users_by_tenant_v2(limit: $limit, offset: $offset, order_by: [{column: "username", order: asc}], where: __WHERE__) {
    rows {
      display_name
      id
      status
...
query GetUsersByTenant($offset: Int, $limit: Int) {
  admin_get_users_by_tenant_v2 (limit: $limit, offset: $offset, order_by: __ORDERBY__, where: __WHERE__) {
    rows{
      display_name
      id
      status
  ...
  admin_get_users_grouping_by_tenant_v2 (where: __WHERE__){
    rows{
      count
    }
  }
...
query FilterUserByStatus($limit: Int, $offset: Int) {
  users: admin_get_users_by_tenant_v2(where: __WHERE__, limit: $limit, offset: $offset) {
    rows {
      id
      display_name
      status
...
query FilterUserByRole($limit: Int, $offset: Int) {
  users: admin_get_users_by_tenant_v2(limit: $limit, offset: $offset) {
    rows {
      id
      status
      display_name
...
query GetUserGroups($offset: Int, $limit: Int) {
  admin_get_user_groups_v2(limit: $limit, offset: $offset, order_by: [{column:"name", order:asc}], where: __WHERE__) {
    rows{
      id
      name
      description
  ...
  admin_get_user_groups_grouping_v2(where: __WHERE__) {
    rows{
      count
    }
  }
...
query GetUserGroup($id:uuid) {
  user_groups: admin_get_user_groups_v2(where: {id: {_eq: $id}}) {
    rows {
      id
      name
      description
...
query GetUserGroupUsers($offset: Int, $limit: Int, $id: String) {
  usergroup_users_v2(limit: $limit, offset: $offset, where: {group_id: {_eq: $id}}) {
    rows {
      user_id
      display_name
      username
  ...
  usergroup_users_grouping_v2(where: {group_id: {_eq: $id}}) {
    rows {
      count
    }
  }
...
export async function getUserById({
  ...
  const rows = response?.data?.data?.user_details_v2?.rows || [];
...
export async function getUserByAccountIdAndAccountProvider({
  ...
  const rows = response?.data?.data?.user_by_provider_account_v2?.rows || [];
  return {
    data: {
...
export async function getUserByUsernameAndAccountProviderAndCredential({
  ...
  const rows = response?.data?.data?.user_auth_by_username_v2?.rows || [];
...
export async function createUserAuthAccount(request: CreateUserAccountRequest) {
  ...
  const result = response.data.data.user_create_auth;
  return {
    data: {
      id: result.id,
      name: result.name,
...
const USER_SUPER_ADMIN_ROLE_QUERY = `query UserSuperAdminRole($where: UserSuperAdminRoleWhereRequest) {
  user_super_admin_role_v2(where: $where) {
    rows { role }
  }
}`;
...
export async function getUserSuperAdminRole(userId: string): Promise<string | null> {
  ...
  return response?.data?.data?.user_super_admin_role_v2?.rows?.[0]?.role || null;
}
...
export async function createUser(request: CreateUserRequest) {
  ...
  return {
    data: response.data.data.users_insert_one,
  };
}
...
export async function onboardUser(request: CreateUserRequest) {
  ...
  return { data: response.data.data.user_onboard };
}
...
export async function updateUserStatus(user_id: string, status: string) {
  ...
  return {
    data: response.data.data.user_update_status,
  };
}
...
export async function getUserGroupUsers(request: GetUserGroupRequest) {
  ...
  const rows = response.data?.data?.usergroup_users_v2?.rows || [];
  const count = response.data?.data?.usergroup_users_grouping_v2?.rows?.[0]?.count || 0;
...
export async function listUserTenantRoles(username: string, tenantId: string) {
  const query = `
    mutation UserTenantRoles($object: user_tenant_roles_input!) {
      user_tenant_roles(object: $object) {
        roles {
          entity_id
          entity_type
          role
  ...
  return {
    data: response?.data?.data?.user_tenant_roles?.roles || [],
    tenantName: response?.data?.data?.user_tenant_roles?.tenant_name,
  };
}
...
const USER_ACCOUNT_IDS_BY_TENANT_QUERY = `query UserAccountIdsByTenant($where: UserAccountIdsByTenantWhereRequest) {
  user_account_ids_by_tenant_v2(where: $where) {
    rows { id }
  }
}`;
...
export async function getAccountByTenant(tenantId: string) {
  ...
  const rows = response?.data?.data?.user_account_ids_by_tenant_v2?.rows || [];
  return {
...
export async function getFeatures() {
  ...
  if (!cachedTenantAttrList) {
    ...
    const features = response?.data?.data?.feature_v2?.rows || [];
...
export async function getTenantIdByName(tenantName: string, username: string, skipUserFilter = false): Promise<string | null> {
  ...
  return response?.data?.data?.tenant_v2?.rows?.[0]?.id ?? null;
}
...
export async function syncUserRoles(
  ...
  try {
    ...
    const result = response?.data?.data?.user_sync_roles;
