...
// next to this module is now the sole source of truth (Hasura's metadata
// directory was deleted when the Hasura service was removed). Edit it
...
export type RpcRoute = {
  ...
  // Mirrors Hasura's role-based gate: callers whose active role isn't in the
  // set get blocked at the gateway. Empty set means no role can invoke
  // (admin-secret only under Hasura → super_admin only here).
...
export function loadRpcRoutes(): Record<string, RpcRoute> {
  ...
  let doc: ActionsYaml;
  try {
    doc = yaml.load(raw) as ActionsYaml;
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
...
// Hasura applies GraphQL input coercion (notably: single value → single-
// element list when the schema declares a list type). Our bypass skips
// Hasura's validation pipeline, so we have to do the same coercion ourselves
...
// frontend code passes single values relying on Hasura's recursive coercion.
...
export function loadActionInputSchema(): ActionInputSchema {
  ...
  for (const def of doc.definitions) {
    // Hasura emits one `type Query { actionName(...) }` block per action, so
