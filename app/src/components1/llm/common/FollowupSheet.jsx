...
const NubiBeeIcon = () => (
  <Box component='img' src={getNubiIconUrl()} alt='Nudgebee' sx={{ flexShrink: 0, width: '22px', height: '22px', display: 'block' }} />
);
...
const FollowupSheet = ({ followup, accountId, conversationId, selectedModel, popup, onStop, onSubmitted }) => {
  ...
  const renderHead = () => (
    ...
    >
      ...
      {onStop && (
        <CustomTooltip title='Stop conversation' placement='top'>
          <Box
            component='button'
            type='button'
            onClick={onStop}
            ...
            sx={{
              ...
              '&:hover': { color: colors.text.error || '#dc2626', background: 'rgba(220,38,38,0.08)' },
            }}
          >
  ...
  const renderFilter = () => (
    ...
    >
      <SearchIcon sx={{ fontSize: '14px', color: colors.text.tertiary, flexShrink: 0 }} />
      <Box
        component='input'
        type='text'
      ...
      {filter && (
        <Box
          component='span'
          sx={{ fontSize: '11px', fontWeight: 500, color: colors.text.tertiary, flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}
        >
  ...
  const renderFreetextRow = () => (
    <Box
      sx={{
        ...
        margin: '8px 0 4px',
        padding: '8px 10px',
        borderTop: `1px dashed ${colors.border.nudgebeeSuggestion}`,
      }}
    >
      <EditOutlinedIcon sx={{ fontSize: '14px', color: colors.text.tertiary, flexShrink: 0 }} />
      <Box
        component='input'
        type='text'
      ...
      {freeText.trim() && (
        <Box
          ...
          sx={{
            ...
            '&:hover': { background: colors.button.primaryHover },
          }}
        >
          {pendingAnswer && isSubmitting ? <ThreeDotsLoader size={4} color={colors.text.white} /> : <ArrowForwardIcon sx={{ fontSize: '13px' }} />}
        </Box>
      )}
    </Box>
  );
  ...
  const renderSingle = () => (
    <Box
      ...
      sx={{ display: 'flex', flexDirection: 'column', gap: '2px', maxHeight: '240px', overflow: 'auto', margin: '0 -2px', padding: '0 2px' }}
    >
      {visibleOptions.length === 0 && (
        <Box sx={{ padding: '12px 10px', fontSize: '12.5px', color: colors.text.tertiary, fontStyle: 'italic' }}>No matches</Box>
      )}
          ...
          >
            <Box
              sx={{
                ...
                height: '18px',
                borderRadius: '4px',
                background: isPending || isFocused ? colors.primary : colors.background.shimmerBase,
                border: `1px solid ${isPending || isFocused ? colors.primary : colors.border.nudgebeeSuggestion}`,
                display: 'grid',
                placeItems: 'center',
                fontFamily: 'ui-monospace, "SF Mono", Menlo, monospace',
                fontWeight: 600,
                fontSize: '10px',
                lineHeight: 1,
                color: isPending || isFocused ? colors.text.white : colors.text.tertiary,
                transition: 'all 0.1s',
              }}
            >
              {isPending ? <CheckIcon sx={{ fontSize: '11px' }} /> : i + 1}
            </Box>
            <Box sx={{ flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: isPending ? 600 : 500 }}>
              {option}
            </Box>
            {isPending && isSubmitting ? (
              <ThreeDotsLoader color={colors.primary} sx={{ flexShrink: 0, mr: '4px' }} />
            ) : (
              <ArrowForwardIcon
                sx={{
                  fontSize: '12px',
                  color: isPending || isFocused ? colors.primary : 'transparent',
                }}
              />
            )}
          </Box>
        ...
        return (
          <Box
            ...
            sx={{
              ...
              ...(isSelected && {
                background: colors.background.primaryLightest,
                borderColor: isFocused ? colors.primary : colors.border.primaryLight,
              }),
            }}
          >
            <Box
              sx={{
                ...
                height: '14px',
                borderRadius: '3px',
                border: `1.5px solid ${isSelected ? colors.primary : colors.border.nudgebeeSuggestion}`,
            ...
            >
              {isSelected && <CheckIcon sx={{ fontSize: '9px', color: colors.text.white }} />}
            </Box>
  ...
  const renderMultiFooter = () => (
    <Box
      sx={{
        ...
        borderTop: `1px solid ${colors.border.nudgebeeSuggestion}`,
      }}
    >
      <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '11.5px', color: colors.text.tertiary, fontWeight: 500 }}>
        <Box
          sx={{
            width: '5px',
            height: '5px',
            borderRadius: '999px',
            background: selectedMulti.length > 0 ? colors.primary : colors.border.nudgebeeSuggestion,
          }}
        />
        <Box component='strong' sx={{ color: selectedMulti.length > 0 ? colors.primary : colors.text.tertiary, fontWeight: 700 }}>
      ...
      <Box
        ...
        sx={{
          ...
          borderRadius: '4px',
          transition: 'all 0.1s',
          '&:hover': { color: colors.primary, background: colors.background.primaryLightest },
        }}
      >
        Select all
      </Box>
      ...
      <Box
        ...
        sx={{
          ...
          borderRadius: '6px',
          border: 'none',
          background: selectedMulti.length === 0 ? colors.background.shimmerBase : colors.primary,
          color: selectedMulti.length === 0 ? colors.text.tertiary : colors.text.white,
          fontWeight: 600,
          fontSize: '12px',
          letterSpacing: '-0.005em',
          ...
          '&:hover': {
            background: selectedMulti.length === 0 ? colors.background.shimmerBase : colors.button.primaryHover,
          },
        }}
      >
        {pendingAnswer && isSubmitting ? <ThreeDotsLoader size={4} color={colors.text.white} /> : null}
        {pendingAnswer && !isSubmitting ? <CheckIcon sx={{ fontSize: '13px' }} /> : null}
  ...
  const renderYn = () => {
    ...
    const yesBg = isDanger ? '#dc2626' : colors.primary;
    const yesBgHover = isDanger ? '#b91c1c' : colors.button.primaryHover;

    return (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '14px 14px' }}>
        <Box
          component='button'
          type='button'
        ...
        >
          {noPending && isSubmitting ? (
            <ThreeDotsLoader size={4} color={colors.text.primary} />
          ) : noPending ? (
        ...
        >
          {yesPending && isSubmitting ? (
            <ThreeDotsLoader size={4} color={colors.text.white} />
          ) : yesPending ? (
            <CheckIcon sx={{ fontSize: '14px' }} />
          ) : null}
          {yesLabel}
  ...
  const renderInput = () => (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '6px', padding: '0 14px 12px' }}>
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '2px 0' }}>
        <Box
          component='textarea'
          ref={textareaRef}
          rows={1}
        ...
        <Box
          ...
          sx={{
            ...
            borderRadius: '7px',
            border: 'none',
            background: pendingAnswer ? colors.primary : textareaValue.trim() && !isSubmitting ? colors.primary : colors.background.shimmerBase,
            color: pendingAnswer ? colors.text.white : textareaValue.trim() && !isSubmitting ? colors.text.white : colors.text.tertiary,
            display: 'grid',
            placeItems: 'center',
            ...
            opacity: isSubmitting ? 0.85 : 1,
            '&:hover': {
              background: textareaValue.trim() && !isSubmitting ? colors.button.primaryHover : colors.background.shimmerBase,
            },
          }}
        >
          ...
          {isLongQuestion && (
            <Box
              sx={{
                ...
                color: '#1E293B',
                wordBreak: 'break-word',
                marginBottom: sheetType === 'single' || sheetType === 'multi' ? '14px' : '0px',
                paddingBottom: sheetType === 'single' || sheetType === 'multi' ? '12px' : '0px',
