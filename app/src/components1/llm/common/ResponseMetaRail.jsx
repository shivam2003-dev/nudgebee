...
const Dot = () => (
  <Box component='span' sx={{ color: colors.text.tertiary, fontSize: '11px', userSelect: 'none', lineHeight: 1 }}>
    ·
  </Box>
);
...
const countItem = (key, tone, count, onClick) => {
  ...
  return {
    key,
    node: <CustomChip variant='count' tone={tone} count={count} label={label} onClick={onClick} ariaLabel={`${count} ${label}, open details`} />,
  };
};
...
const buildItems = (props) => {
  ...
  if (props.taskCount > 0 && props.onOpenTasks) {
    items.push(countItem('tasks', 'blue', props.taskCount, props.onOpenTasks));
  }
  if (props.contextCount > 0 && props.onOpenContexts) {
    items.push(countItem('contexts', 'lavender', props.contextCount, props.onOpenContexts));
  }
  if (props.memoryCount > 0 && props.onOpenMemories) {
    items.push(countItem('memories', 'pink', props.memoryCount, props.onOpenMemories));
  }
  if (props.duration) {
    // `boundary: true` swaps the trailing separator from `·` to `|` — visually distinguishes
    // "how long it took" from "when it happened".
    items.push({ key: 'duration', node: <CustomChip variant='info' label={props.duration} />, boundary: true });
  }
  if (props.absoluteTime) {
    items.push({ key: 'time', node: <CustomChip variant='info' label={props.absoluteTime} /> });
  }
  return items;
};
...
const ResponseMetaRail = ({
  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        <Box key={item.key} sx={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
          {item.node}
