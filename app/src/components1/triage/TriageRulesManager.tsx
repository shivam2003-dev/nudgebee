    ...
    const data = paginatedRules.map((rule) => {
      ...
      const getStatusDisplay = () => {
        if (rule.is_system_rule && rule.is_overridden) {
          return <CustomLabels text='Disabled (Override)' variant='grey' dot />;
        }
        return <CustomLabels text={rule.enabled ? 'Enabled' : 'Disabled'} variant={rule.enabled ? 'green' : 'grey'} dot />;
      };
      ...
      return [
        ...
        {
          component: <CustomLabels text={getRuleTypeLabel(rule.rule_type)} variant='grey' />,
        },
