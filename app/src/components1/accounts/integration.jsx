...
const ACCOUNT_CARD_STYLES = {
  container: {
    ...
    borderLeft: `6px solid ${colors.border.primaryLight}`,
    ...
    '&:hover': {
      ...
      boxShadow: '0px 4px 10px 0px rgba(232, 232, 232, 0.25)',
      background: colors.background.intigrationCard, // typo fix from original 'intigrationCard' if applicable
      borderLeft: `6px solid ${colors.border.primary}`,
    },
  },
};
  ...
  return (
    ...
    >
      ...
      {needsChannelMapping && (
        ...
        >
          <Typography fontSize='10px' color='#B45309' fontWeight={600} lineHeight={1}>
            Channel Missing
          </Typography>
        </Box>
      )}
      ...
      <Box display='flex' alignItems='center' width='100%' gap={2}>
        ...
        <Box display='flex' flexDirection='column' flexGrow={1} minWidth={0}>
          <Typography color={colors.text.secondary} fontSize={'16px'} fontWeight={600} noWrap>
            {label === 'Kubernetes' ? 'Kubernetes Clusters' : label}
          </Typography>

          <Typography color={colors.text.tertiary} fontSize={'12px'} fontWeight={400} sx={{ mb: 0.5 }} noWrap>
          ...
          <Stack direction='row' spacing={2} alignItems='center' height='20px'>
            {active > 0 && (
              <Stack direction='row' spacing={0.5} alignItems='center'>
                ...
                <Typography fontSize='12px' color={colors.text.tertiary}>
                  Active
                </Typography>
                <Typography fontSize='12px' fontWeight={600} color={colors.text.secondary}>
                  {active}
                </Typography>
              </Stack>
            )}
            ...
            {disabled > 0 && (
              <Stack direction='row' spacing={0.5} alignItems='center'>
                <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: colors.border.secondary }} />
                <Typography fontSize='12px' color={colors.text.tertiary}>
                  Inactive
                </Typography>
                <Typography fontSize='12px' fontWeight={600} color={colors.text.secondary}>
            ...
            {!hasAnyConnections && !isDisabled && (
              <Typography fontSize='12px' color={colors.text.tertiary} fontStyle='italic'>
                No connections
              </Typography>
            )}
          </Stack>
...
const Integrations = () => {
  ...
  const SectionTitle = ({ label, count, color, icon }) => (
    <Box display='flex' alignItems='center' gap='10px' mb={2} pb={1} borderBottom={`2px solid ${colors.border.vertical}`} width='100%'>
      <Box sx={{ width: '4px', height: '20px', borderRadius: '2px', bgcolor: color }} />
      {icon && <SafeIcon src={icon} alt='' width={20} height={20} style={{ filter: 'grayscale(100%)' }} />}
      <Typography fontSize='18px' fontWeight={600} color={colors.text.secondary}>
        {label}
      </Typography>
      <Typography fontSize='14px' fontWeight={500} color={colors.text.tertiary}>
        ({count})
      </Typography>
    </Box>
  );
    ...
    >
      {loading ? (
        ...
        <>
          ...
          {renderGrid(connectedSections, { label: 'Connected', color: colors.background.primary })}
          {renderGrid(availableSections, { label: 'Available', color: colors.background.warning })}
        </>
      )}
    </BoxLayout2>
  );
