import { Box, Button, Container, IconButton, Menu, Tooltip, tooltipClasses, Typography } from '@mui/material';
...
const SideDrawerButton = ({ open = false, item = {}, handleDrawerOpen, isFirstItem }) => {
  ...
  return (
    <React.Fragment>
      ...
      >
        {isActive && <Box sx={{ width: '4px', height: '100%', position: 'absolute', left: 0, background: '#FACF39' }} />}
        <Box
          sx={{
            display: 'flex',
                ...
                {menuItems?.map((item, idx) => (
                  <React.Fragment key={item.text + '-' + idx}>
                    <SideDrawerButton
                      ...
                      isFirstItem={idx === 1}
                    />
                    {idx === 0 && <Box sx={{ borderTop: `1px solid ${colors.border.secondaryLightest}`, my: '2px' }} />}
                  </React.Fragment>
                ))}
              </Box>
              <Box
