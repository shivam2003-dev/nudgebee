...
const DevOpsTimelineMUI = ({ eventId }: { eventId: string }) => {
  ...
  return (
    <Box sx={{ margin: '8px 24px 0px 24px' }}>
      {/* Header */}
      <Box sx={{ mb: 3, pb: 2, borderBottom: 1, borderColor: 'divider', display: 'flex', alignItems: 'center', gap: '4px' }}>
        <Typography sx={{ fontSize: '14px', fontWeight: 500, color: colors.text.secondary }}>Event ID:</Typography>
        <CopyableText
          ...
          return (
            <Box component='li' key={index} sx={{ display: 'flex', gap: 2, minHeight: 60, pb: 0 }}>
              ...
              <Box display='flex' alignItems='flex-start' justifyContent='right' color='text.secondary' minWidth={140} pt={0.5}>
                <Typography variant='caption' sx={{ fontFamily: 'monospace', fontSize: '12px' }}>
              ...
              >
                <Box display='flex' flexDirection='column'>
                  ...
                  <Typography className='timeline-text' sx={{ fontSize: '13px', fontWeight: 400, color: colors.text.secondary }}>
                    {item.summary}
                  </Typography>
                </Box>
              </Box>
