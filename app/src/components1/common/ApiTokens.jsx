...
const ApiTokens = ({ open, title, onClose }) => {
  ...
  return (
    <>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 'var(--ds-space-5)' }}>
          ...
          {createdToken && (
            ...
            >
              ...
              <Typography sx={{ fontSize: 'var(--ds-text-small)', color: colors.text.tertiary, mb: 'var(--ds-space-2)' }}>
              ...
              <Box sx={{ mt: 'var(--ds-space-3)', display: 'flex', justifyContent: 'flex-end' }}>
                <CustomButton
                  size='Small'
          ...
          {showCreateForm && (
            ...
            >
              ...
              <Box sx={{ display: 'flex', gap: 'var(--ds-space-3)', justifyContent: 'flex-end' }}>
                <CustomButton
                  size='Medium'
                  text='Cancel'
                  variant='secondary'
          ...
          {/* Tokens List */}
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 'var(--ds-space-4)' }}>
                ...
                <Table>
                  <TableHead>
                    <TableRow sx={{ bgcolor: colors.background.primaryLightest }}>
                      <TableCell sx={{ fontWeight: 'var(--ds-font-weight-semibold)', color: colors.text.secondary }}>Name</TableCell>
                      <TableCell sx={{ fontWeight: 'var(--ds-font-weight-semibold)', color: colors.text.secondary }}>Created</TableCell>
                      <TableCell sx={{ fontWeight: 'var(--ds-font-weight-semibold)', color: colors.text.secondary }}>Last Used</TableCell>
                      <TableCell sx={{ fontWeight: 'var(--ds-font-weight-semibold)', color: colors.text.secondary, width: '100px' }}>
                        Actions
                      </TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {tokens.map((token) => (
                      <TableRow key={token.id} sx={{ '&:hover': { bgcolor: colors.background.primaryLightest } }}>
                        <TableCell sx={{ color: colors.text.secondary, fontWeight: 'var(--ds-font-weight-medium)' }}>{token.name}</TableCell>
                        <TableCell sx={{ color: colors.text.tertiary, fontSize: 'var(--ds-text-body-lg)' }}>{formatDate(token.created_at)}</TableCell>
                        <TableCell sx={{ color: colors.text.tertiary, fontSize: 'var(--ds-text-body-lg)' }}>
                          {formatDate(token.accessed_at)}
                        </TableCell>
                        <TableCell>
                          <IconButton
                            size='small'
          ...
          >
            <CustomButton
              ...
              startIcon={<InfoIcon sx={{ fontSize: 'var(--ds-text-title)' }} />}
            />
            <CustomButton
              variant='secondary'
              size='Medium'
              onClick={handleClose}
              text='Close'
              sx={{ padding: 'var(--ds-space-2) var(--ds-space-4)' }}
            />
          </Box>
        </Box>
      </Modal>
      ...
      >
        <Box sx={{ color: colors.text.tertiary, fontSize: 'var(--ds-text-body-lg)', lineHeight: '20px' }}>
          <Typography sx={{ mb: 'var(--ds-space-3)' }}>
          ...
          <Typography sx={{ mb: 'var(--ds-space-2)' }}>First, exchange your API token for a temporary JWT token:</Typography>
          <Box
            sx={{
              mb: 'var(--ds-space-4)',
              p: 'var(--ds-space-3)',
          ...
          <Typography sx={{ mb: 'var(--ds-space-2)' }}>Use the JWT token from Step 1 to make GraphQL API calls:</Typography>
          <Box
            sx={{
              mb: 'var(--ds-space-4)',
              p: 'var(--ds-space-3)',
          ...
          <Box component='ul' sx={{ pl: 'var(--ds-space-4)', mb: 'var(--ds-space-3)' }}>
            <li style={{ marginBottom: 'var(--ds-space-2)' }}>
      ...
      >
        ...
        <DialogContent sx={{ pb: 'var(--ds-space-4)' }}>
          <Typography sx={{ color: colors.text.tertiary, fontSize: 'var(--ds-text-body-lg)', lineHeight: '20px' }}>
            Are you sure you want to delete &quot;{deleteDialog.token?.name}&quot;? This action cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ p: 'var(--ds-space-4) var(--ds-space-5) var(--ds-space-5) var(--ds-space-5)', gap: 'var(--ds-space-3)' }}>
