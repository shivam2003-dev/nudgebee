...
const IntegrationDynamicFormModal = ({
  ...
  const [workflowRules, setWorkflowRules] = useState([{ match: [{ key: '', value: '' }], workflowId: '', accountId: '' }]);
  ...
  // Hydrate workflowRules from saved workflow_mapping. Single canonical shape:
  //   { rules: [{ match: {k:v|[v,...]}, workflowId }] }
  // Match values may be string or string-array — array values render as
  // comma-separated text in the input; submit splits them back to JSON arrays.
  useEffect(() => {
    if (integrationName !== 'workflow_webhook') return;
    if (!editData?.integration_config_values?.workflow_mapping) return;
    const parsed = safeJSONParse(editData.integration_config_values.workflow_mapping) || {};
    if (!Array.isArray(parsed.rules) || parsed.rules.length === 0) return;
    const formatMatchValue = (value) => {
      if (Array.isArray(value))
        return value
          .map((v) => String(v ?? '').trim())
  ...
  }, [editData, integrationName]);

  // Fetch workflows for *every* selected account so the THEN dropdown can
  // list them. workflow_webhook is multi-account: a single integration may
  // route to workflows across multiple accounts via workflow_mapping rules.
  // Re-runs when the user changes the account selection. Workflow labels
  // are suffixed with the account name when more than one account is
  // selected so users can disambiguate same-named workflows across accounts.
  useEffect(() => {
    if (integrationName !== 'workflow_webhook') return;
    const formAccountIds = Array.isArray(formValues.account_id)
      ? formValues.account_id.filter(Boolean)
      : formValues.account_id
      ? [formValues.account_id]
      : [];
    const fallbackAccountIds = (editData?.integrations_cloud_accounts || []).map((a) => a.cloud_account_id).filter(Boolean);
    const accountIds = formAccountIds.length > 0 ? formAccountIds : fallbackAccountIds;
    if (accountIds.length === 0) {
      setWorkflowOptions([]);
      return;
    }
    const accountNameById = new Map((config.properties?.account_id?.possible_values || []).map((opt) => [opt.value, opt.label]));
    setIsLoadingWorkflows(true);
    // Page through workflow_list per account until next_page_token is empty
    // so accounts with more than the page size don't silently drop options.
    // Hard cap on pages defends against a runaway loop if the server ever
    // returns a non-empty token without advancing.
    const fetchAllWorkflows = async (accId) => {
      const pageSize = 200;
      const maxPages = 25;
      const acc = [];
      let token;
      for (let page = 0; page < maxPages; page++) {
        const res = await apiWorkflow.listWorkflows(accId, undefined, undefined, undefined, pageSize, token);
        const list = res?.data?.workflow_list?.workflows || [];
        acc.push(...list);
        token = res?.data?.workflow_list?.next_page_token || '';
    ...
    Promise.all(
      accountIds.map((accId) =>
        fetchAllWorkflows(accId)
          .then((workflows) => ({ accId, workflows }))
          .catch((err) => {
            console.error('Failed to fetch workflows for workflow_mapping:', err, 'account', accId);
            return { accId, workflows: [] };
          })
      )
    )
      .then((results) => {
        const showAccountSuffix = accountIds.length > 1;
        const opts = [];
        for (const { accId, workflows } of results) {
          const accName = accountNameById.get(accId) || accId;
          for (const w of workflows) {
            opts.push({
              label: showAccountSuffix ? `${w.name} (${accName})` : w.name,
              value: w.id,
              accountId: accId,
            });
          }
  ...
  }, [integrationName, formValues.account_id, editData, config.properties?.account_id?.possible_values]);
  ...
  const handleAddWorkflowRule = () => {
    setWorkflowRules([...workflowRules, { match: [{ key: '', value: '' }], workflowId: '', accountId: '' }]);
  };

  const handleRemoveWorkflowRule = (ruleIdx) => {
    const next = workflowRules.filter((_, i) => i !== ruleIdx);
    setWorkflowRules(next.length > 0 ? next : [{ match: [{ key: '', value: '' }], workflowId: '', accountId: '' }]);
  };

  // Receive the full selected option so we can capture the workflow's
  // owning accountId alongside the workflowId. The mapping then carries
  // (workflowId, accountId) per rule so cross-account routing tells
  // generic_webhook which x-account-id header to forward.
  const handleWorkflowRuleChange = (ruleIdx, option) => {
    const workflowId = option?.value ?? option ?? '';
    const accountId = option?.accountId ?? '';
    setWorkflowRules(workflowRules.map((r, i) => (i === ruleIdx ? { ...r, workflowId, accountId } : r)));
  };

  const handleAddWorkflowCondition = (ruleIdx) => {
    setWorkflowRules(workflowRules.map((r, i) => (i === ruleIdx ? { ...r, match: [...r.match, { key: '', value: '' }] } : r)));
  };

  const handleRemoveWorkflowCondition = (ruleIdx, condIdx) => {
    setWorkflowRules(
  ...
  const handleWorkflowConditionChange = (ruleIdx, condIdx, field, value) => {
    setWorkflowRules(
      workflowRules.map((r, i) => {
        if (i !== ruleIdx) return r;
        const nextMatch = r.match.map((c, j) => (j === condIdx ? { ...c, [field]: value } : c));
  ...
  const handleCloseModal = (trigger) => {
    ...
    setWorkflowRules([{ match: [{ key: '', value: '' }], workflowId: '', accountId: '' }]);
    setWorkflowOptions([]);
    setAgentAccountProviders([]);
    setProviderFields([]);
  ...
  const submitForm = async () => {
    ...
    const { account_id, integration_config_name, account_mapping: _, workflow_mapping: _wm, ...restFormValues } = formValues;
    ...
    // Workflow mapping rules — emit { rules: [{ match: {k:v,...}, workflowId }] }.
    // Mirrors account_mapping shape but routes payloads to a specific workflow
    // instead of an account. Same kept-rule criteria and comma-as-OR semantics.
    if (integrationName === 'workflow_webhook') {
      const cleanedWorkflowRules = workflowRules
        .map((r) => {
          const match = {};
          (r.match || []).forEach((c) => {
            const k = (c.key || '').trim();
          ...
          return { match, workflowId: r.workflowId || '', accountId: r.accountId || '' };
        })
        .filter((r) => r.workflowId && Object.keys(r.match).length > 0);

      if (cleanedWorkflowRules.length > 0) {
        transformedValues.push({
          name: 'workflow_mapping',
          value: JSON.stringify({ rules: cleanedWorkflowRules }),
          is_encrypted: false,
        });
      }
    }
      ...
      >
        {isAgentSource ? (
          ...
          <>
            ...
            {integrationName === 'workflow_webhook' && editData?.name && (
              <>
                <Typography
                  variant='body2'
                  sx={{
                ...
                >
                  Define rules that route incoming webhooks to a specific automation based on payload values. Conditions inside a rule are combined
                  with AND. Rules are evaluated top-to-bottom; the first matching rule wins. Enter multiple values separated by commas to match any of
                  them (e.g. <em>deploy, scale</em>). When no rule matches, the webhook falls back to the workflow that bound this integration.
                </Typography>
                ...
                <Box sx={{ mt: 1 }}>
                  {workflowRules.map((rule, ruleIdx) => (
                    <Box
                      ...
                      data-testid={`workflow-rule-card-${ruleIdx}`}
                    >
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
                        <Typography sx={{ fontSize: '13px', fontWeight: 600, color: colors.text.primary }}>Rule {ruleIdx + 1}</Typography>
                        {workflowRules.length > 1 && (
                          <IconButton
                            onClick={() => handleRemoveWorkflowRule(ruleIdx)}
                            sx={{
                              backgroundColor: '#FEE2E2',
                              borderRadius: '6px',
                              padding: '6px',
                            ...
                            data-testid={`remove-workflow-rule-btn-${ruleIdx}`}
                            aria-label='Remove rule'
                          >
                            <SafeIcon
                              src={NewDelete}
                      ...
                      {rule.match.map((cond, condIdx) => (
                        <Box key={condIdx}>
                          ...
                          <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-end', mb: 1 }}>
                            <Box sx={{ flex: 1 }}>
                              <CustomTextField
                                label={condIdx === 0 ? 'Payload key' : ''}
                                placeholder='e.g. alert_type'
                                value={cond.key}
                                onChange={(e) => handleWorkflowConditionChange(ruleIdx, condIdx, 'key', e.target.value)}
                                fullWidth
                                size='small'
                              />
                            </Box>
                            ...
                            <Box sx={{ flex: 1 }}>
                              <CustomTextField
                                label={condIdx === 0 ? 'Value(s)' : ''}
                                placeholder='e.g. deploy or scale, restart'
                                value={cond.value}
                                onChange={(e) => handleWorkflowConditionChange(ruleIdx, condIdx, 'value', e.target.value)}
                                fullWidth
                                size='small'
                              />
                            </Box>
                            <IconButton
                              onClick={() => handleRemoveWorkflowCondition(ruleIdx, condIdx)}
                              disabled={rule.match.length === 1}
                              sx={{
                                backgroundColor: '#FEE2E2',
                                borderRadius: '6px',
                              ...
                              data-testid={`remove-workflow-condition-btn-${ruleIdx}-${condIdx}`}
                              aria-label='Remove condition'
                            >
                              <SafeIcon
                                src={NewDelete}
                      ...
                      <Box sx={{ mb: 2 }}>
                        <CustomButton
                          id={`add-workflow-condition-btn-${ruleIdx}`}
                          text='+ Add condition'
                          variant='secondary'
                          size='Small'
                          onClick={() => handleAddWorkflowCondition(ruleIdx)}
                        />
                      </Box>

                      <Typography sx={{ fontSize: '12px', fontWeight: 600, color: colors.text.secondary, mb: 1 }}>THEN run workflow</Typography>
                      <FilterDropdownButton
                        label='Workflow'
                        options={workflowOptions}
                        value={rule.workflowId}
                        onSelect={(_event, value) => handleWorkflowRuleChange(ruleIdx, value)}
                        isOptionsLoading={isLoadingWorkflows}
                        disabled={!workflowOptions.length}
                        sx={{ height: '45px' }}
                      />
                    </Box>
                  ))}
                  ...
                  <Box sx={{ mt: 1 }}>
                    <CustomButton id='add-workflow-rule-btn' text='+ Add rule' variant='secondary' size='Medium' onClick={handleAddWorkflowRule} />
                  </Box>
                  ...
                  >
                    If no rule matches, the webhook routes to the workflow that bound this integration via its webhook trigger.
                  </Typography>
                </Box>
              </>
            )}
