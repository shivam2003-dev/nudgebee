...
const JiraAccountModal = ({ openModal, handleClose, editConfig = null }) => {
  ...
  useEffect(() => {
    if (isEdit && editConfig) {
      ...
      setJiraToken(''); // never prefill token
  ...
  return (
    <Modal width='md' open={openModal} handleClose={handleClose} title={isEdit ? 'Edit Jira Account' : 'Add Jira Account'} loader={isSubmitting}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Box sx={{ mb: 0.5 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1, pl: 0.5 }}>
              ...
              >
                {isEdit ? 'API token for authentication. Leave empty to keep existing token.' : 'API token for authentication with Jira'}
              </Typography>
              {isEdit && (
      ...
      >
        <CustomButton id='cancel-create-account' text='Cancel' variant='secondary' size='Medium' onClick={handleClose} disabled={isSubmitting} />
        <CustomButton
          size='Medium'
