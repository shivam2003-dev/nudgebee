...
export default function Currency({
  ...
  if (value == null || value == undefined || isNaN(value) || value === '' || value === 0) {
    return withTooltip ? (
      <CustomTooltip title={defaultVal !== '-' ? defaultVal : ''} placement='bottom-start' tooltipStyle={{ margin: '0px' }}>
        <Typography sx={{ fontSize: '13px', color: colors.text.secondary, fontWeight: 400, lineHeight: 'auto', ...sx }} display='inline'>
          {defaultVal}
        </Typography>
      </CustomTooltip>
    ) : (
  ...
  if (isSavingPotential && negPotentialSaving > 0) {
    ...
    return (
      <CustomTooltip title={description} placement='top'>
        <Typography sx={{ fontSize: '13px', color: colors.text.secondary, fontWeight: 400, letterSpacing: '0.5px', ...sx }} display='inline'>
          {'~ '}
          {prefix}
          {'0'}
          {suffix && (
            <Typography component='span' sx={{ fontSize: '12px', color: colors.text.secondaryDark, fontWeight: 400, ...sxSuffix }}>
              {suffix}
            </Typography>
          )}
        </Typography>
  ...
  return withTooltip ? (
    ...
    >
      <Box>
        {parseFloat(actualValue) < 1 && precison === 0 && (
          <Typography sx={{ fontSize: '12px', color: colors.text.secondaryDark, fontWeight: 400, ...sxPrefix }} display='inline'>
            {'<'}
          </Typography>
        )}
        <Typography sx={{ fontSize: '12px', color: colors.text.secondaryDark, fontWeight: 400, ...sxPrefix }} display='inline'>
        ...
        {suffix && (
          <Typography sx={{ fontSize: '12px', color: colors.text.secondaryDark, fontWeight: 400, ...sxSuffix }} display='inline'>
            {suffix}
          </Typography>
        )}
      </Box>
    ...
    <Box>
      ...
      {percent && (
        <Typography
          sx={{ ...sx, marginLeft: '10px', color: parseFloat(actualValue) > 0 ? colors.text.lowest : colors.text.red, fontSize: '22px' }}
          display='inline'
        >
          ({percent})
        </Typography>
