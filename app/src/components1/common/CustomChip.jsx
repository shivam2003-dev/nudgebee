...
const CountLabel = ({ count, label, hasTonePalette }) => (
  <>
    <Box component='span' sx={{ fontWeight: 600, ...(hasTonePalette ? null : { color: colors.text.secondary }) }}>
      {count ?? 0}
    </Box>
    <Box component='span' sx={{ fontWeight: 400, ...(hasTonePalette ? null : { color: colors.text.secondaryDark }) }}>
      {label}
    </Box>
  </>
);
