  ...
  return (
    <>
      ...
      >
        ...
        {isExpandable ? (
          <TableCell data-export-enabled={false} sx={{ padding: 'var(--ds-space-2) var(--ds-space-2) var(--ds-space-2) 0px !important' }}>
...
const CustomTable = ({
  ...
  const renderEmptyState = () => {
    ...
    if (errorMessage) {
      ...
      content = (
        <Typography
          sx={{ color: colors.text.greyDark, fontSize: 'var(--ds-text-body-lg)', fontWeight: 'var(--ds-font-weight-regular)', fontFamily: 'Poppins' }}
        >
          {emptyStateText}
        </Typography>
      );
      ...
      return (
        <Box display='flex' justifyContent='flex-end' width='100%' mt={'0px'} mb={'10px'} pr={'50px'}>
          <CustomButton
            text='View all'
            variant='link'
            startIcon={<VisibilityOutlinedIcon sx={{ fontSize: 'var(--ds-text-small)', verticalAlign: 'middle' }} />}
            sx={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-space-1)' }}
    ...
    <>
                  ...
                  >
                    {(sort?.name && head.name == sort?.name && head?.sortEnabled) || head?.sortEnabled ? (
                      ...
                      >
                        ...
                        <span
                          style={{ color: colors.text.secondaryDark, fontSize: 'var(--ds-text-small)', fontWeight: 'var(--ds-font-weight-regular)' }}
                        >
                          {head.secondryText}
                        </span>
      ...
      {showColumnSelector && (
        <Popover
          ...
          slotProps={{ paper: { sx: { p: 'var(--ds-space-3)', borderRadius: 'var(--ds-radius-lg)', maxHeight: '320px', overflowY: 'auto' } } }}
        >
          <Typography
            sx={{
