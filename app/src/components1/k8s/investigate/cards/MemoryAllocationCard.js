...
class MemoryAllocationCard {
  ...
  ResolveMemoryComponent = (props) => {
    ...
    if (this.memoryAllocationItem.length > 0) {
      ...
      const memoryObject = this.podMemoryAllocationItem.find((g) => g.resource_type === 'memory');
      const cpuObject = this.podMemoryAllocationItem.find((g) => g.resource_type === 'cpu');
      if (memoryObject) {
        data['memory'] = {
          request: formatMemory(memoryObject.request * 1.1, 'bytes', 'mb')?.replace(',', ''),
          limit: formatMemory(memoryObject.limits * 1.1, 'bytes', 'mb')?.replace(',', ''),
          oldRequest: formatMemory(memoryObject.request, 'bytes', 'mb')?.replace(',', ''),
          oldLimit: formatMemory(memoryObject.limits, 'bytes', 'mb')?.replace(',', ''),
        };
      }
      if (cpuObject) {
        data['cpu'] = {
          request: cpuObject.cpu_request,
          limit: undefined,
          oldRequest: cpuObject.cpu_request,
          oldLimit: undefined,
        };
      }
    }
