...
const TriageRuleEventsTable = ({ query, onOpenTicketForm }) => {
  ...
  const tableData = events.map((event) => {
    return [
      ...
      {
        component: <Datetime baseDate={new Date()} value={event.classified_at} />,
      },
      {
  ...
  if (!events || events.length === 0) {
    return (
      <Box p={2}>
        <Typography color='textSecondary'>No events matched by this rule in the selected time range.</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ p: 1 }}>
      <Typography variant='body2' sx={{ mb: 1, color: colors.text.tertiary }}>
          ...
          <Grid container sx={chartContainerStyle}>
            <Grid item xs={12} md={6}>
              <LineChart
                ...
                colors={[colors.text.cpuUsage, colors.text.cpuRequested, colors.text.cpuLimit]}
                dataset={[
                  {
                    label: 'CPU Usage',
                    data: cpuData.data[0],
                ...
                dataset={[
                  {
                    label: 'Memory Usage',
                    data: memData.data[0],
                    borderColor: colors.text.memoryUsage,
                    backgroundColor: 'white',
                    pointRadius: 0,
                    borderWidth: 1,
                  },
                  {
                    ...
                    data: memData.data[1],
                    borderColor: colors.text.memoryRequested,
                    backgroundColor: 'white',
                    pointRadius: 0,
                    borderWidth: 1,
                  },
                  {
                    label: 'Memory Limit',
                    data: memData.data[2],
                    borderColor: colors.text.memoryLimit,
                    backgroundColor: 'white',
                    pointRadius: 0,
                    borderWidth: 1,
                  },
            ...
            >
              <Grid item xs={12}>
                <LineChart
                  ...
                  colors={[colors.text.cpuUsage, colors.text.cpuRequested]}
                  dataset={[
                    {
                      label: 'Disk Used',
      ...
      const convertedJson2 = convertedJson.rows.map((item) => {
        ...
        const components = Object.entries(item).map(([key, value]) => ({
          component: (
            <Text
              sx={{
                color: isRowRed ? 'red' : colors.text.secondary,
    ...
    <div>
      {labelEntries.length > 0 ? (
        <Grid container sx={{ backgroundColor: 'white' }}>
          ...
          <Grid item xs={12} sx={{ backgroundColor: 'white', paddingX: '14px', marginTop: '18px' }}>
            <Box sx={{ display: 'flex' }}>
              <Typography sx={{ fontWeight: 'bold', minWidth: '130px', marginTop: '14px' }}>Message</Typography>
      ...
      return (
        <Box
          sx={{
            ...
            backgroundColor: colors.background.recommendationChart,
            // padding: '10px 30px',
            marginTop: '4px',
            marginBottom: '10px',
          }}
        ...
        <>
          {availability && Object.keys(availability).length > 0 ? (
            <>
              ...
              <Typography sx={{ marginLeft: '27px', color: colors.text.availability }}>
          ...
          <Grid container sx={chartContainerStyle}>
            <Grid item xs={12}>
              <LineChart
                ...
                dataset={data}
                labels={labels}
                // Ensure chartLabel matches the dataset labels for the legend
  ...
  return (
    ...
    >
      <LineChart
        ...
        dataset={data}
        labels={labels}
        colors={[colors.text.cpuRequested, colors.text.cpuUsage]}
...
export const KubernetesRequestResponseTrend = ({ accountId, query = {}, selectedDateRange, additionalFilters = [], heading = '' }) => {
  ...
  return (
    <BoxLayout2
      id='box-request-response-charts'
      dateTimeRange={{
        enabled: true,
        onChange: handleDateRangeChange,
        passedSelectedDateTime: {
