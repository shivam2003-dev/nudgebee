...
const KubernetesServiceMap = ({ accountId, appName, namespaceName, dateRange, showSourceType = true, dataForServiceMap = [] }) => {
  ...
  // 1. Fetch Logic for OTel (RPC Action)
  const getServiceMapFromRpcAction = async () => {
    try {
      const reqBody = {
        accountId,
      ...
      const rawData = res?.data?.data?.traces_get_service_map?.data?.applications || [];
      const labels = res?.data?.data?.traces_get_service_map?.data?.labels || [];
  ...
  return (
    <div>
      ...
      <ListingLayout id='service-map-content'>
                ...
                <>
                  <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-space-1)' }}>
                      <AccessTimeOutlinedIcon sx={{ fontSize: 18, color: 'var(--ds-gray-500)' }} />
                      <Typography sx={{ fontSize: 'var(--ds-text-body-lg)', color: 'var(--ds-gray-700)', whiteSpace: 'nowrap' }}>Duration</Typography>
                    </Box>
                    <CustomizedSlider
                  ...
                  >
                    <VisibilityOutlinedIcon sx={{ fontSize: 18, color: 'var(--ds-gray-500)' }} />
                    <Typography sx={{ fontSize: 'var(--ds-text-body-lg)', color: 'var(--ds-gray-700)', whiteSpace: 'nowrap' }}>Red Edges</Typography>
                    <Switch
                      ...
                      sx={{
                        ml: 'var(--ds-space-1)',
                        '& .MuiSwitch-switchBase.Mui-checked': { color: 'var(--ds-background-100)' },
                  ...
                  <Tooltip
                    ...
                    placement='right'
                    tooltipStyle={{ maxWidth: '400px', padding: 'var(--ds-space-4)' }}
                  >
              ...
              {!appName && (
                <CustomDateTimeRangePicker
                  ...
                  onChange={(result) => {
                    ...
                    if (val) handleStartDateEndDate(val);
                  }}
        ...
        >
          ...
          {!appName && selectedSourceType !== 'otel' && (
            <FilterDropdown
              label='Namespaces'
              options={namespaces}
        ...
        <ListingLayout.Body>
          ...
          >
            {(isLoading || isLayoutLoading) && (
              <div
                style={{
                  ...
                  bottom: 0,
                  zIndex: 10,
                  background: 'color-mix(in srgb, var(--ds-background-100) 70%, transparent)',
                  display: 'flex',
                  alignItems: 'center',
            ...
            <ReactFlow
              ...
              style={{ background: 'var(--ds-background-100)', width: '100%', borderRadius: 'var(--ds-radius-sm)' }}
            >
              <Background color='var(--ds-gray-200)' variant={BackgroundVariant.Lines} gap={24} />
              <Controls position={'top-left'} fitViewOptions={{ minZoom: 0 }} />
              <MiniMap />
