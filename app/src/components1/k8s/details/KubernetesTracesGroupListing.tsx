...
const KubernetesTracesGroupListing: React.FC<KubernetesTracesGroupListingProps> = ({
  ...
  const onResourceFilterChange = (value: string) => {
    if (resource && value.trim() == '') {
      setResource('');
      setCurrentPage(0);
    }
    setInputResource(value);
  };
  ...
  return (
    <ListingLayout id='k8s-traces-group-box'>
      <ListingLayout.Toolbar
        actions={
          <>
            ...
            <DownloadButton onClick={() => ({ tableId: 'k8s-trace-group-listing' })} />
          </>
        }
      >
        {showNamespaceFilter && (
          <FilterDropdown
            ...
            onSelect={(e: React.ChangeEvent<HTMLInputElement>) => {
              setSelectedDestNamespace(e.target.value);
              setCurrentPage(0);
            }}
          />
        )}
        {showWorkloadFilter && (
          <FilterDropdown
            ...
            options={workloads.map((o) => ({ value: o, label: o }))}
        ...
        <FilterDropdown
          label='Span Type'
          options={['http', 'query'].map((o) => ({ value: o, label: o }))}
          value={spanType}
          onSelect={(e: React.ChangeEvent<HTMLInputElement>) => {
