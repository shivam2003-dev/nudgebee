    ...
    <>
      ...
      <Modal
        ...
        actionButtons={
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', p: '12px 24px' }}>
            <DsButton id='cancel-delete-pod-btn' tone='secondary' size='sm' onClick={handleCloseDeletePopUp}>
              Cancel
            </DsButton>
            <DsButton id='confirm-delete-pod-btn' tone='danger' size='sm' onClick={handleSubmit}>
              Delete
            </DsButton>
          </Box>
        }
      >
        <Box sx={{ padding: '24px' }}>
          ...
          >
            Are you sure you want to delete the pod <strong>{selectedPodName.name}</strong>?
          </Typography>
        </Box>
      </Modal>
      {debugPodOpen ? (
