            ...
            tabs: [
              {
                ...
                componentFn: (opt: any, drilldownQuery: any) => {
                  ...
                  return (
                    <ListingLayout id='box-trend-charts'>
                      <ListingLayout.Toolbar actions={<DownloadButton onClick={() => ({ canvasId, tableId: '' })} />} />
                      <ListingLayout.Body>
...
const KubernetesAnomaly = ({ accountId }: { accountId: string }) => {
  ...
  const listAnomalies = () => {
    ...
    apiKubernetes1
      ...
      .then((res) => {
        const anomaliesData = res?.data?.data?.anomalies_list?.rows || [];
        const tableData = anomaliesData.map((item: any) => {
          return [
            { text: item.namespace, drilldownQuery: item },
            { text: item.name },
  ...
  return (
    <ListingLayout id={'anomaly-table-container'}>
      <ListingLayout.Toolbar
        actions={
          <>
            <DownloadButton onClick={() => ({ tableId: 'anomaly-table' })} />
            <CustomButton
      ...
      >
        ...
        <FilterDropdown
          label='Anomaly Type'
          options={anomalyTypes.map((o) => ({ value: o, label: o }))}
