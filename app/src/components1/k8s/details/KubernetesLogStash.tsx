...
const KubernetesLogStash: React.FC<KubernetesLogStashProps> = ({ accountId, showQueryTextBox = true, timeRange, query, showPolling = true }) => {
  ...
  return (
    <div>
      ...
      />
      <ListingLayout id='k8s-logs-stash'>
        <ListingLayout.Toolbar
          actions={
            <>
              <DownloadButton onClick={() => ({ tableId: k8sLogs })} id='k8s-logs-stash' />
              <CustomDateTimeRangePicker onChange={(ranges: any) => onDateTimeRangeChange(ranges.selection)} passedSelectedDateTime={time} />
              {showPolling && (
                <RefreshSubmitButton
              ...
              <UserHistoryButton accountId={accountId} module='log_query_es' />
            </>
          }
        />
        <ListingLayout.Body>
