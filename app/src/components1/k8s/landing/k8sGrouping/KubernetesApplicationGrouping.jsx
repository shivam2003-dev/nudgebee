...
const KubernetesApplicationGrouping = () => {
  ...
  const [inputName, setInuptName] = useState('');
  ...
  return (
    <div>
      ...
      <ListingLayout id='k8s-grouping'>
        <ListingLayout.Toolbar
          actions={
            <>
              <DownloadButton id='k8s-grouping-download' onClick={async () => ({ tableId: k8sGroupigId })} />
              {hasWriteAccess() ? (
                <Button
                  tone='primary'
                  onClick={() => {
