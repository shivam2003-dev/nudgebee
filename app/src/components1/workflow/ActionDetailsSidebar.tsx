...
const ActionDetailsSidebar: React.FC<ActionDetailsSidebarProps> = ({
  ...
  useEffect(() => {
    ...
    // Always track the latest parent state as the committed baseline so dirty
    // detection stays accurate even when external updates arrive mid-edit.
    setCommittedSnapshot(nextStr);
    // Only overwrite local edits when the user has no pending changes.
    // If they are dirty, preserve their in-progress work.
  ...
  const applyAutoDefault = useCallback(
    (updated: any) => {
      ...
      setCommittedSnapshot(JSON.stringify(updated));
  ...
  // the provider, so we delegate to the existing get_default_provider RPC
  // action.
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          {!viewOnlyMode && isDirty && (
            <Button
              id='action-sidebar-save-btn'
              data-testid='action-sidebar-save-btn'
              variant='contained'
              size='small'
          ...
          <IconButton id='action-sidebar-close-btn' onClick={requestClose} sx={{ p: 0.5, '&:hover': { bgcolor: '#f3f4f6' } }}>
