...
import { PlayArrow, Timer, Storage, GridView, ErrorOutline, Close, AltRoute } from '@mui/icons-material';
...
import {
  ...
  KeyValueField,
  MatrixField,
} from './components/advanced-config';
import {
  TimestampPicker,
  JsonEditor,
  ArrayEditor,
...
const renderDescriptiveOption = (
  ...
  return (
    <li {...props} key={option}>
      <Box sx={{ py: 0.25 }}>
        <Typography component='span' sx={{ fontWeight: 600, fontSize: '13px', color: colors.text.primary, fontFamily: 'monospace' }}>
          {option}
        </Typography>
        {found?.description && (
          <Typography component='p' sx={{ fontSize: '12px', color: colors.text.secondary, mt: 0.25, lineHeight: 1.3 }}>
            {found.description}
          </Typography>
        )}
      </Box>
...
const ActionDetailsSidebar: React.FC<ActionDetailsSidebarProps> = ({
  ...
  // Track if we're currently making internal changes to avoid sync loops
  ...
  useEffect(() => {
    // Only sync from parent if it's NOT an internal change (to avoid overwriting during typing)
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedActionType, taskDefinitions]);
  ...
  useEffect(() => {
    ...
    onTaskDataChange(updated);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedActionType, timeMode, taskDefinitions]);
  ...
  useEffect(() => {
    ...
    if (needsNormalize) {
      ...
      onTaskDataChange(normalized);
    }
  }, [selectedActionType, taskDefinitions, onTaskDataChange]);
  ...
  useEffect(() => {
    ...
    setLocalData(updated);
    onTaskDataChange(updated);
  }, [open, accountId, selectedActionType, taskDefinitions, onTaskDataChange]);
  ...
  // the provider, so we delegate to the existing get_default_provider Hasura
  // action.
  ...
  // get_default_provider call and the chip label so the Traces action shows
  ...
  useEffect(() => {
    ...
    setLocalData(updated);
    onTaskDataChange(updated);
  }, [cloudAccounts, selectedActionType, resolveAccountProviderType, onTaskDataChange, defaultProvider, defaultProviderKind]);
  ...
  useEffect(() => {
    ...
    setLocalData(updated);
    onTaskDataChange(updated);
  }, [ticketTool, selectedActionType, onTaskDataChange]);
  ...
  const handleDataChange = useCallback(
    (field: string, value: any) => {
      ...
      onTaskDataChange(updatedData);
    },
    [onTaskDataChange, cleanupDependentFields, applyAccountFieldChange]
  );
  ...
  const setFieldValueNoCascade = useCallback(
    (field: string, value: any) => {
      isInternalChangeRef.current = true;
      const updatedData = { ...localDataRef.current };
  ...
  const renderWorkflowInputFields = () => {
    if (!workflowInputs || workflowInputs.length === 0) {
      return <Typography sx={{ fontSize: '12px', color: colors.text.secondaryDark, fontStyle: 'italic' }}>No automation inputs defined</Typography>;
    }
  ...
  const renderWorkflowConfigFields = () => {
    if (!workflowConfigs || workflowConfigs.length === 0) {
      return <Typography sx={{ fontSize: '12px', color: colors.text.secondaryDark, fontStyle: 'italic' }}>No automation configs defined</Typography>;
    }
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
        ...
        {hasMoreConfigs && (
          <Button
            id='action-sidebar-toggle-configs-btn'
            size='small'
  ...
  const renderPreviousActionsColumn = () => {
    ...
    return (
      ...
      >
        ...
        >
          <Typography sx={{ fontSize: '13px', fontWeight: 600, color: colors.text.secondary, mb: 0.5 }}>Available Data</Typography>
          <Typography sx={{ fontSize: '11px', color: colors.text.secondaryDark }}>Drag fields to use in configuration</Typography>
        </Box>

        {/* Dry Run Button */}
        <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid #e5e7eb', bgcolor: '#fff' }}>
          <Button
            ...
            startIcon={previousDryRunLoading ? <CircularProgress size={14} /> : <PlayArrow sx={{ fontSize: 16 }} />}
            sx={{
              fontSize: '12px',
              textTransform: 'none',
              py: 0.75,
          ...
          {previousDryRunError && <Typography sx={{ fontSize: '10px', color: colors.errorText, mt: 0.5 }}>{previousDryRunError}</Typography>}
        </Box>
        ...
        >
          ...
          {workflowInputs && workflowInputs.length > 0 && (
            ...
            >
              <Typography sx={{ fontSize: '12px', fontWeight: 600, color: colors.text.secondary, mb: 1 }}>Automation Inputs</Typography>
          ...
          {workflowConfigs && workflowConfigs.length > 0 && (
            ...
            >
              <Typography sx={{ fontSize: '12px', fontWeight: 600, color: colors.text.secondary, mb: 1 }}>Automation Configs</Typography>
              ...
              {previousTasks.map((task) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <Typography sx={{ fontSize: '12px', fontWeight: 600, color: colors.text.secondary }}>{task.name || task.id}</Typography>
                      {statusColors && (
                        <Chip
                    ...
                    <Typography sx={{ fontSize: '10px', color: colors.text.secondaryDark, mb: 1 }}>{task.type}</Typography>
                    ...
                    {taskOutput?.output && (
                      ...
                      >
                        <Typography sx={{ fontSize: '10px', color: '#166534', mb: 0.5, fontWeight: 600 }}>Actual Output:</Typography>
                        <JsonTreeView
                    ...
                    {taskOutput?.error && (
                      ...
                      >
                        <Typography sx={{ fontSize: '10px', color: '#991b1b' }}>{taskOutput.error}</Typography>
                      </Box>
                    )}
                  </Box>
                );
  ...
  const renderTestCurrentActionColumn = () => {
      ...
      return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography sx={{ fontSize: '11px', fontWeight: 600, color: colors.text.secondary }}>{title}:</Typography>
            {statusColors && (
              <Chip
                label={result.status}
                size='small'
          ...
          {result.output && (
            ...
            >
              <Typography sx={{ fontSize: '10px', color: '#166534', mb: 0.5, fontWeight: 600 }}>Output:</Typography>
              <JsonTreeView
                data={result.output}
                defaultExpanded={2}
                maxHeight='none'
          ...
          {result.error && (
            ...
            >
              <Typography sx={{ fontSize: '10px', color: '#991b1b', fontWeight: 600, mb: 0.25 }}>Error:</Typography>
              <Typography
                component='pre'
                sx={{
                  fontSize: '10px',
    ...
    return (
      ...
      >
        ...
        >
          <Typography sx={{ fontSize: '13px', fontWeight: 600, color: colors.text.secondary, mb: 0.5 }}>Test Action</Typography>
          <Typography sx={{ fontSize: '11px', color: colors.text.secondaryDark }}>Test this action with dry run or live execution</Typography>
        </Box>

        {/* Buttons Section */}
        <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid #e5e7eb', bgcolor: '#fff', display: 'flex', flexDirection: 'column', gap: 1 }}>
          {/* Dry Run Button */}
          <Box>
            <CustomButton
              size={'Medium'}
              sx={{ width: '100%' }}
              text={currentDryRunLoading ? 'Running...' : 'Dry Run'}
              variant='secondary'
              onClick={handleDryRunCurrentTask}
              disabled={currentDryRunLoading || runTaskLoading || viewOnlyMode || !onDryRunToTask || !supportsDryRun}
              startIcon={currentDryRunLoading ? <CircularProgress size={14} /> : <PlayArrow sx={{ fontSize: 16 }} />}
            />
            <Typography
              sx={{ fontSize: '10px', color: !supportsDryRun ? colors.errorText : colors.text.secondaryDark, mt: 0.5, textAlign: 'center' }}
            >
              {dryRunHelperText}
            </Typography>
          </Box>
          ...
          <Box>
            <CustomButton
              ...
              startIcon={runTaskLoading ? <CircularProgress size={14} color='inherit' /> : <PlayArrow sx={{ fontSize: 16 }} />}
              text={runTaskLoading ? 'Executing...' : 'Run Task'}
            />
            <Typography
              sx={{ fontSize: '10px', color: !supportsIndividualRun ? colors.errorText : colors.text.secondaryDark, mt: 0.5, textAlign: 'center' }}
            >
        ...
        >
          ...
          {!currentTaskResult && !runTaskResult && switchChildrenResults.length === 0 && (
            ...
            >
              <Typography sx={{ fontSize: '12px', fontStyle: 'italic', textAlign: 'center' }}>
  ...
  const renderDynamicForm = (schema: any, _title: string, description: string) => {
            ...
            <Box key={fieldName} sx={{ mb: 2, display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: 'wrap' }}>
              ...
              <Box sx={{ flex: '1 1 300px', minWidth: '200px' }}>
                <HybridField
                  ...
                  value={fieldValue}
                  onChange={(newValue: string) => handleDataChange(fieldName, newValue)}
                  placeholder={!dependenciesMet ? 'Select issue type first' : 'Select severity'}
                  disabled={isReadOnly || viewOnlyMode || !dependenciesMet}
                  error={validationErrors[fieldName] || ''}
                  required={isRequired}
                ...
                <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
                  ...
                  <Alert
                    ...
                    sx={{
                      ...
                      '& .MuiAlert-message': { fontSize: '11px', py: 0.25 },
                    }}
                  ...
                  >
                    ...
                    <Button
                      ...
                      sx={{ fontSize: '11px', textTransform: 'none', p: 0, mt: 0.5, minWidth: 'auto', lineHeight: 1.2 }}
                      data-testid='mcp-manage-integrations-btn'
                    >
            ...
            <Box sx={{ mb: 2, display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: 'wrap' }}>
              ...
              <Box sx={{ flex: '1 1 300px', minWidth: '200px' }}>
                ...
                {!isAccountField && (options as any[]).some((o) => o?.icon) && validationErrors[fieldName] && (
                  <Typography sx={{ mt: 0.5, fontSize: '12px', color: colors.border?.error || '#d32f2f' }}>{validationErrors[fieldName]}</Typography>
                )}
                {showDefaultProviderChip && (
                  <Box sx={{ mt: 0.75 }}>
                    <Chip
                      ...
                      sx={{ fontSize: '11px', height: '20px', bgcolor: colors.lowestLight, color: colors.text.secondary }}
                    />
                  </Box>
                )}
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
                ...
                >
                  ...
                  <Button
                    ...
                    sx={{ fontSize: '11px', textTransform: 'none', p: 0, mt: 0.5, minWidth: 'auto', lineHeight: 1.2 }}
                    data-testid='ssm-manage-permissions-btn'
                  >
    ...
    const renderParametersTab = () => {
      ...
      return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {/* Form Description */}
          <Typography sx={{ fontSize: '12px', color: colors.text.secondaryDark, mb: 1 }}>{description}</Typography>
          ...
          {isTicketCreateTask && Object.keys(ticketDynamicFields).length > 0 && (
            <Box sx={{ mt: 1 }}>
              <Typography sx={{ fontSize: '13px', fontWeight: 600, color: colors.text.secondary, mb: 1 }}>Platform Fields</Typography>
              {Object.entries(ticketDynamicFields)
                // summary/description map to the static title/description fields;
                // priority is already surfaced by the static "Severity" field
                // (which reuses ticketFieldOptions['priority'] for its options).
                // urgency is the PagerDuty/ZenDuty equivalent of priority and gets
                // aliased into priority by useTicketDynamicFields, so dropping it
                // here keeps Severity from rendering twice for those tools.
                .filter(([key]) => key !== 'summary' && key !== 'description' && key !== 'priority' && key !== 'urgency')
      ...
      return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
            ...
            <Typography sx={{ fontSize: '13px', fontWeight: 600, color: colors.text.secondary }}>{'Conditional Execution'}</Typography>
            {hasCondition && (
              <Box
                sx={{
                  width: 6,
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          ...
          >
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography sx={{ fontSize: '13px', fontWeight: 600, color: colors.text.primary }}>Disable task</Typography>
              <Typography sx={{ fontSize: '11px', color: colors.text.secondary, mt: 0.25 }}>
                {taskConfig.disabled
          ...
          <ConfigGroup
            ...
            icon={<Timer sx={{ fontSize: 16, color: colors.text.secondary }} />}
            hasConfigured={hasExecutionControl}
            defaultExpanded={hasExecutionControl}
          >
            <DurationField
              label='Task Timeout'
          ...
          <ConfigGroup
            ...
            hasConfigured={hasDataManagement}
            defaultExpanded={hasDataManagement}
          >
            <KeyValueField
          ...
          <ConfigGroup
            ...
            icon={<GridView sx={{ fontSize: 16, color: colors.text.secondary }} />}
            hasConfigured={hasParallelExecution}
            defaultExpanded={hasParallelExecution}
          >
          ...
          <ConfigGroup
            ...
            icon={<ErrorOutline sx={{ fontSize: 16, color: colors.text.secondary }} />}
            hasConfigured={hasErrorHandling}
            defaultExpanded={hasErrorHandling}
          >
            <FailurePolicyField
          ...
          <Box sx={{ mt: 2, pt: 2, borderTop: `1px solid ${colors.lowestLight}` }}>
            <Typography variant='subtitle2' sx={{ mb: 1, fontSize: '14px', fontWeight: 600, color: colors.text.secondary }}>
              Schema Information
            </Typography>
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        {/* Tabs Header */}
        <Box sx={{ borderBottom: '1px solid #e5e7eb', px: 1, bgcolor: '#fafafa' }}>
          <Tabs
    ...
    >
      ...
      >
        <Box>
          ...
          <Typography sx={{ fontSize: '12px', color: '#6B7280', mt: 0.25 }}>{'Configure and test this automation action'}</Typography>
        </Box>
        <IconButton id='action-sidebar-close-btn' onClick={onClose} sx={{ p: 0.5, '&:hover': { bgcolor: '#f3f4f6' } }}>
      ...
      <DialogContent sx={{ p: 0, display: 'flex', height: 'calc(100% - 72px)' }}>
        ...
        <Box sx={{ width: '50%', overflow: 'hidden', bgcolor: '#fff', display: 'flex', flexDirection: 'column' }}>
      ...
      <Dialog open={disableConfirmOpen} onClose={() => setDisableConfirmOpen(false)} maxWidth='xs' fullWidth>
        <Box sx={{ p: 3 }}>
          <Typography sx={{ fontSize: '15px', fontWeight: 600, mb: 1 }}>Disable this task?</Typography>
          <Typography sx={{ fontSize: '13px', color: colors.text.secondary, mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
            <Button id='action-sidebar-disable-cancel-btn' onClick={() => setDisableConfirmOpen(false)} size='small' sx={{ textTransform: 'none' }}>
              Cancel
            </Button>
            <Button
              id='action-sidebar-disable-confirm-btn'
              data-testid='task-disable-confirm-btn'
              onClick={() => {
