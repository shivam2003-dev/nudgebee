...
const CallWorkflowFields: React.FC<CallWorkflowFieldsProps> = ({
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Typography sx={{ fontSize: '12px', color: colors.text.secondaryDark, mb: 1 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: 'wrap' }}>
        ...
        <Box sx={FIELD_COL_SX}>
          ...
          {(() => {
            if (nameError) {
              return <Typography sx={{ fontSize: '11px', color: colors.border.error, mt: 0.5 }}>{nameError}</Typography>;
            }
            ...
            return <Typography sx={{ fontSize: '11px', color: colors.text.secondary, mt: 0.5 }}>{helper}</Typography>;
          })()}
          {loadError ? (
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Typography sx={{ fontSize: '13px', fontWeight: 600, color: colors.text.secondary }}>Inputs</Typography>
          <FormControlLabel
            control={
              <Switch
                size='small'
            ...
            label={<Typography sx={{ fontSize: '12px' }}>Raw JSON</Typography>}
            sx={{ m: 0 }}
          />
        </Box>
        ...
        {advancedInputs || isTemplated || !selectedWorkflow ? (
          ...
          <Typography sx={{ fontSize: '12px', color: colors.text.secondaryDark, fontStyle: 'italic' }}>This workflow takes no inputs.</Typography>
        ) : (
              ...
              return (
                <Box key={input.id} sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: 'wrap' }}>
                  <Typography sx={LABEL_COL_SX}>
                    ...
                    <Typography component='span' sx={{ fontSize: '10px', color: colors.text.secondaryDark, ml: 0.5 }}>
                      ({input.type || 'string'})
                    </Typography>
                  </Typography>
                  <Box sx={FIELD_COL_SX}>
