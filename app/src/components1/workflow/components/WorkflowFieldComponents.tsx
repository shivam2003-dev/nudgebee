      ...
      >
        ...
        >
          <Box {...dragHandleProps} aria-label='Drag to reorder' sx={{ display: 'flex', alignItems: 'center', mr: 0.5 }}>
            <DragIndicator sx={{ color: '#9ca3af', fontSize: 18 }} />
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 1, cursor: 'pointer' }} onClick={() => toggleExpand(index)}>
            <Typography sx={{ fontSize: '13px', fontWeight: 500, color: '#374151' }}>Item {index + 1}</Typography>
            {configuredCount > 0 && (
              <Chip label={`${configuredCount} fields`} size='small' variant='outlined' sx={{ height: '18px', fontSize: '10px' }} />
            )}
            {isExpanded ? (
              <ExpandLess sx={{ fontSize: 18, color: '#6b7280', ml: 'auto' }} />
            ) : (
              <ExpandMore sx={{ fontSize: 18, color: '#6b7280', ml: 'auto' }} />
            )}
          </Box>
          <IconButton size='small' onClick={() => removeItem(index)} sx={{ color: '#ef4444', ml: 1 }}>
          ...
          <Box sx={{ p: 1.5 }}>
            {Object.entries(itemSchema).map(([fieldName, fieldSchema]) => (
              <Box key={fieldName} sx={{ mb: 1.5 }}>
                ...
                {fieldSchema.description && <Typography sx={{ fontSize: '11px', color: '#6b7280', mb: 0.5 }}>{fieldSchema.description}</Typography>}
  ...
  return (
    <Box>
      ...
      >
        ...
        <CustomButton onClick={addItem} text='Add Item' variant='secondary' size='Small' startIcon={<Add />} sx={{ mt: 1 }} />
      </Box>
      {error && (
        <Typography variant='body2' sx={{ color: '#ef4444', fontSize: '12px', mt: 0.5 }}>
          {error}
...
export const KeyValueEditor: React.FC<KeyValueEditorProps> = ({
  ...
  return (
    <Box>
      ...
      >
        {entries.length === 0 && (
          <Typography variant='body2' sx={{ color: '#9ca3af', fontSize: '13px', mb: 1, textAlign: 'center' }}>
          ...
          return (
            <Box key={entry.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
              <TextField
                size='small'
                value={entry.key}
                onChange={(e) => updateEntry(entry.id, e.target.value, entry.value)}
              ...
              <TextField
                ...
                sx={{ flex: 2 }}
              />
              <IconButton size='small' onClick={() => removeEntry(entry.id)} disabled={disabled} sx={{ color: '#ef4444' }}>
                <Delete fontSize='small' />
              </IconButton>
            </Box>
          );
        ...
        {duplicateKeys.size > 0 && (
          <Typography variant='body2' sx={{ color: '#f59e0b', fontSize: '11px', mb: 1 }}>
            Warning: Duplicate keys detected. Only the last value for each key will be saved.
          </Typography>
        )}
        <CustomButton onClick={addEntry} text='Add Entry' variant='secondary' size='Small' startIcon={<Add />} disabled={disabled} sx={{ mt: 1 }} />
      </Box>
      {error && (
        <Typography variant='body2' sx={{ color: '#ef4444', fontSize: '12px', mt: 0.5 }}>
          {error}
        ...
        {options.map((option) => (
          <Chip
            ...
            sx={{
              ...
              '&:hover': {
                backgroundColor: value.includes(option) ? undefined : '#e5e7eb',
              },
            }}
          />
        ))}
    ...
    <Box>
      ...
      >
        ...
        >
          ...
          <Typography sx={{ fontSize: '12px', color: '#6b7280' }}>{expanded ? '▼' : '▶'}</Typography>
        </Box>

        {/* Content */}
        {expanded && (
          <Box sx={{ p: 1.5, pt: 0, borderTop: '1px solid #e5e7eb' }}>
