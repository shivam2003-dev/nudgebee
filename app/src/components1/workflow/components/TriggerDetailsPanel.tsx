...
const DetailRow = ({ label, value, mono }: { label: string; value: React.ReactNode; mono?: boolean }) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      <Typography sx={{ fontSize: '11px', fontWeight: 600, color: colors.text.tertiary, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
        {label}
      </Typography>
      {typeof value === 'string' ? (
        <Typography
      ...
      {hasStructured ? (
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px' }}>
          {EVENT_STRUCTURED_FIELDS.map(
            (f) =>
              parsed[f.key] && (
                <Box key={f.key} sx={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <Typography
                    sx={{
                      fontSize: '11px',
                      fontWeight: 600,
...
const TriggerDetailsPanel: React.FC<TriggerDetailsPanelProps> = ({ triggerNode, executionData, selectedExecution, getDuration, copyToClipboard }) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Header */}
      <Box sx={{ padding: '16px 20px', borderBottom: '1px solid #e5e7eb', flexShrink: 0 }}>
      ...
      {/* Scrollable content */}
      <Box className='custom-scrollbar' sx={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
          ...
          >
            {triggeredBy && (
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                ...
                <Typography sx={{ fontSize: '13px', fontWeight: 500, color: colors.text.secondary }}>{triggeredBy}</Typography>
              </Box>
            )}
        ...
        {hasConfig && (
          <Box sx={{ mb: '20px' }} data-testid='trigger-config-section'>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: '6px', mb: '14px' }}>
              <Schedule sx={{ fontSize: '15px', color: colors.text.secondaryDark }} />
          ...
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: '14px' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <InputIcon sx={{ fontSize: '15px', color: colors.text.secondaryDark }} />
                <Typography sx={{ fontSize: '12px', fontWeight: 600, color: colors.text.secondary, fontFamily: 'Poppins, sans-serif' }}>
              ...
              <IconButton
                size='small'
                data-testid='trigger-inputs-copy-btn'
        ...
        {!hasConfig && !hasInputs && (
          <Box
            ...
            sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', py: 6, gap: '8px' }}
          >
            <Typography sx={{ fontSize: '13px', color: colors.text.secondaryDark }}>
