...
const TriggerConfigSidebar: React.FC<TriggerConfigSidebarProps> = ({ open, selectedNode, onClose, onSave, accountId, workflowData }) => {
  ...
  const fetchWebhookIntegrationOptions = async () => {
    ...
    try {
      ...
      // Show only webhooks that aren't bound to any workflow. Also restrict to
      // the current account when available — workflow_webhook is single-account.
      // The currently-bound webhook (if any) is already shown in the input value
      // even when missing from the options list.
      const available = rows.filter((row: any) => {
        const rawConfig = row?.integration_config_values;
        const configValues = Array.isArray(rawConfig) ? rawConfig : typeof rawConfig === 'string' ? JSON.parse(rawConfig || '[]') : [];
        const boundWorkflowId = configValues.find((c: any) => c?.name === 'workflow_id')?.value || '';
        if (boundWorkflowId) return false;
        if (currentAccountId && currentAccountId !== 'demo') {
