...
const TriggerConfigSidebar: React.FC<TriggerConfigSidebarProps> = ({ open, selectedNode, onClose, onSave, accountId, workflowData }) => {
  ...
  const fetchWebhookIntegrationOptions = async () => {
    ...
    try {
      ...
      // Cache token-by-name for *all* enabled rows (not just unbound ones) so we
      // can render the webhook URL for the currently selected integration even
      // when it's filtered out of the dropdown.
      ...
      // List every webhook for the current account. workflow_webhook is no
      // longer 1:1 — the same integration can fan out to multiple workflows
      // either via workflow_mapping rules on the integration, or by N
      // workflows each registering a webhook trigger pointing at the same
      // integration. The integration's workflow_id config is treated as the
      // "most recently bound" fallback, not an exclusivity lock.
      const available = rows.filter((row: any) => {
        if (currentAccountId && currentAccountId !== 'demo') {
          const rawAccounts = row?.integrations_cloud_accounts;
          const cloudAccounts = Array.isArray(rawAccounts) ? rawAccounts : typeof rawAccounts === 'string' ? JSON.parse(rawAccounts || '[]') : [];
          const matches = cloudAccounts.some((ca: any) => String(ca?.cloud_account_id) === String(currentAccountId));
          if (!matches) return false;
        }
        return true;
      });
      const options = available.map((row: any) => ({ label: row.name, value: row.name }));
