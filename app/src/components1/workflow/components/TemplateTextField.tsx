...
import { Box, TextField, Popper, Paper, List, ListItem, ListItemText, Typography, IconButton, Tooltip, ClickAwayListener } from '@mui/material';
    ...
    <Box sx={{ ...fieldContainer, position: 'relative', width: fullWidth ? '100%' : 'auto' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: multiline ? 'flex-start' : 'center', gap: 0.5, width: '100%' }}>
        ...
        {allSuggestions.length > 0 && !disabled && (
          <Tooltip title='Insert variable' arrow>
            <IconButton
              ref={pickerButtonRef}
              size='small'
              onClick={handlePickerOpen}
              data-testid='template-variable-picker-button'
              sx={{
                mt: multiline ? '4px' : 0,
                color: colors.text?.secondary || '#616161',
                border: `1px solid ${colors.border?.vertical || '#e0e0e0'}`,
                borderRadius: '6px',
                padding: '6px',
                '&:hover': {
                ...
                <React.Fragment key={group.category}>
                  ...
                  {group.items.map((suggestion, index) => (
                    ...
                    >
                      <ListItemText
                        primary={
                          <Typography sx={{ fontFamily: 'monospace', fontSize: '12px', color: '#1976d2' }}>{suggestion.insertText}</Typography>
                        }
                        secondary={<Typography sx={{ fontSize: '11px', color: colors.text.secondaryDark }}>{suggestion.description}</Typography>}
                      />
                    </ListItem>
                  ))}
                </React.Fragment>
