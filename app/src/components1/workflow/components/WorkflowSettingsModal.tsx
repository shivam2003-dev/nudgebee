      ...
      >
        {items.map((item, index) => (
          <Box key={index} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
            <DragIndicator sx={{ color: 'var(--ds-gray-400)', cursor: 'grab' }} />
            <Box sx={{ flex: 1 }}>
              <Input size='sm' value={item} onChange={(next) => updateItem(index, next)} placeholder={`Item ${index + 1}`} />
            </Box>
            <Button
              id={`wf-settings-array-remove-${index}-btn`}
        ...
        <Box sx={{ mt: 1 }}>
          <Button id='wf-settings-array-add-btn' tone='secondary' size='sm' icon={<Add />} onClick={addItem}>
            Add Item
          </Button>
        </Box>
      </Box>
...
const WorkflowSettingsModal: React.FC<WorkflowSettingsModalProps> = ({ open, onClose, onSave, initialSettings = defaultSettings, taskTimeouts }) => {
  ...
  const actionButtons = (
    <Box sx={{ display: 'flex', gap: 2, p: 2 }}>
      <Button id='wf-settings-cancel-btn' tone='secondary' size='md' onClick={handleCancel}>
        Cancel
      </Button>
      <Button id='wf-settings-apply-btn' tone='primary' size='md' onClick={handleSave} disabled={taskTimeoutSumExceedsWorkflow}>
        Apply Settings
      </Button>
    </Box>
  );
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 2 }}>
                  {settings.inputs.length === 0 && !showAddForm && (
                    <Typography variant='body2' sx={{ color: 'var(--ds-gray-400)', fontStyle: 'italic', textAlign: 'center', py: 2 }}>
                      ...
                      {editingInputId !== input.id && (
                        ...
                        >
                          <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                            ...
                            >
                              <Typography component='span' sx={{ color: 'var(--ds-gray-400)', fontSize: 'var(--ds-text-small)' }}>
                                value:{' '}
                              </Typography>
                            ...
                            {input.description && !input.description.startsWith('Input parameter:') && (
                              ...
                              >
                                <Typography component='span' sx={{ color: 'var(--ds-brand-300)', fontSize: 'var(--ds-text-small)' }}>
                                  description:{' '}
                                </Typography>
                                {input.description}
                              </Typography>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
                              <Button
                                ...
                                icon={<EditIcon sx={{ fontSize: 'var(--ds-text-title)' }} />}
                                onClick={(e) => {
                                  e.stopPropagation();
                              ...
                              <Button
                                ...
                                icon={<Close sx={{ fontSize: 'var(--ds-text-title)' }} />}
                                onClick={(e) => {
                                  e.stopPropagation();
                      ...
                      {editingInputId === input.id && editingInput && (
                        ...
                        >
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                            <Typography
                              variant='body2'
                              sx={{ color: 'var(--ds-brand-500)', fontWeight: 'var(--ds-font-weight-semibold)', fontSize: 'var(--ds-text-body)' }}
                            >
                              Editing: {input.id}
                            </Typography>
                            <Box sx={{ display: 'flex', gap: 1 }}>
                              <Button tone='secondary' size='sm' onClick={handleCancelEdit}>
                                Cancel
                              </Button>
                              <Button tone='primary' size='sm' onClick={handleSaveEdit} disabled={!editingInput.validation?.isValid}>
                                Save
                              </Button>
                            </Box>
                          </Box>
                          <Box sx={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 2, mb: 2, alignItems: 'start' }}>
                            ...
                            <FormField
                              ...
                              customStyle={{
                                ...
                                '& .MuiTextField-root': { margin: '0px', marginTop: 'var(--ds-space-1)', marginBottom: '0px' },
                          ...
                          />
                          <Box sx={{ mt: 2 }}>
                            <Typography variant='body2' sx={{ color: 'var(--ds-brand-500)', fontWeight: 'var(--ds-font-weight-medium)', mb: 1 }}>
                              Default Value
                            </Typography>
                ...
                {!showAddForm && (
                  <Button tone='secondary' size='sm' icon={<Add />} onClick={handleToggleAddForm}>
                    Add Parameter
                  </Button>
                )}
              </Box>
          ...
          <FormField
            ...
            customRender={
              <Box>
                ...
                />
                <Typography variant='body2' sx={{ color: 'var(--ds-gray-600)', fontSize: 'var(--ds-text-caption)', mt: 1 }}>
              ...
              <Box>
                <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
                  <FormField
                    ...
                    customRender={null}
                  />
                  <Button tone='secondary' size='sm' onClick={handleAddTag} disabled={!newTag.trim() || settings.tags.includes(newTag.trim())}>
                    Add
                  </Button>
                </Box>
