  ...
  useEffect(() => {
    if (hasIntegrationField && selectedActionType) {
      ...
      apiIntegrations
        ...
        .then((response) => {
          if (response?.data?.data?.admin_get_integrations_v2?.rows && response.data.data.admin_get_integrations_v2.rows.length > 0) {
            const integrationOptions = response.data.data.admin_get_integrations_v2.rows.map((item: any) => ({
              label: item.name,
              value: item.id,
              type: item.type,
