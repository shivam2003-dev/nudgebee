      ...
      >
        <DialogContent sx={{ padding: '24px' }}>
          ...
          {deleteCallersLoading && (
            <Box sx={{ mt: 2, display: 'flex', alignItems: 'center', gap: 1, color: colors.text.secondary, fontSize: '12px' }}>
          ...
          {!deleteCallersLoading && deleteCallers && deleteCallers.length > 0 && (
            ...
            >
              <Box sx={{ fontSize: '13px', fontWeight: 600, color: colors.error ?? '#b91c1c', mb: 0.5 }}>
                Used by {deleteCallers.length} other automation{deleteCallers.length === 1 ? '' : 's'}
              </Box>
              <Box sx={{ fontSize: '12px', color: colors.text.secondary, mb: 1 }}>
              ...
              <Box component='ul' sx={{ m: 0, pl: 2.5, maxHeight: 140, overflowY: 'auto' }}>
                {deleteCallers.map((c) => (
                  <Box component='li' key={c.id} sx={{ fontSize: '12px', color: colors.text.primary, mb: 0.25 }}>
                    <span>{c.name}</span>
                    <span style={{ color: colors.text.tertiary, marginLeft: 8 }}>({c.status})</span>
                  </Box>
                ))}
              </Box>
              <Box sx={{ fontSize: '11px', color: colors.text.tertiary, mt: 1 }}>
