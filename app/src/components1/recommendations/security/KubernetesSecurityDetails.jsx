          ...
          <Stack direction='column' spacing={1}>
            ...
            {item.ticket ? (
              <Typography sx={{ fontSize: '12px' }}>
                Ticket -
                <CustomLink href={item.ticket?.url} style={{ fontSize: '12px' }} target='_blank'>
      ...
      data.push({
        component: (
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            <CustomLabels text={item?.severity} />
          </Box>
        ),
        data: item?.severity,
      });
        ...
        {isGitReposLoading || filteredGitIntegrations.length > 0 ? (
          Object.keys(selectedWorkloadAnnotations).length > 0 ? (
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', marginBottom: '10px' }}>
                <CustomButton text='Cancel' variant='secondary' onClick={closeCreatePRModal} />
                <CustomButton
                  text='Create PR'
            ...
            <Box sx={{ p: 2 }}>
              ...
              <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }}>
                <CustomButton text='Close' variant='secondary' onClick={closeCreatePRModal} />
              </Box>
            </Box>
          )
        ) : (
