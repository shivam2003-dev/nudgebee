...
import { AWSIcon, AWSEC2Icon, AWSRDSIcon } from '@assets';
...
const AWSOptimisation = ({ accounts }) => {
  const [awsTotalSavings, setAwsTotalSavings] = useState(0);
  ...
  useEffect(() => {
    recommendationApi
      .getK8sRecommendationSummaryByRuleName({
        ...
        serviceName: ['AmazonEC2', 'AmazonRDS'],
      ...
      .then((res) => {
        if (res?.length > 0) {
          ...
          setAwsTotalSavings(Math.ceil(total));
  ...
  const getSectionProps = (serviceId, serviceName, category) => {
    ...
    return {
      ...
      viewAllHref: `/optimise-old/details?type=aws&subType=${category.id}&service=${serviceName}`,
    ...
    <>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', mt: '32px', px: '8px' }}>
        <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '8px' }}>
          <SafeIcon src={AWSIcon} alt='aws-ec2' width={24} height={24} />
          <Typography sx={{ color: colors.text.secondary, fontSize: '20px', fontWeight: 500 }}>AWS</Typography>
        </Box>

        <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
          <Typography sx={{ color: colors.text.secondaryDark, fontSize: '14px', fontWeight: 400, mt: '8px' }}>savings</Typography>
          {[
            { val: awsTotalSavings, suffix: '/mo' },
            { val: awsTotalSavings * 12, suffix: '/yr' },
          ].map((item, index) => (
...
export default AWSOptimisation;
