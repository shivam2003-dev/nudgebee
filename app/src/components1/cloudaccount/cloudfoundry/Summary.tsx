...
// Card-level section label — `bodyLg + medium + gray[700]`. CF cards stand on
// their own (no outer section heading wrapping them like in EC2/RDS/S3/ECS), so
// this label IS the section heading for each card.
const SectionLabel = ({ children }: any) => (
  <Typography sx={{ fontSize: ds.text.bodyLg, fontWeight: ds.weight.medium, color: ds.gray[700], mb: ds.space[3] }}>{children}</Typography>
);
...
const AppHealthBreakdown = ({ apps }: { apps: any[] }) => {
  ...
  return (
    <WidgetCard>
      ...
      <Box sx={{ display: 'flex', height: '8px', borderRadius: ds.radius.sm, overflow: 'hidden', mb: ds.space[2] }}>
        {segments.map((seg) => (
  ...
  return (
    <WidgetCard>
      ...
      {appsWithMem.map((app, idx) => (
        ...
        >
          <Box sx={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: app.isActive ? '#22C55E' : ds.gray[300], flexShrink: 0 }} />
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <LinearProgress
              ...
              sx={{
                ...
                '& .MuiLinearProgress-bar': { backgroundColor: ds.blue[500], borderRadius: 2 },
              }}
            />
          </Box>
        </Box>
