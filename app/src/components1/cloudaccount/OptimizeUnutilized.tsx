...
const OptimizeUtilization = ({ row, account, podName, recc, workloadName, namespaceName }: OptimizeUtilizationType) => {
  const kubeid = account;

  return (
    <OptimizeUtilizationCharts row={row} accountId={kubeid} podName={podName} workloadName={workloadName} namespaceName={namespaceName} recc={recc} />
  );
};

OptimizeUtilization.propTypes = {
  row: PropTypes.any,
...
export default OptimizeUtilization;
