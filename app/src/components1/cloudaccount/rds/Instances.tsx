    ...
    apiCloudAccount
      ...
      .then((res: any) => {
        ...
        const rdsResourceData = res.data?.data?.cloud_resourses?.map((item: any) => {
          ...
          const state = item?.status || item?.meta?.DBInstanceStatus || '-';
          const menuItems = buildMenuItems(rdsActions, state, writeAccess);
          data.push({
            component: (
    ...
    <BoxLayout2
      ...
      sharingOptions={{
        download: {
          ...
          onClick: () => {
            return {
              tableId: rdsOptimizeInstancesTable,
            };
          },
        },
        sharing: { enabled: false, onClick: null },
