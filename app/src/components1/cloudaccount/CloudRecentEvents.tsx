...
export function CloudRecentEvents({
  ...
  return (
    <Stack>
      <Typography sx={{ fontSize: ds.text.bodyLg, fontWeight: ds.weight.medium, color: ds.gray[700], mb: ds.space[2] }}>Recent Events</Typography>
      {/* `paddingRight` adds breathing room to the right edge so the "View
          all" link inside CustomTable2 doesn't sit flush against the card
          edge. CustomTable2's own internal pr is preserved on top of this. */}
      <WidgetCard sx={{ mt: 0, padding: 0, paddingRight: ds.space[3], overflow: 'hidden' }}>
        <CustomTable2
