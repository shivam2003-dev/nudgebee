...
const CloudLogsQueryHelp: React.FC<CloudLogsQueryHelpProps> = ({ provider, onInsertQuery }) => {
  ...
  return (
    <Box>
      ...
      >
        <HelpOutlineIcon sx={{ fontSize: 16, color: ds.gray[500] }} />
        <Typography sx={{ fontSize: ds.text.small, color: ds.gray[500] }}>Query Help — {syntaxInfo?.title}</Typography>
        {expanded ? <ExpandLessIcon sx={{ fontSize: 16, color: ds.gray[500] }} /> : <ExpandMoreIcon sx={{ fontSize: 16, color: ds.gray[500] }} />}
      </Box>
      <Collapse in={expanded}>
        ...
        >
          <Typography sx={{ fontSize: 11, fontFamily: 'monospace', whiteSpace: 'pre-line', mb: ds.space[3], color: ds.gray[600] }}>
            {syntaxInfo?.syntax}
          </Typography>
            ...
            {examples.map((example, idx) => (
              ...
              >
                ...
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <Typography sx={{ fontSize: 11, fontFamily: 'monospace', color: ds.gray[700], wordBreak: 'break-all' }}>{example.query}</Typography>
                  <Typography sx={{ fontSize: ds.text.caption, color: ds.gray[500], mt: '2px' }}>{example.description}</Typography>
                </Box>
                <DsButton
                  tone='ghost'
                  size='xs'
