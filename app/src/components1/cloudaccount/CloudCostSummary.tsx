 ...
 * one DSCard, and a green-tinted Savings card that is always rendered — when
 * there are no estimated savings, it shows a "No savings available" placeholder
...
// `CloudAccountSummary.SectionHeading`. Rendered inside DSCard's `header` slot;
// DSCard owns the divider + spacing below the heading.
const SectionTitle = ({ children }: { children: React.ReactNode }) => (
  <Typography sx={{ fontSize: ds.text.bodyLg, fontWeight: ds.weight.medium, color: ds.gray[700] }}>{children}</Typography>
);

// Bumps a Stat's label one weight notch (regular → medium). Selector targets the
// label Typography (`component='span'`) inside Stat's header row only — value
// uses `component='div'` and `sub` Typographies live outside the header row.
...
export function CloudCostSummary({ clusterSummary = {}, currencySymbol = '$' }: CloudCostSummaryProps) {
  ...
  return (
    <Stack gap={ds.space[2]}>
      <DSCard size='md' elevation='flat' header={<SectionTitle>Cost Summary</SectionTitle>}>
        <Stack direction='column' gap={ds.space[4]}>
          ...
          {(currentCredits > 0 || lastMonthCredits > 0) && (
            <Stat
              ...
              sx={STAT_LABEL_BOLD_SX}
              value={
                <Box sx={{ display: 'inline-flex', flexDirection: 'column', gap: ds.space[1] }}>
                  {currentCredits > 0 && (
      ...
      {/* Savings card — always rendered for layout parity with the parent cost
          card. When there's no estimated savings to show, the figure is replaced
          with a "No savings available" placeholder (mirrors CloudAccountSummary),
          and the doughnut chart is suppressed via its own savingsPercentage>0 gate. */}
      <DSCard
        size='md'
        elevation='flat'
        sx={{
