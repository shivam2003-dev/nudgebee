  ...
  try {
    ...
    return (
      <Box>
        ...
        <Typography sx={{ color: '#9F9F9F', fontSize: 11, marginTop: '2px' }}>Period: {period}</Typography>
        <Typography sx={{ color: '#9F9F9F', fontSize: 11, marginTop: '2px' }}>Query-Date: {queryDate}</Typography>
      </Box>
    );
  } catch (error) {
...
const DetailBlockDeviceMappingEc2Table = ({ blockDeviceMappingData }: any) => {
  ...
  if (blockDeviceMappingData && blockDeviceMappingData.length > 0) {
    ...
    return (
      <BoxLayout2
        heading='Block Device Mapping'
        sharingOptions={{
          download: {
            enabled: true,
            onClick: () => {
              return {
                tableId: blockDeviceTableId,
              };
            },
          },
          sharing: { enabled: false, onClick: null },
...
const GCPSectionHeader = ({ children }: { children: React.ReactNode }) => (
  <Typography sx={{ gridColumn: '1 / -1', fontWeight: 600, fontSize: 14, color: '#374151' }}>{children}</Typography>
);
...
const InstancesView = (props: {
  ...
  const onInstanceSearchFilterChange = (e: any) => {
    if (selectedInstanceIdName != '' && e.target.value === '') {
      setPage(0);
    }
    setSelectedInstanceIdName(e.target.value);
  };
  ...
  const listEC2Instances = async () => {
    ...
    try {
      ...
      const ec2ResourceData = resources.map((item: any) => {
        ...
        data.push({
          component: <CustomLabels variant={color} text={state} />,
        });
    ...
    <BoxLayout2
      ...
      sharingOptions={{
        download: {
          ...
          onClick: () => {
            return {
              tableId: ec2OptimizeInstancesTable,
            };
          },
        },
        sharing: { enabled: false, onClick: null },
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        {azureProps.storageProfile?.osDisk?.osType && <DataBlock title={'OS Type'} data={azureProps.storageProfile.osDisk.osType} />}
                        {azureProps.storageProfile?.osDisk?.name && <DataBlock title={'OS Disk Name'} data={azureProps.storageProfile.osDisk.name} />}
                        ...
                        {azureProps.networkProfile?.networkInterfaces && azureProps.networkProfile.networkInterfaces.length > 0 && (
                          <DataBlock title={'Network Interfaces'}>
                            {azureProps.networkProfile.networkInterfaces.map((ni: any, idx: number) => (
                              <Typography key={`${ni.id}-${idx}`} fontSize={'13px'}>
                      ...
                      {drilldownQuery.meta.SecurityGroups && drilldownQuery.meta.SecurityGroups.length > 0 && (
                        <DataBlock title={'Security Groups'}>
                          {drilldownQuery.meta.SecurityGroups.map((sg: any, idx: number) => (
                            <Typography key={`${sg.GroupId ?? sg.GroupName}-${idx}`} fontSize={'13px'}>
                      ...
                      {drilldownQuery.meta?.NetworkInterfaces && drilldownQuery.meta?.NetworkInterfaces?.length > 0 ? (
                        <DataBlock title={'Private IPv4 addresses'}>
                          {drilldownQuery.meta.NetworkInterfaces.map((ni: any, idx: number) => (
                            <Typography key={`${ni.NetworkInterfaceId ?? ni.PrivateIpAddress}-${idx}`} fontSize={'13px'}>
