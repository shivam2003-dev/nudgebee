/**
 * @deprecated Use `Stat` from '@components1/ds/Stat' instead.
 * V2 ships size variants (sm/md/hero), cost-axis delta tones (D7), structured
 * delta + info props, optional icon slot, and `--ds-*` tokens.
 *   V1 prop           →  V2 prop
 *   title             →  label
 *   value             →  value
 *   variant='savings' →  pair with `delta={{ tone: 'savings', ... }}` (V2 spec: don't tone the value, tone the delta)
 *   size='small'      →  size='sm'
...
let _summaryWidgetWarned = false;
...
const SummaryWidget = ({
  ...
  useEffect(() => {
    if (_summaryWidgetWarned) return;
    _summaryWidgetWarned = true;
    // eslint-disable-next-line no-console
    console.warn(
      '[deprecated] SummaryWidget is deprecated. Use `import { Stat } from "@components1/ds/Stat"` instead. ' + 'Tracked for removal 2026-06-06.'
    );
  }, []);
