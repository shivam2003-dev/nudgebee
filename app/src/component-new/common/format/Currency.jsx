...
export default function Currency({
  ...
  const variantStyles = {
    default: {},
    savings: { color: 'var(--ds-green-500)' },
    expense: { color: 'var(--ds-red-500)' },
  };
  ...
  if (value == null || value == undefined || isNaN(value) || value === '' || value === 0) {
    return withTooltip ? (
      <CustomTooltip title={defaultVal !== '-' ? defaultVal : ''} placement='bottom-start' tooltipStyle={{ margin: '0px' }}>
        <Typography
          sx={{ fontSize: 'var(--ds-text-body)', color: colors.text.secondary, fontWeight: 400, lineHeight: 'auto', ...sx }}
          display='inline'
        >
          {defaultVal}
        </Typography>
      ...
      <Typography sx={{ fontSize: 'var(--ds-text-body)', color: colors.text.secondary, fontWeight: 400, lineHeight: 'auto', ...sx }} display='inline'>
        {defaultVal}
      </Typography>
    );
  }
  ...
  if (isSavingPotential && negPotentialSaving > 0) {
    ...
    return (
      <CustomTooltip title={description} placement='top'>
        <Typography
          sx={{ fontSize: 'var(--ds-text-body)', color: colors.text.secondary, fontWeight: 400, letterSpacing: '0.5px', ...sx }}
          display='inline'
        >
          {'~ '}
          {prefix}
          {'0'}
          {suffix && (
            <Typography component='span' sx={{ fontSize: 'var(--ds-text-small)', color: colors.text.secondaryDark, fontWeight: 400, ...sxSuffix }}>
              {suffix}
            </Typography>
          )}
        </Typography>
    ...
    >
      <Box>
        {parseFloat(actualValue) < 1 && precison === 0 && (
          <Typography sx={{ fontSize: 'var(--ds-text-small)', color: colors.text.secondaryDark, fontWeight: 400, ...sxPrefix }} display='inline'>
            {'<'}
          </Typography>
        )}
        <Typography sx={{ fontSize: 'var(--ds-text-small)', color: colors.text.secondaryDark, fontWeight: 400, ...sxPrefix }} display='inline'>
        ...
        {suffix && (
          <Typography sx={{ fontSize: 'var(--ds-text-small)', color: colors.text.secondaryDark, fontWeight: 400, ...sxSuffix }} display='inline'>
            {suffix}
          </Typography>
        )}
      </Box>
