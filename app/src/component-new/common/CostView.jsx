      ...
      {data.map((entry, index) => (
        <Box key={index}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            {entry?.name === 'Forecast Month' && data[1]?.cost != null && data[2]?.cost != null && data[1].cost !== 0 && (
              <Trend sign={data[1].cost > data[2].cost ? 1 : -1} value={(Math.abs(data[1].cost - data[2].cost) * 100) / data[1].cost} />
            )}
          </Box>
        </Box>
      ))}
