...
export const ExpandedRowComponent = ({ row = [], tabOptions = [], isExpanded = false, tabPadding }) => {
  ...
  return isExpanded ? (
    <Box
      sx={{
        // Redesigned Table: the expanded panel is a calm inset surface — one 1px
        // top rule for separation, no drop shadow or side borders.
        p: 'var(--ds-space-4)',
  ...
  return (
    <>
      <TableRow
        sx={{
          ...
          '& td': {
            ...
            // Redesigned Table: an expanded row tints to gray-100 and drops its
            // divider so it reads as one unit with the panel beneath.
            ...(isExpandable && collapsedObj[itemNo] ? { backgroundColor: ds.gray[100], borderBottom: '0 !important' } : {}),
          },
          ...row.sx,
        }}
        onClick={() => {
      ...
      >
        ...
        {isExpandable ? (
          <TableCell data-export-enabled={false} sx={{ width: 40, padding: '8px' }} onClick={(e) => e.stopPropagation()}>
...
const CustomTable = ({
  ...
  const renderSkeletonBody = () => {
    const skeletonRowCount = Math.min(rowsPerPage, 10);
    return (
      <TableBody>
    ...
    <>
      <TableContainer
        ...
        sx={{
          ...
          // rules so the table breathes against the page surface.
          borderTop: `1px solid ${ds.gray[200]}`,
      ...
      >
        ...
        >
          ...
          >
            ...
            <TableRow
              sx={{
                // Redesigned Table header: white background (not a tinted fill),
                // delimited by a single 1px brand-navy bottom border at brand-300
                // (two steps lighter than the original brand-500). Header text
                // reads in brand-navy at medium weight. No corner rounding.
                th: {
                  fontWeight: 500,
                  fontFamily: 'poppins',
                  lineHeight: '17px',
                  color: ds.brand[600],
                  background: ds.background[100],
                  borderBottom: `1px solid ${ds.brand[300]}`,
                },
              }}
            >
              {filteredHeaders.map((head, idx) => {
