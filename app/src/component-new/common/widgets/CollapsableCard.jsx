  ...
  return (
    <Box
      ...
      sx={{
        ...
        borderBottom: collapsedObj[idx] ? `2px solid ${colors.text.primaryLight}` : '0.5px solid var(--ds-gray-200)',
    ...
    >
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column' }}>
        ...
        >
          <Grid item md={5} sx={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
            <SafeIcon
              ...
              className='icon'
            />
            <Typography sx={{ fontFamily: 'Poppins', fontWeight: 500, fontSize: '12px', color: colors.secondary.default }} variant='body1'>
              {text}
            </Typography>
          ...
          {highlightsData.length > 0 ? (
            ...
            >
              {visibleItems?.map((hobj, index) => (
                <Box key={index} sx={{ display: 'flex', alignItems: 'center', gap: '4px', minWidth: 0 }}>
              ...
              {highlightsData?.length > MAX_VISIBLE && (
                <ListItem
                  alignItems='center'
                  sx={{ p: '0px 0px 4px 16px', cursor: 'pointer' }}
                  onClick={(e) => {
                    e.stopPropagation();
                    setShowAll(!showAll);
                  }}
        ...
        >
          {isExpanded && eventResolution && (
            <CustomBorderCard
              ...
              borderLeftWidth='3px'
              padding='10px 16px'
              sx={{ mb: '12px', bgcolor: colors.background.suggestionCardBG }}
            >
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                    ...
                    {eventResolution.status === 'Failed' && eventResolution.status_message && (
                      <Typography sx={{ fontSize: '12px', color: colors.highest, mt: '4px' }}>{eventResolution.status_message}</Typography>
                    )}
                  </Box>
                </Box>
