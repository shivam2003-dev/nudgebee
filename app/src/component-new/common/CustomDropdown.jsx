/**
 * CustomDropdown — domain swiss-army-knife value-picker for the existing app.
 *
 * NOTE: despite its name, this is NOT a DropdownMenu (action menu). It's an
 * Autocomplete-style value-picker / Select with cluster/cloud-account-specific
 * chrome. The audit's original "DropdownMenu cluster" framing was wrong.
 *
 * For NEW code, prefer the appropriate DS primitive based on shape:
 *   - `{ Select } from '@components1/ds/Select'` — short option list, no search.
 *   - `{ Autocomplete } from '@components1/ds/Autocomplete'` — searchable, async, multi.
 *
 * For existing call sites that depend on any of the following, keep using
 * CustomDropdown — neither `ds/Select` nor `ds/Autocomplete` models them
 * (they're cluster-picker / cloud-account-specific behavior):
 *   - `groupByCloudProvider` (AWS/Azure/GCP/K8s/OCI/CloudFoundry section
 *     headers + per-section sort by agent connection-status priority)
 *   - `clusterData` + `showStatusIndicator` / `showIndicator` (cluster
 *     connection-status circle rendered in the input + each row)
 *   - `openDirection: 'auto'` + `showDynamicPaper` (popup direction calculated
 *     from viewport space below the trigger; `ds/Select` is bottom-only)
 *   - `headerStyle` (top-bar mode used by `Header1.jsx` only)
 *   - `additionalAutoCompleteProps` / `componentsProps` / `customStyle` escape
 *     hatches used by tabular filter pickers
 *   - `inputVariant: 'outlined' | 'standard'` (V2 is outlined-only)
 *
 * Previously deprecated 2026-05-07 → soft-demoted to domain composition
 * 2026-05-07 (Phase 1 Step 3). Hard migration was rejected: the audit's
 * "lump CustomDropdown into the DropdownMenu cluster" framing was wrong;
 * neither `ds/Select` nor `ds/Autocomplete` covers the cluster-picker
 * surface. The two primitive families co-exist: `ds/Select` + `ds/Autocomplete`
 * for DS-clean value pickers, `CustomDropdown` for the existing 31 cluster /
 * cloud-account / form-field call sites. Wrapper `ClusterDropDown.jsx` is
 * THE app cluster picker; rebuilding its features into `ds/` would require
 * ~5 DS-feature builds (groupBy with custom sort, status indicator slot,
 * auto-direction popup, header-bar mode, escape-hatch sx forwarding).
 */
    ...
    return (
      <Paper
        sx={{
          ...
          ...paperStyle,
          ...positionStyle,
          border: headerStyle && `0.5px solid ${colors.border.primaryLight}`,
          boxShadow: headerStyle && '0px 4px 10px 0px #89899340',
  ...
  const renderOption = (props, option) => {
    ...
    return (
      ...
      >
        ...
        {option?.icon && (
          <SafeIcon src={option.icon} alt={option?.label || ''} style={{ height: '18px', width: '18px', marginRight: '8px', flexShrink: 0 }} />
        )}
