...
const StyledTooltip = styled(
  ...
  const getPadding = () => {
    switch (variant) {
      case 'explainer':
      case 'interactive':
        return 'var(--ds-space-3) var(--ds-space-4)';
      case 'default':
      default:
        return 'var(--ds-space-2) var(--ds-space-3)';
    }
  };
  ...
  return {
    ...
    [`& .${tooltipClasses.arrow}`]: {
      ...
      '&::before': {
        backgroundColor: 'var(--ds-background-100)',
        border: '1px solid var(--ds-brand-300)',
        boxShadow: '0px 2px 4px var(--ds-gray-alpha-100)',
      },
    },
  };
});
...
const TooltipContent: React.FC<{
  ...
  if (variant === 'explainer') {
    return (
      <Box>
        <Box
          sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--ds-space-0)', gap: 'var(--ds-space-1)' }}
        >
          <Typography
            variant='body2'
            sx={{
  ...
  return (
    <Box
      sx={{
        ...
        '&::-webkit-scrollbar-thumb': { background: 'var(--ds-gray-alpha-300)', borderRadius: 'var(--ds-space-0)' },
        scrollbarWidth: 'thin',
        scrollbarColor: 'var(--ds-gray-alpha-300) transparent',
      }}
    >
      {title}
    </Box>
