 ...
 *
 * Owns ONLY the shell: card chrome, sticky toolbar styling, body padding,
 ...
 *     Table spec.
 *   - Don't override the toolbar's sticky background unless the host scroll
 *     container has a non-default background — the default is ds.background.100
 *     so the sticky toolbar visually merges with the card surface.
 */
...
export interface ListingLayoutToolbarProps {
  ...
  /** Stick to the top of the nearest scroll container. Default true. */
  sticky?: boolean;
  id?: string;
  'data-testid'?: string;
  sx?: SxProps<Theme>;
...
function Toolbar({ title, children, actions, sticky = true, id, sx, ...rest }: ListingLayoutToolbarProps) {
  return (
    <Box
      id={id}
      sx={{
