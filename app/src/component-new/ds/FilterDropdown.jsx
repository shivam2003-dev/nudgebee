  ...
  return (
    <Box
      ...
      sx={{
        ...
        '&:hover': {
          backgroundColor: selected ? 'var(--ds-blue-100)' : 'var(--ds-gray-100)',
        },
        ...style,
      }}
    >
      {multiple && (
        <Box
          sx={{
            ...
            height: 16,
            borderRadius: '4px',
            border: selected ? 'none' : `1.5px solid var(--ds-gray-300)`,
            backgroundColor: selected ? 'var(--ds-blue-600)' : 'transparent',
            display: 'flex',
            alignItems: 'center',
...
function OptionsList({ isOptionsLoading, filteredOptions, multiple, isSelected, handleToggle, handleClear, handleSelectAll }) {
  ...
  if (isOptionsLoading) {
    return (
      <Box sx={scrollboxSx}>
        <Typography sx={{ padding: '16px 14px', fontSize: '13px', color: 'var(--ds-gray-500)', textAlign: 'center' }}>Loading...</Typography>
      </Box>
    );
  }
  ...
  if (filteredOptions.length === 0) {
    return (
      <Box sx={scrollboxSx}>
        <Typography sx={{ padding: '16px 14px', fontSize: '13px', color: 'var(--ds-gray-500)', textAlign: 'center' }}>No results found</Typography>
      </Box>
    );
  }
          ...
          >
            ...
            >
              <Box
                sx={{
                  ...
                  height: 16,
                  borderRadius: '4px',
                  border: filteredOptions.every(isSelected) ? 'none' : `1.5px solid var(--ds-gray-300)`,
                  backgroundColor: filteredOptions.every(isSelected) ? 'var(--ds-blue-600)' : 'transparent',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  transition: 'all 0.15s ease',
            ...
            {selectedOptions.length > 0 && (
              <Typography
                ...
                sx={{ fontSize: '11px', fontWeight: 500, color: 'var(--ds-blue-600)', cursor: 'pointer', '&:hover': { opacity: 0.7 } }}
              >
                Clear All
              </Typography>
            )}
...
function GroupedOptionsList({ isOptionsLoading, filteredOptions, multiple, isSelected, handleToggle, value, onSelect, groupIcon }) {
          ...
          <Box key={groupName}>
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <Typography
                  sx={{ fontSize: '11px', fontWeight: 600, color: 'var(--ds-gray-700)', textTransform: 'uppercase', letterSpacing: '0.02em' }}
                >
                  {groupName}
                </Typography>
                {hasGroupSelection && (
    ...
    <>
      <Box
        ...
        sx={{
          ...
          fontWeight: 450,
          fontFamily: 'inherit',
          color: hasSelection ? 'var(--ds-blue-600)' : 'var(--ds-gray-700)',
          backgroundColor: hasSelection ? 'var(--ds-blue-100)' : 'var(--ds-background-100)',
          border: hasSelection ? '1px solid rgba(0, 98, 255, 0.21)' : '1px solid var(--ds-gray-300)',
          boxShadow: hasSelection ? '0 0 0 2px var(--ds-blue-100)' : '0 4px 4px rgba(0, 0, 0, 0.04)',
          borderRadius: '6px',
      ...
      >
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', flex: 1, overflow: 'hidden', minWidth: 0 }}>
          {(label || (!hasSelection && placeholder) || isOptionsLoading) && (
            <span style={{ color: hasSelection ? 'var(--ds-gray-500)' : 'inherit', fontWeight: 400, flexShrink: 0 }}>
              {label || (!hasSelection ? placeholder : '')}
              {required && (label || !hasSelection) && <span style={{ color: 'var(--ds-red-600)' }}> *</span>}
              {isOptionsLoading && <span style={{ marginLeft: '4px', fontSize: '11px', color: 'var(--ds-gray-500)', fontWeight: 400 }}>...</span>}
            </span>
          )}
            ...
            <>
              {selectedDisplayText.labels.map((lbl, idx) => (
                <React.Fragment key={lbl}>
                  {idx > 0 && <span style={{ color: 'var(--ds-blue-600)', fontWeight: 500 }}>, </span>}
                  <TruncatedLabel label={lbl} />
                </React.Fragment>
              ))}
