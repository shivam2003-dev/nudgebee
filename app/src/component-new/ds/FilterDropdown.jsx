  ...
  return (
    <Box
      ...
      sx={{
        ...
        alignItems: 'center',
        gap: '10px',
        // Geometry from shared --ds-overlay-item-* tokens so DropdownMenu /
        // Select / MultiSelect / FilterDropdown rows stay byte-identical.
    ...
    >
      ...
      <Text sx={{ color: 'inherit', flex: 1, minWidth: 0 }} value={getLabel(opt)} showAutoEllipsis />
      {opt?.type && (
        <Box sx={{ ml: 'auto', flexShrink: 0 }}>
          <CustomLabels text={opt.type} />
        </Box>
      )}
    </Box>
  );
  ...
  return (
    <>
      <Box
        ...
        disabled={disabled}
        sx={{
          // Field chrome — mirrors ds/Input so FilterDropdown rows align with
          // Input / Select rows when they sit together. When a filter IS applied,
          // the trigger picks up the same blue-500 border + 3px blue-100 halo as
          // the focus state — a persistent visual cue that the filter is active.
          display: 'inline-flex',
          alignItems: 'center',
          justifyContent: 'space-between',
          gap: 'var(--ds-space-2)',
          ...
          // Anchor fontFamily on the parent so the label span and the value
          // (TruncatedLabel) inherit the same family — without this, browser
          // defaults on <button> + per-weight font fallbacks made "Issue Type"
          // (weight 400) and "All Issues" (weight 600) render in different
          // fonts in some configurations.
          fontFamily: 'inherit',
          fontSize: '12px',
          fontWeight: 400,
          lineHeight: 1.4,
            ...
            <>
              {selectedDisplayText.labels.map((lbl, idx) => (
                <React.Fragment key={lbl}>
                  {idx > 0 && <span style={{ color: 'var(--ds-gray-800)', fontWeight: 400 }}>, </span>}
