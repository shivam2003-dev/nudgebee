 ...
 *                  Only meaningful when variant='accent'. Dev-warns otherwise.
 *   interactive  = boolean
 ...
 *
 * Don't:
 *   - Don't tint the background. Use Banner if you need a tinted surface.
...
export const Card: React.FC<CardProps> = ({
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (tone !== 'neutral' && variant !== 'accent') {
      // eslint-disable-next-line no-console
      console.warn(`[Card] tone="${tone}" is only meaningful when variant="accent" (got variant="${variant}"). Ignoring tone.`);
    }
  ...
  const showShadow = elevation === 'raised';

  const isOutlined = variant === 'outlined';
  const isAccent = variant === 'accent';

  const borderWidth = selected ? 2 : isOutlined ? 1 : 1;
  const borderColor = selected ? 'var(--ds-blue-500)' : 'var(--ds-gray-200)';
