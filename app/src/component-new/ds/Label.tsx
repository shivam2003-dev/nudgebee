 ...
 *
 * Tooltip:     set `displayTooltip` + `tooltipCharLimit` to enable truncation tooltip.
 *              Text longer than `tooltipCharLimit` is visually truncated; hovering
 *              shows the full string via CustomTooltip (white-card, no native black bg).
 *              `tooltipPosition` controls placement (default 'bottom').
 *
...
export interface LabelProps {
  ...
  /** Tooltip placement when truncation tooltip is shown. Defaults to 'bottom'. */
...
const Label: React.FC<LabelProps> = ({
  ...
  const labelEl = (
    ...
    <Box component='span' id={id} className={className} sx={labelStyle}>
      {showDot && (
        <Box
          component='span'
          aria-hidden='true'
  ...
  if (shouldShowTooltip) {
    return (
      <Tooltip title={text!} tooltipClassName={''} placement={tooltipPosition}>
        <span style={{ display: 'inline-flex' }}>{labelEl}</span>
      </Tooltip>
    );
  }
