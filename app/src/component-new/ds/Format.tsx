...
function CurrencyFmt({ value, unit = 'USD', variant, period, locale, className, id }: FormatCurrencyProps) {
  const resolvedVariant: FormatCurrencyVariant = variant ?? (period ? 'with-period' : 'full');
  const main = React.useMemo(() => {
    const opts: Intl.NumberFormatOptions = { style: 'currency', currency: unit, maximumFractionDigits: 0 };
    if (resolvedVariant === 'compact') {
      opts.notation = 'compact';
      opts.maximumFractionDigits = 1;
    }
    return new Intl.NumberFormat(locale, opts).format(value);
  }, [value, unit, resolvedVariant, locale]);
  return (
    <Box component='span' className={className} id={id} sx={{ fontVariantNumeric: 'tabular-nums' }}>
      {main}
      {resolvedVariant === 'with-period' && period && (
        <Box component='span' sx={{ color: 'var(--ds-gray-500)', marginLeft: '4px' }}>
          {period}
        </Box>
      )}
    </Box>
