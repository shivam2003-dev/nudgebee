...
jest.mock('@api1/kubernetes', () => ({
  __esModule: true,
  default: {
    getLatestVersions: jest.fn().mockResolvedValue({ data: { nudgebee_list_versions: null } }),
  },
}));
...
describe('Header1', () => {
  ...
  beforeEach(() => {
    ...
    apiKubernetes.getLatestVersions.mockResolvedValue({ data: { nudgebee_list_versions: null } });
  ...
  describe('demo account banner', () => {
    ...
    it('navigates to user-management integrations on Connect Account click', async () => {
      useData.mockReturnValue({
        selectedCluster: { value: 'demo', cloud_provider: 'K8s', agent: {} },
        allCluster: [{ value: 'demo', cloud_provider: 'K8s' }],
      });
      ...
      fireEvent.click(screen.getByTestId('custom-btn-Connect Account'));
      expect(mockRouter.push).toHaveBeenCalledWith('/user-management#integrations');
    });

    it('opens help docs on demo-banner "Need help?" click', async () => {
      useData.mockReturnValue({
        selectedCluster: { value: 'demo', cloud_provider: 'K8s', agent: {} },
        allCluster: [{ value: 'demo', cloud_provider: 'K8s' }],
      });
      ...
      fireEvent.click(screen.getByTestId('demo-banner-help'));
      expect(window.open).toHaveBeenCalledWith('https://docs.nudgebee.com/docs/features/', '_blank', 'noopener');
    });
  });
  ...
  describe('snackbar / agent version alerts', () => {
    it('shows snackbar when agent is NOT_CONNECTED', async () => {
      ...
      apiKubernetes.getLatestVersions.mockResolvedValue({ data: { nudgebee_list_versions: { agent_version_latest: '2.0.0' } } });
    ...
    it('shows snackbar when agent version is outdated', async () => {
      ...
      apiKubernetes.getLatestVersions.mockResolvedValue({
        data: { nudgebee_list_versions: { agent_version_latest: '2.0.0' } },
      });
  ...
  describe('connect account buttons', () => {
    ...
    it('opens Slack install URL in new tab', async () => {
      ...
      expect(window.open).toHaveBeenCalledWith('/api/integrations/slack/install', '_blank');
    });
    ...
    it('opens MS Teams install URL in new tab', async () => {
      ...
      expect(window.open).toHaveBeenCalledWith('/api/integrations/ms-teams/install', '_blank', '"noopener"');
    });
    ...
    it('opens Google Chat install URL in new tab', async () => {
      ...
      expect(window.open).toHaveBeenCalledWith('/api/integrations/google/install', '_blank');
    });
