...
// Ask for a name — must be one of TC_TESTER_NAMES (case-insensitive)
async function askTesterName(rl, fieldLabel) {
  const list = TESTER_NAMES.join(" / ");
  while (true) {
    const input = await ask(rl, `   ${fieldLabel} (${list}): `);
    const name = input.charAt(0).toUpperCase() + input.slice(1).toLowerCase();
    if (TESTER_NAMES.includes(name)) return name;
    console.log(`   ⚠️  Invalid input "${input}". Please enter one of: ${list}`);
  }
}
