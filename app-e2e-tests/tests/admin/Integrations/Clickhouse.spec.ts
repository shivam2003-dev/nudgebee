...
test("Add Clickhouse Account Integration", async ({ page }) => {
  ...
  await waitForGraphQLAndValidate(
    ...
    },
    {
      ...
      ignoreErrorMessages: [`account '${process.env.CLUSTER}' already has a 'clickhouse' integration ('clickhouse-test'); only one 'clickhouse' integration per account is supported — edit the existing one or remove it before adding another`],
    },
  );
});
