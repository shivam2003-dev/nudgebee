...
| Source branch | `main` |
| Source commit | `e0a70c9ae77784c54d65895689e0ca9e146a9059` |
| Drop date | 2026-05-26 |
| Iteration | 14 |
...
#   deploy/kubernetes/nudgebee/values.yaml — stripped 12 stale per-
#     subchart `image: { repository: nudgebee-<x>, tag: 2024-... }`
#     overrides left over from EE-internal registry naming. Subchart
#     defaults (which release.yaml's chart-bump auto-updates to
#     `ghcr.io/<owner>/<image>:<version>`) now win, producing the
#     correct OSS image refs at install time. EE deploys override
#     globally via `values-enterprise.yaml`. **Back-port to EE so this
#     falls out of Group B** once the EE pipeline either inherits
#     OSS defaults or moves its prefixed-image-name override into
#     `values-enterprise.yaml` (which is .oss-exclude'd).
#
