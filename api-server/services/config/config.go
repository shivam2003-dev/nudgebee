...
type appConfig struct {
	...
	HasuraMetadataEndpoint   string `mapstructure:"hasura_metadata_endpoint"`
	HasuraGraphqlEndpoint    string `mapstructure:"hasura_graphql_endpoint"`
	HasuraGraphqlAdminSecret string `mapstructure:"hasura_graphql_admin_secret"`
	...
	BillingFreeCluster        int     `mapstructure:"billing_free_cluster"`
	BillingFreeNodes          int     `mapstructure:"billing_free_nodes"`
	BillingFreeAutomationRuns int     `mapstructure:"billing_free_automation_runs"`
	BillingClusterCharges     float32 `mapstructure:"billing_cluster_charges"`
	BillingNodeCharges        float32 `mapstructure:"billing_node_charges"`
	BillingAutomationCharges  float32 `mapstructure:"billing_automation_charges"`

	AwsSellerAccessKey                    string `mapstructure:"aws_seller_access_key"`
	AwsSellerSecretKey                    string `mapstructure:"aws_seller_secret_key"`
	AwsMarketplaceSubscriptionStatusQueue string `mapstructure:"aws_marketplace_subscription_status_queue"`
	AwsMarketplacePurchaseUpdatesQueue    string `mapstructure:"aws_marketplace_purchase_updates_queue"`

	NBRetentionDaysHasuraCronEvents        int `mapstructure:"nb_retention_days_hasura_cron_events"`
...
func init() {
	...
	viper.SetDefault("hasura_metadata_endpoint", "http://localhost:8080/v1/metadata")
	viper.SetDefault("hasura_graphql_endpoint", "http://localhost:8080/v1/graphql")
	...
	viper.SetDefault("hasura_graphql_admin_secret", "")
	...
	viper.SetDefault("git_commit_nudgebee_user_email", "nudgebee-git-bot@nudgebee.com")
	viper.SetDefault("aws_template_url", "https://nudgebee-documents.s3.amazonaws.com/nudgebee-aws-cloud-formation.json")
	...
	viper.SetDefault("cloud_collector_aws_eventbridge_sqs", "nudgebee-eventbridge-queue")
	...
	viper.SetDefault("billing_free_cluster", 2)
	viper.SetDefault("billing_free_nodes", 3)
	viper.SetDefault("billing_free_automation_runs", 1000)
	viper.SetDefault("billing_cluster_charges", 5.0)
	viper.SetDefault("billing_node_charges", 0.50)
	viper.SetDefault("billing_automation_charges", 100)

	viper.SetDefault("aws_seller_access_key", "")
	viper.SetDefault("aws_seller_secret_key", "")

	viper.SetDefault("aws_marketplace_purchase_updates_queue", "")
	viper.SetDefault("aws_marketplace_subscription_status_queue", "")
	...
	if !strings.Contains(Config.HasuraGraphqlEndpoint, "/v1/graphql") {
		Config.HasuraGraphqlEndpoint = Config.HasuraGraphqlEndpoint + "/v1/graphql"
	}
