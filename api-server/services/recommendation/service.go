...
func ApplyRecommendation(ctx *security.RequestContext, query RecommendationApplyRequest) (RecommendationApplyResponse, error) {
	...
	if query.Provider == "git" {
		...
		if gitRepoURL == "" {
			return RecommendationApplyResponse{}, fmt.Errorf("recommendation: ci.nudgebee.com/git.repo annotation not found on workload")
		}
...
func UpdateResolutionStatus(ctx *security.RequestContext) error {
	...
	for _, resolution := range recommendationResolutionsToCheck {
		...
		adapterContext := security.NewRequestContext(ctx.GetContext(), security.NewSecurityContextForSuperAdminAndTenant(recommendation.TenantId), ctx.GetLogger(), ctx.GetTracer(), ctx.GetMeter())
