...
// traceTree is the parent→children adjacency for a single trace_id, built once
// per insight pass and reused by both error aggregation and the service-flow
// walk. Building from the parent_span_id field rather than wall-clock timestamp
// is what makes the downstream output correct: a distributed trace with
// retries, concurrent fan-out, or out-of-order span arrival has timestamps
// that do not match call order, but `parent_span_id` always does.
type traceTree struct {
	// spans is span_id → raw span map for O(1) lookup.
	spans map[string]map[string]any
	// children is parent_span_id → []child_span_id. A span_id missing from this
	// map means the span has no children in the batch we received.
	children map[string][]string
	// roots are the span_ids in this trace whose `parent_span_id` is empty OR
	// refers to a span we don't have in the batch (a partial trace — common
	// when sampling drops upstream spans). DFS starts from each root.
	roots []string
}

// buildTraceTrees groups the batch by trace_id and builds the parent→children
// adjacency for each. A span with an unknown parent is treated as a root of
// its trace — that handles partial traces gracefully without dropping spans.
// Spans without a span_id (older shapes, synthetic test inputs) are admitted
// as single-node roots so the aggregation surface still works on minimal
// inputs that lack tree metadata.
func buildTraceTrees(traces []map[string]any) map[string]*traceTree {
	trees := make(map[string]*traceTree)
	synthetic := 0
	for _, span := range traces {
		traceID := traceGetString(span, "trace_id")
		if traceID == "" {
			continue
		}
		spanID := traceGetString(span, "span_id")
		if spanID == "" {
			// Synthesise a unique id so the span is still admitted as a
			// single-node root. Cannot collide with real span_ids because
			// real ids never contain "/".
			synthetic++
			spanID = fmt.Sprintf("__synthetic__/%d", synthetic)
		}
		t, ok := trees[traceID]
		if !ok {
			t = &traceTree{
	...
	// Second pass: now that every span_id is known per trace, resolve parent
	// links and surface orphans as roots.
	for _, tree := range trees {
		for id, span := range tree.spans {
			parent := traceGetString(span, "parent_span_id")
			if parent == "" || tree.spans[parent] == nil {
				tree.roots = append(tree.roots, id)
				continue
			}
			tree.children[parent] = append(tree.children[parent], id)
		}
		// Deterministic root order: by span timestamp so output is stable.
		sort.SliceStable(tree.roots, func(i, j int) bool {
			return traceGetString(tree.spans[tree.roots[i]], "timestamp") <
				traceGetString(tree.spans[tree.roots[j]], "timestamp")
		})
		// Pre-sort each parent's children by start timestamp. Done once here
		// instead of inside the DFS walk so the walk is allocation-free and
		// O(N) over the trace, and every consumer (errorSubtreeLeaves,
		// traceExtractServiceFlow, future readers) sees the same canonical
		// child order.
		for parent := range tree.children {
			kids := tree.children[parent]
			sort.SliceStable(kids, func(i, j int) bool {
				return traceGetString(tree.spans[kids[i]], "timestamp") <
					traceGetString(tree.spans[kids[j]], "timestamp")
			})
...
// errorSubtreeLeaves returns the span_ids whose subtree (the span itself plus
// every descendant) contains exactly one error span — the span itself. These
// are the "root cause" error spans: they triggered the failure, and every
// error span above them in the tree is just propagation up the call chain.
//
// For a typical microservice cascade
//
//	load-gen → frontend → checkout → payment (real failure here)
//
// the four spans are all marked error, but only payment is the root cause;
// the others received the failure as a gRPC return value and propagated it.
// Aggregating over only the leaves collapses the 4× cascade into 1× actual
// problem.
func (t *traceTree) errorSubtreeLeaves() []map[string]any {
	var leaves []map[string]any
	for id, span := range t.spans {
		attrs := traceGetSpanAttributes(span)
		if !traceHasError(span, attrs) {
			continue
		}
		hasErrorDescendant := false
		// BFS: any error in the subtree below this span means we're not the leaf.
		queue := append([]string{}, t.children[id]...)
		for len(queue) > 0 && !hasErrorDescendant {
			head := queue[0]
			queue = queue[1:]
			child := t.spans[head]
			childAttrs := traceGetSpanAttributes(child)
			if traceHasError(child, childAttrs) {
				hasErrorDescendant = true
				break
			}
			queue = append(queue, t.children[head]...)
		}
...
//
// "Root cause" = leaf of the error subtree per trace, see errorSubtreeLeaves.
// Pre-this-change the function counted every error span in the batch, so a
// single failed request that propagated through N services produced N
// signature buckets — readers saw "10 × different error" when there was
// really one error and nine cascade propagations.
func aggregateErrors(traces []map[string]any) []aggregatedError {
	...
	for _, tree := range buildTraceTrees(traces) {
		for _, span := range tree.errorSubtreeLeaves() {
			sig := extractErrorSignature(span)
			if sig == nil {
				continue
			}
			...
			b.count++
			if b.traceID == "" {
				b.traceID = traceGetString(span, "trace_id")
			}
		}
	}
	if len(buckets) == 0 {
...
// the request actually flowed through the system, not the order spans happened
// to arrive in our batch. We walk the parent→child adjacency from the root
// span; pre-this-change we sorted by `timestamp`, which gives wall-clock order
// of span START — wrong any time a trace has retries, concurrent fan-out, or
// out-of-order span arrival.
//
// When multiple traces are passed (the trace_auto_action expands every error
// trace, so it's common), we pick the trace with the most spans as the
// representative — it's the most complete picture of the request flow.
// Smaller traces are typically partial fragments dominated by one branch.
//
// Adjacent same-service spans (e.g., a service that calls itself or has
// multiple internal spans before calling out) are deduped: emit each service
// only on the first encounter in the walk. That preserves the "path" framing
// without losing fan-out — siblings in the tree are visited in start-time
// order so the resulting list reads as a depth-first traversal.
func traceExtractServiceFlow(traces []map[string]any) []traceServiceInfo {
	...
	// Pick the longest trace as representative. Ties broken by trace_id
	// (lexicographic) so output is deterministic — Go map iteration order
	// is randomised, and two equally-large traces would otherwise produce
	// different paths across calls with the same input.
	var representative *traceTree
	var representativeID string
	for id, t := range trees {
		if representative == nil ||
			len(t.spans) > len(representative.spans) ||
			(len(t.spans) == len(representative.spans) && id < representativeID) {
			representative = t
			representativeID = id
		}
	}
	if representative == nil || len(representative.roots) == 0 {
		return nil
	}
	...
	// Iterative DFS; explicit stack avoids unbounded recursion on pathological
	// traces. Push roots in REVERSE so the earliest root is at the top of the
	// stack and gets popped first — LIFO + reverse-push gives "earliest first"
	// processing order. Same trick applied to children below. Children are
	// already pre-sorted by timestamp in buildTraceTrees.
	stack := make([]string, 0, len(representative.spans))
	for i := len(representative.roots) - 1; i >= 0; i-- {
		stack = append(stack, representative.roots[i])
	}
	for len(stack) > 0 {
		top := stack[len(stack)-1]
		stack = stack[:len(stack)-1]

		span := representative.spans[top]
		serviceName := traceGetServiceName(span)
		if serviceName != "Unknown" && !seenServices[serviceName] {
			spanAttrs := traceGetSpanAttributes(span)
			serviceFlow = append(serviceFlow, traceServiceInfo{
				Name:       serviceName,
				SpanName:   traceGetString(span, "span_name"),
				HTTPMethod: traceGetString(spanAttrs, TraceHTTPMethod),
			})
			seenServices[serviceName] = true
		}

		kids := representative.children[top]
