...
// Returns SELECT log, namespace, pod, container, level, COUNT(*), MAX(ts) FROM ... GROUP BY ... ORDER BY count DESC LIMIT N.
...
// MAX(ts) is required so the UI can render per-group "Last Time" — without it every group
// renders the same time. Lexicographic MAX on STRING tsCols is consistent with the BETWEEN
// range filter, which already assumes lex order = chronological order.
...
// pinotMaxTsToUnixSec returns a function that converts a Pinot MAX(ts) cell value to unix seconds.
// Mirrors getTimestampConverter but returns int64 seconds (the frontend multiplies by 1000 to get ms).
// Returns 0 on parse failure so the caller can fall back to a window-level timestamp.
func pinotMaxTsToUnixSec(mode pinotTsMode) func(any) int64 {
	if mode.IsString {
		goFmt := mode.GoFormat
	...
	return func(v any) int64 {
		...
		switch sf {
		case 1: // column unit is ms
			return int64(f) / 1000
		case 1_000: // column unit is s
			return int64(f)
		default:
			// column unit → seconds: f * sf (ms/unit) / 1000 (ms/s)
			return int64(f) * sf / 1_000
		}
	}
}

// parsePinotLogGroupBytes parses a Pinot resultTable from the GROUP BY query into LogGroupOutput.
// Expected columns (resolved by name, not position): msg, namespace, pod, container, [level], cnt, max_ts.
// max_ts is the per-group MAX(tsCol); endTime (epoch ms or s) is used as a fallback if max_ts is missing
// or fails to parse for a row.
func parsePinotLogGroupBytes(data []byte, cols pinotLogGroupCols, tsMode pinotTsMode, endTime int64) (LogGroupOutput, error) {
	var r pinotQueryResponse
	...
	idxMaxTs, hasMaxTs := colIdx["max_ts"]
	convertMaxTs := pinotMaxTsToUnixSec(tsMode)
