...
func (j *JaegerTraceSource) traceMatchesBinaryFilters(trace common.OpenTelemetryTrace, binary query.BinaryWhereClause) bool {
	...
	// ebpf/clickhouse records both ends of a call in one row, so "destination_workload_name = X"
	// literally means "X was the receiver". Jaeger spans are per-service and never list their
	// own service as the destination, so the literal check misses every legitimate
	// "X was called" trace (the SLO Availability case). We accept either:
	//   (a) the literal DestinationWorkload match — covers self-calls and any span where
	//       peer.service was tagged as X, or
	//   (b) a server-kind span owned by X (the Jaeger-native equivalent of "X was called").
	if destFilter, exists := binary["destination_workload_name"]; exists {
		serverSideMatch := isServerSpan(trace.SpanKind) && matchStringFilter(trace.WorkloadName, destFilter)
		if !serverSideMatch && !matchStringFilter(trace.DestinationWorkload, destFilter) {
			return false
		}
	}

	// Check destination_workload_namespace filter — same translation as destination_workload_name.
	if destNsFilter, exists := binary["destination_workload_namespace"]; exists {
		serverSideMatch := isServerSpan(trace.SpanKind) && matchStringFilter(trace.WorkloadNamespace, destNsFilter)
		if !serverSideMatch && !matchStringFilter(trace.DestinationNamespace, destNsFilter) {
			return false
		}
	}
...
// isServerSpan returns true when a Jaeger span represents the receiving side of a call.
// In ebpf/clickhouse semantics, "destination_workload_name = X" means "X was called";
// the Jaeger equivalent is a span owned by X with kind=server (or kind=consumer for
// messaging). Used by traceMatchesBinaryFilters to translate destination filters onto
// Jaeger's per-service span model.
func isServerSpan(spanKind string) bool {
...
func (j *JaegerTraceSource) jaegerSpanToOTEL(span map[string]any, processes map[string]map[string]any) common.OpenTelemetryTrace {
	...
	// span.kind tells us which side of the call this span represents (server = receiver,
	// client = caller). Needed by the post-filter to map ebpf-style destination filters
	// onto Jaeger's per-service span model — see traceMatchesBinaryFilters.
	spanKind := strings.ToLower(j.extractFirstNonEmpty(spanTags, "span.kind", "kind"))
