...
func TestCloudLBResolve(t *testing.T) {
	...
	awsAccountID := "18f0be4c-5951-4282-bf44-938398395c86" // For cloud.ExecuteCli (LoadBalancer queries)
	k8sAccountID := "e9dc2b39-78d3-46da-9691-f160bd3c4c19" // For relay.ExecutePrometheus (kube_pod_info queries)
...
func TestCloudResolveDNS(t *testing.T) {
	...
	testHostname := "staging-redis.fourkites.internal"
	...
	// Step 2: Find the zone matching our hostname (e.g., fourkites.internal)
...
func TestResolveStagingInternalCache(t *testing.T) {
	ctxt := security.NewRequestContextForTenantAdmin(os.Getenv("TEST_TENANT"), nil, nil, nil)
	awsAccountID := "18f0be4c-5951-4282-bf44-938398395c86"

	// Test the specific hostname from the debug output
	testHostname := "staging-internal-cache.fourkites.internal"
	t.Logf("Testing DNS resolution for: %s", testHostname)
...
func TestEnrichRoute53DNSWithTargets(t *testing.T) {
	...
	// IMPORTANT: AWS and K8s use different account IDs
	awsAccountID := "18f0be4c-5951-4282-bf44-938398395c86" // For cloud.ExecuteCli
	k8sAccountID := "aa659817-687f-40bc-b9bd-8027dc489e4c" // For relay.ExecutePrometheus
	tenantID := os.Getenv("TEST_TENANT")

	// Test DNS hostname to resolve
	testHostname := "api-gateway-admin-eks-staging.fourkites.internal"
