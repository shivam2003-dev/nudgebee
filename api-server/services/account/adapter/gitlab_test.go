...
func TestCheckoutCodeRepoGitLab(t *testing.T) {
	...
	ctxt := &testAccountAdapterContext{
		...
		logger:          slog.Default(),
		securityContext: security.NewSecurityContextForSuperAdminAndTenant(os.Getenv("TEST_TENANT")),
	}
...
func TestParseGitLabMRURL(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			mrURL:         "https://gitlab.com/nudgebee/nudgebee/-/merge_requests/123",
			gitLabURL:     "https://gitlab.com",
		...
		},
		{
			...
			mrURL:         "https://gitlab.com/nudgebee/nudgebee/-/merge_requests/789/",
			gitLabURL:     "https://gitlab.com",
			expectedPath:  "nudgebee/nudgebee",
			expectedMRIID: "789",
			expectedError: false,
		},
		{
			...
			mrURL:         "https://gitlab.com/nudgebee/nudgebee/-/issues/123",
			gitLabURL:     "https://gitlab.com",
			expectedPath:  "",
			expectedMRIID: "",
			expectedError: true,
...
func TestGetResolutionStatusWithGitLabToken(t *testing.T) {
	...
	ctxt := &testAccountAdapterContext{
		ctx:             context.Background(),
		logger:          slog.Default(),
		securityContext: security.NewSecurityContextForSuperAdminAndTenant(recommendation.TenantId),
	}
