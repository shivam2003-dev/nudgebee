...
type EventIncomingWebhookInvestigation struct {
	...
	Fingerprint string                     `json:"fignerprint" validate:"required"`
...
func getIntegrationSettings(dbms *database.DatabaseManager, integrationId string) ([]IntegrationConfigValue, error) {
	...
	query := "SELECT name, value, is_encrypted FROM integration_config_values WHERE integration_id = $1"

	err := dbms.Db.Select(&settings, query, integrationId)
	if err != nil {
		return nil, err
	}
