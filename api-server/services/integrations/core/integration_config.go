...
func UpdateIntegrationConfigStatus(
	...
	// active configs per account (mirrors the create-path skip-list).
	// IsProxyIntegrationType is type-only — for dual-mode types it conservatively
	// treats the type as "potentially proxy" and skips the check; the
	// create-path already enforces the invariant for direct-mode dual rows,
	// so this slight over-skip on enable is acceptable.
