...
func TestScoring(t *testing.T) {
	...
	assert.NotNil(t, result.Factors["service_tier"])
	assert.NotNil(t, result.Factors["tier_bonus"])
}
...
func TestScoringMultipleEvents(t *testing.T) {
	...
	testCases := []struct {
		...
		{"workflow-server event", "6a786dbb-b3e1-4965-9eb0-2ea7ef38f292"},
		{"service_map event", "beabc0e6-c276-46ad-aeb7-734190314c1a"},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Logf("Score: %d, Priority: %s, Tier: %v, TierBonus: %v",
				result.Score, result.Priority,
				result.Factors["service_tier"],
				result.Factors["tier_bonus"])
...
func TestServiceTierDetection(t *testing.T) {
	ctx := context.Background()
	...
	db := dbManager.Db
	cloudAccountID := "a2a30b02-0f67-42e5-a2ab-c658230fd798"
	...
	testCases := []struct {
		...
		{"ingress controller", "ingress-nginx-controller", "ingress-nginx", TierCustomerFacing},
		{"postgres database", "postgres", "default", TierCoreInfra},
		{"rabbitmq queue", "rabbitmq", "rabbit", TierCoreInfra},
		{"victoria metrics", "victoria-metrics", "monitoring", TierMonitoring},
		{"prometheus", "prometheus-server", "monitoring", TierMonitoring},
		{"business service", "workflow-server", "nudgebee", TierDefault},
		{"services-server", "services-server", "nudgebee", TierDefault},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tier := getServiceTier(ctx, db, tc.subjectOwner, tc.namespace, cloudAccountID)
			t.Logf("%s -> Tier %d (expected %d)", tc.subjectOwner, tier, tc.expectedTier)
...
func TestComputeAndUpdateScore(t *testing.T) {
	...
	eventID := "6a786dbb-b3e1-4965-9eb0-2ea7ef38f292"
	...
	fmt.Printf("Service Tier: %v\n", result.Factors["service_tier"])
	fmt.Printf("Tier Bonus: %v\n", result.Factors["tier_bonus"])
