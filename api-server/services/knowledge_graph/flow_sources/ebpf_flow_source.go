...
func (s *EbpfFlowSource) BuildFlowRelationships(
	...
	for _, account := range k8sAccounts {
		...
		accountEdges, accountNewNodes, accountExternalNodes, err := s.processK8sAccount(ctx, req, account, externalServiceNodesByAccount[account.CloudAccountID])
		if err != nil {
...
func (s *EbpfFlowSource) processK8sAccount(
	...
	for i := range serviceMap.Applications {
		...
		for j := range app.Downstreams {
			...
			if destApp != nil {
				// Skip ephemeral/intermediate K8s resource kinds and raw IP names
				if s.isIgnoredKind(destApp.Id.Kind) || s.isIPAddress(destApp.Id.Name) {
					continue
				}
				...
				// Skip ephemeral kinds and raw IP addresses
				if s.isIgnoredKind(downstream.Id.Kind) || s.isIPAddress(downstream.Id.Name) {
					continue
				}
				var matchErr error
		...
		for k := range app.Upstreams {
			...
			if upstreamApp != nil && !skipNodeSearch {
				// Skip ephemeral kinds (pods, replicasets, staticpods) and raw IP names
				if s.isIgnoredKind(upstreamApp.Id.Kind) || s.isIPAddress(upstreamApp.Id.Name) {
					continue
				}
				...
				if s.isIgnoredKind(upstreamID.Kind) || s.isIPAddress(upstreamID.Name) {
					continue
				}
				var matchErr error
