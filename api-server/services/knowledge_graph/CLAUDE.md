...
- [api-server/services/api/hasura_actions_knowledge_graph.go](../api/hasura_actions_knowledge_graph.go) — HTTP entry points (`kg_*` actions)
- [api-server/services/api/hasura_cron.go](../api/hasura_cron.go) — daily 23:30 UTC cron, fans out via queue
...
1. **Cron** — daily 23:30 UTC (`build_knowledge_graphs_with_db_filters`) → [hasura_cron.go](../api/hasura_cron.go) reads enabled `knowledge_graph_tenant_filters` rows → publishes one RabbitMQ message per tenant.
...
3. **Manual / cloud-event** — Hasura action `build_knowledge_graph` (synchronous) or cloud-collector publishing on resource change.
...
Exposed via Hasura actions in [hasura_actions_knowledge_graph.go](../api/hasura_actions_knowledge_graph.go): `kg_get_complete_graph`, `kg_get_node`, `kg_get_edge`, `kg_get_filter_options`, `kg_get_node_neighbors`, `kg_traverse`, `kg_search_nodes`, `build_knowledge_graph`.
...
- **Don't add `CREATE INDEX CONCURRENTLY` to KG migrations.** Hasura runs migrations in a transaction block — it will fail. Use plain `CREATE INDEX`. (See root [CLAUDE.md](../../../CLAUDE.md#hasura-migrations--metadata).)
