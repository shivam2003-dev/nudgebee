	...
	cases := []struct {
		...
		},
		{
			...
			props: map[string]interface{}{
				...
				"name":               "my-table",
				"region":             "us-east-1",
				"aws_account_number": "331803013664",
			},
			wantDNS: "",
		},
		{
			...
			props: map[string]interface{}{
				...
				"region":             "us-east-1",
				"aws_account_number": "740395098545",
				"repository_uri":     "740395098545.dkr.ecr.us-east-1.amazonaws.com/myapp",
			},
			wantDNS: "",
		},
		{
...
func TestRepoURIHost(t *testing.T) {
	cases := []struct {
		...
		{"with_scheme", "https://740395098545.dkr.ecr.us-east-1.amazonaws.com/foo", "740395098545.dkr.ecr.us-east-1.amazonaws.com"},
		{"without_scheme", "740395098545.dkr.ecr.us-east-1.amazonaws.com/foo", "740395098545.dkr.ecr.us-east-1.amazonaws.com"},
		{"public_ecr", "public.ecr.aws/abc/myrepo", "public.ecr.aws"},
		{"empty", "", ""},
		{"whitespace", "   ", ""},
		{"eks_endpoint", "https://AD02F73E.gr7.us-east-1.eks.amazonaws.com", "AD02F73E.gr7.us-east-1.eks.amazonaws.com"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
