		...
		[]dataCleanupJob{
			{
				...
				Query:     fmt.Sprintf(`DELETE FROM hdb_catalog.hdb_cron_events WHERE scheduled_time < now() - interval '%d days'`, config.Config.NBRetentionDaysHasuraCronEvents),
			},
			{
				Name:      "hdb_scheduled_events",
				Metastore: database.Metastore,
				Query:     fmt.Sprintf(`DELETE FROM hdb_catalog.hdb_scheduled_events WHERE scheduled_time < now() - interval '%d days'`, config.Config.NBRetentionDaysHasuraCronEvents),
			},
			{
				Name:      "hdb_event_invocation_logs",
				Metastore: database.Metastore,
				Query:     fmt.Sprintf(`DELETE FROM hdb_catalog.event_invocation_logs WHERE created_at < now() - interval '%d days'`, config.Config.NBRetentionDaysHasuraCronEvents),
			},
			{
				Name:      "hdb_event_log",
				Metastore: database.Metastore,
				Query:     fmt.Sprintf(`DELETE FROM hdb_catalog.event_log WHERE created_at < now() - interval '%d days'`, config.Config.NBRetentionDaysHasuraCronEvents),
			},
			{
			...
			},
			{
				...
				Metastore: database.Metastore,
				Query:     fmt.Sprintf(`DELETE FROM sent_notifications WHERE created_at < now() - interval '%d days'`, config.Config.NBRetentionDaysHasuraCronEvents),
			},
			{
