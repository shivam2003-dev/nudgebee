...
func handleEventAction(actionPayload *ActionRequest, c *gin.Context, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	actionRequest := actionPayload.Input

	switch actionPayload.Action.Name {
	case "event_resolve", "events_update_resolution":
		var request event.EventRecommendationApplyRequest
		var err error
		if actionRequest["object"] == nil {
			err = common.UnmarshalMapToStruct(actionRequest, &request)
		} else {
			err = common.UnmarshalMapToStruct(actionRequest["object"].(map[string]any), &request)
		}
	...
	case "refresh_investigation":
		var eventId string
		if actionRequest["event_id"] != nil {
			if id, ok := actionRequest["event_id"].(string); ok {
				eventId = id
			} else {
				c.JSON(400, common.ErrorActionBadRequest("event_id must be a string"))
				return
			}
		} else if actionRequest["object"] != nil {
			if objectMap, ok := actionRequest["object"].(map[string]any); ok {
				...
				c.JSON(400, common.ErrorActionBadRequest("object field is not valid"))
				return
			}
		}
	...
	case "trigger_investigation":
		...
		for idx, eventInput := range eventInput {
			...
			if eventDto.FindingId == "" || eventDto.AggregationKey == "" || eventDto.Title == "" || eventDto.Tenant == "" || eventDto.AccountId == "" || eventDto.FindingType == "" || eventDto.Priority == "" || eventDto.SubjectName == "" || eventDto.Cluster == "" {
				...
				c.JSON(400, common.ErrorActionBadRequest("finding_id/aggregation_key/title/tenant/account_id/finding_type/priority/subject_name/cluster is required. findingId - "+eventDto.FindingId))
				return
			}
