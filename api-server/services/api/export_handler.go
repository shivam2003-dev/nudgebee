...
// handleExportRecommendations handles the export recommendations HTTP endpoint (Hasura action)
func handleExportRecommendations(c *gin.Context, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	...
	// Parse request from Hasura input
	...
	if mlRequest["request"] == nil {
		...
		if obj, ok := mlRequest["request"].(map[string]interface{}); ok {
			...
			c.JSON(400, common.ErrorHasuraActionBadRequest("Invalid request: 'request' field must be a valid JSON object"))
			return
		}
	}
	...
	if request.Format != "csv" && request.Format != "xlsx" {
		c.JSON(400, common.ErrorHasuraActionBadRequest("Format must be csv or xlsx"))
		return
	}

	// Build context from Hasura payload (includes user/tenant info)
	...
	// Return result in Hasura action format
	c.JSON(200, result)
}
