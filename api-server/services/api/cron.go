...
func handleCrons(r *gin.Engine, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	r.POST("/rpc-cron", func(c *gin.Context) {
		common.MetricsApiRequestsTotal(c.Request.Context(), "rpc_cron")
		var actionPayload CronRequest
		err := c.ShouldBindJSON(&actionPayload)
		if err != nil {
			common.MetricsApiRequestsFailedTotal(c.Request.Context(), "rpc_cron", "invalid_json")
		...
		case "daily-highlight-report":
			...
			if err != nil {
				common.MetricsApiRequestsFailedTotal(c.Request.Context(), "rpc_cron", "unmarshal_payload_failed")
				c.JSON(400, common.ErrorActionBadRequest(err.Error()))
				return
			}
		...
		case "Security recommendation refresh":
			...
			c.JSON(200, gin.H{"status": "ok"})
		case "RPC Event Data Cleanup", "NB Data Cleanup":
			go func() {
				defer func() {
					if r := recover(); r != nil {
