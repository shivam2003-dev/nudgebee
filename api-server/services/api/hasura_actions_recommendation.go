...
func handleHasuraRecommendationAction(hasuraPayload *HasuraActionRequest, c *gin.Context, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	...
	case "recommendation_rule_list":
		...
		if request.RuleName != "" {
			...
			if request.RecommendationId != "" {
				ctx, ctxErr := buildContextFromHasuraPayload(c, hasuraPayload, tracer, meter, logger)
				if ctxErr == nil {
					...
					if recErr == nil {
						...
						if err != nil {
							c.JSON(404, common.ErrorHasuraActionBadRequest("rule not found: "+err.Error()))
							return
						}
