...
func handleHasuraTenantAction(hasuraPayload *HasuraActionRequest, c *gin.Context, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	tenantRequest := hasuraPayload.Input
	...
	case "tenant_insert_one":
		...
		return
	case "tenant_user_roles_upsert_one", "auth_tenant_user_roles_upsert_one":
		...
		c.JSON(200, resp)
		return
	case "tenant_group_roles_upsert_one", "auth_tenant_group_roles_upsert_one":
		...
		c.JSON(200, resp)
		return
	case "account_user_roles_upsert_one", "auth_account_user_roles_upsert_one":
		...
		c.JSON(200, resp)
		return
	case "account_group_roles_upsert_one", "auth_account_group_roles_upsert_one":
		...
		c.JSON(200, resp)
		return
	case "auth_k8saccount_namespace_user_roles_upsert_one":
		...
		c.JSON(200, resp)
		return
	case "auth_k8saccount_namespace_group_roles_upsert_one":
	...
	case "cloud_resource_attributes_upsert":
		...
		c.JSON(200, resp)
		return
	case "tenant_onboarding_delete_by_username":
	...
	case "tenant_onboarding_insert":
		...
		return
	case "tenant_onboarding_update_status":
		...
		return
	case "tenant_onboarding_get_by_token":
	...
	case "tenant_list_all":
		...
		allowedRoles, _ := hasuraPayload.SessionVariables["x-hasura-allowed-roles"].(string)
		if !strings.Contains(allowedRoles, "super_admin") {
			c.JSON(400, common.ErrorHasuraActionBadRequest("Not Allowed"))
			return
		}
		...
		return
	case "billing_list":
		var request billing.BillingListRequest
		...
		resp, err := billing.ListBillings(ctx, request)
		if err != nil {
			logger.Error("failed to list billings", "error", err)
			c.JSON(500, common.ErrorHasuraActionBadRequest("internal server error"))
			return
		}
		c.JSON(200, resp)
		return
	case "billing_usage_cost_list":
		var request billing.BillingUsageCostListRequest
		if err := common.UnmarshalMapToStruct(tenantRequest, &request); err != nil {
			c.JSON(400, common.ErrorHasuraActionBadRequest(err.Error()))
			return
		}
		...
		resp, err := billing.ListBillingUsageCosts(ctx, request)
		if err != nil {
			logger.Error("failed to list billing usage costs", "error", err)
			c.JSON(500, common.ErrorHasuraActionBadRequest("internal server error"))
			return
		}
		c.JSON(200, resp)
		return
	case "billing_infographics":
		ctx, err := buildContextFromHasuraPayload(c, hasuraPayload, tracer, meter, logger)
		if err != nil {
			c.JSON(400, common.ErrorHasuraActionBadRequest(err.Error()))
			return
		}
		resp, err := billing.GetBillingInfographics(ctx)
		if err != nil {
			logger.Error("failed to get billing infographics", "error", err)
			c.JSON(500, common.ErrorHasuraActionBadRequest("internal server error"))
			return
		}
		c.JSON(200, resp)
	...
	case "tenant_delete":
		...
		if err != nil {
			c.JSON(400, common.ErrorHasuraActionBadRequest("Failed to fetch accounts for tenant: "+err.Error()))
			return
		}
		...
		for _, accountId := range accountIds {
			...
			if err != nil {
				...
				c.JSON(400, common.ErrorHasuraActionBadRequest("Failed to delete account "+accountId+": "+err.Error()+". WARNING: accounts already deleted: "+strings.Join(deletedAccountIds, ", ")))
				return
			}
