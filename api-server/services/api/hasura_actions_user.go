...
func handleHasuraUserAction(hasuraPayload *HasuraActionRequest, c *gin.Context, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	tenantRequest := hasuraPayload.Input

	switch hasuraPayload.Action.Name {
	case "users_insert_one":
	...
	case "user_delete_auth":
		...
		return
	case "user_update_default_tenant":
