...
func handleHasuraLogsAction(hasuraPayload *HasuraActionRequest, c *gin.Context, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	...
	case "logs_query":
		...
		if err != nil {
			slog.Error("logs_query: failed to decode request", "error", err)
			c.JSON(400, common.ErrorHasuraActionBadRequest(err.Error()))
			return
		}
	...
	case "logs_get_query":
		...
		err := common.UnmarshalMapToStruct(hasuraPayload.Input["request"].(map[string]interface{}), &request)
		if err != nil {
			slog.Error("logs_query: failed to decode request", "error", err) // Original message was "logs_query", kept for consistency
			c.JSON(400, common.ErrorHasuraActionBadRequest(err.Error()))
			return
		}
