...
// simulateHasuraQueryPipeline replicates the handleHasuraQueryAction pipeline
...
func simulateHasuraQueryPipeline(t *testing.T, payload HasuraActionRequest) (query.QueryRequest, string) {
	t.Helper()
	...
	if queryMap["columns"] == nil || len(queryMap["columns"].([]any)) == 0 {
		colsFromGql, err := parseHasuraSelectColumns(&payload)
		require.NoError(t, err, "parseHasuraSelectColumns failed")
...
func parsePostmanJSON(t *testing.T, jsonStr string) HasuraActionRequest {
	t.Helper()
	var payload HasuraActionRequest
...
func TestE2E_LLMConversationList_BasicQuery(t *testing.T) {
	...
	req, sql := simulateHasuraQueryPipeline(t, payload)
...
func TestE2E_GQLParsing_NestedRows(t *testing.T) {
	payload := HasuraActionRequest{
		Action:       HasuraActionRequestAction{Name: "llm_conversation_list_v2"},
		RequestQuery: `query Q { llm_conversation_list_v2(where: {}) { rows { id title source status } } }`,
	}
	cols, err := parseHasuraSelectColumns(&payload)
	require.NoError(t, err)
...
func TestE2E_GQLParsing_WithExprArgs(t *testing.T) {
	payload := HasuraActionRequest{
		Action:       HasuraActionRequestAction{Name: "dw_query_groupings_v2"},
...
func TestE2E_GQLParsing_FlatFields(t *testing.T) {
	payload := HasuraActionRequest{
		Action:       HasuraActionRequestAction{Name: "metrics_v2"},
