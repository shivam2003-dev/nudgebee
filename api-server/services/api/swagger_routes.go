...
func buildMergedOpenAPI(r *gin.Engine) ([]byte, error) {
	...
	// buildContextFromHasuraPayload in api/hasura_actions.go).
	...
	scopeHeaders := []map[string]interface{}{
		{
			...
			"required":    false,
			"type":        "string",
			"description": "Scope the call to a tenant. Falls back to `session_variables.x-hasura-user-tenant-id` if unset.",
		},
		{
			"name":        "x-user-id",
			"in":          "header",
			"required":    false,
			"type":        "string",
			"description": "Scope the call to a user. Falls back to `session_variables.x-hasura-user-id` if unset.",
		},
	}
...
// hasuraEnvelopeSchema returns an inline JSON-schema describing the standard
// Hasura action request envelope, with an `example` payload Swagger UI uses
// to pre-fill the "Try it out" body editor. Each property carries its own
// example so the editor surfaces realistic placeholders.
func hasuraEnvelopeSchema() map[string]interface{} {
	return map[string]interface{}{
		...
		"properties": map[string]interface{}{
			"action": map[string]interface{}{
				...
				"properties": map[string]interface{}{
					"name": map[string]interface{}{
						"type":        "string",
						"description": "Hasura action name being invoked (matches the dispatcher's switch statement).",
			...
			"session_variables": map[string]interface{}{
				"type":        "object",
				"description": "Hasura session vars. Use `x-hasura-role: admin` (with empty tenant/user) for super-admin; set tenant/user IDs to scope the call.",
				"properties": map[string]interface{}{
					"x-hasura-role":           map[string]interface{}{"type": "string", "example": "admin"},
					"x-hasura-user-tenant-id": map[string]interface{}{"type": "string", "example": ""},
					"x-hasura-user-id":        map[string]interface{}{"type": "string", "example": ""},
					"x-hasura-allowed-roles":  map[string]interface{}{"type": "string", "example": "{admin}"},
				},
...
// `nudgebee/services/api.handleHasuraApis.func25` reads as `handleHasuraApis`.
...
func stubOperation(handlerName, path, method string) map[string]interface{} {
	...
	// Body editor for write methods. Pre-fill with the Hasura action envelope
	...
	case "post", "put", "patch", "delete":
		params = append(params, map[string]interface{}{
			...
			"in":          "body",
			"required":    false,
			"description": "Hasura action envelope. Replace `<action_name>`, fill `input` with the action's expected fields, and set `session_variables` to scope the call (use `x-hasura-role: admin` for super-admin, or set tenant/user IDs to scope to a tenant).",
			"schema":      hasuraEnvelopeSchema(),
		})
	}
