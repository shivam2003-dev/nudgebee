...
func handleKnowledgeGraphAction(actionPayload *ActionRequest, c *gin.Context, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	...
	if !ok {
		if actionPayload.Action.Name == "build_knowledge_graph" || actionPayload.Action.Name == "kg_get_filter_options" || actionPayload.Action.Name == "kg_list_nodes" || actionPayload.Action.Name == "kg_get_tenant_filter" {
	...
	case "kg_search_nodes", "kg_list_nodes":
