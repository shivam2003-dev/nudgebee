...
// relayHasuraActionRequest wraps the Hasura action envelope sent to the relay handler.
type relayHasuraActionRequest struct {
...
func parseRelayIdentity(c *gin.Context, sessionVars map[string]any) (tenantId, userId string, ok bool) {
	...
	// Fall back to Hasura session variables when headers are absent (Hasura action flow).
	if tenantId == "" {
		if v, exists := sessionVars["x-hasura-user-tenant-id"]; exists {
			tenantId, _ = v.(string)
		}
	}
	if userId == "" {
		if v, exists := sessionVars["x-hasura-user-id"]; exists {
			userId, _ = v.(string)
		}
	}
...
func handleRelayApis(r *gin.Engine, tracer *trace.Tracer, meter *metric.Meter, logger *slog.Logger) {
	...
	group.POST("/:action", func(c *gin.Context) {
		...
		// Parse Hasura action envelope (session_variables + input).
		var hasuraReq relayHasuraActionRequest
		...
		reqPayload := hasuraReq.Input

		// Resolve identity from headers first, then Hasura session variables.
		tenantId, userId, ok := parseRelayIdentity(c, hasuraReq.SessionVariables)
		if !ok {
