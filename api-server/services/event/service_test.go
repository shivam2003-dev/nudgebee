...
func TestInvestigateEvent(t *testing.T) {
	...
	response, err := InvestigateEvent(security.NewRequestContextForTenantAdmin("6d79c39f-920e-4167-85cf-e2ee83dcbc03", slog.Default(), nil, nil), Event{
		AccountId:      "e9dc2b39-78d3-46da-9691-f160bd3c4c19",
		Tenant:         "6d79c39f-920e-4167-85cf-e2ee83dcbc03",
		...
		Description:    `**Agent URL -** https://fourkites.chronosphere.io/monitors/java-4xx-error-rate?receiver=pagerduty-orchestration-notifier&receiver-type=pagerduty&status=CRITICAL&end=1755050887955&start=1755047287955&signal=%7B%22environment%22%3A%22prod%22%2C%22job%22%3A%22eurl-service%22%7D\n **Client -** Chronosphere\n **Client URL -** https://fourkites.chronosphere.io/monitors/java-4xx-error-rate?receiver=pagerduty-orchestration-notifier&receiver-type=pagerduty&status=CRITICAL&end=1755050887955&start=1755047287955&signal=%7B%22environment%22%3A%22prod%22%2C%22job%22%3A%22eurl-service%22%7D`,
...
func TestApplyEventResolutionWithGitProvider(t *testing.T) {
	...
	// Test data - using environment variables or defaults
