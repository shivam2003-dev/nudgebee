...
CREATE OR REPLACE VIEW "public"."cloud_resource_metrics_groupings_type" AS 
 SELECT 
    a.tenant AS tenant_id,
    a.account_name,
    ...
    crm.metric,
    crm.timestamp,
    sum(crm.value) AS sum_usage,
    avg(crm.value) AS avg_usage,
    min(crm.value) AS min_usage,
    max(crm.value) AS max_usage
  ...
  GROUP BY a.tenant, a.account_name, cr.account, cr.id, cr.name, crm.metric, crm.timestamp
  ORDER BY crm.timestamp desc;

drop view if exists "public"."cloud_resource_metrics_groupings_type";
...
CREATE OR REPLACE FUNCTION public.cloud_resource_groupings(group_by text[] DEFAULT '{}'::text[], "where" json DEFAULT NULL::json, date_unit text DEFAULT 'day' :: text, "limit" integer DEFAULT 100, "offset" integer DEFAULT 0, sort_by text DEFAULT 'timestamp' :: text, sort_order text DEFAULT 'asc' :: text, hasura_session json DEFAULT '{}'::json)
...
DROP FUNCTION IF EXISTS cloud_resource_groupings;
