...
DROP FUNCTION if exists public.dw_query_groupings(_text, json, text, int4, int4, text, text, json);
DROP FUNCTION if exists public.dw_query_groupings(_text, json, text, int4, int4, json);

CREATE OR REPLACE FUNCTION public.dw_query_groupings(group_by text[] DEFAULT '{}'::text[], "where" json DEFAULT NULL::json, date_unit text DEFAULT 'day'::text, "limit" integer DEFAULT 100, "offset" integer DEFAULT 0, sort_by text DEFAULT 'query_started_at', sort_order text DEFAULT 'asc', hasura_session json DEFAULT '{}'::json)
