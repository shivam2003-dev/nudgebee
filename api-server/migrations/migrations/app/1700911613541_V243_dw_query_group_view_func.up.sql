drop view if exists "public"."dw_query_groupings_type" cascade;

CREATE
OR REPLACE VIEW "public"."dw_query_groupings_type" AS
SELECT
  ...
  dw_queries.query_normalized,
  dw_queries.query_normalized_md5,
  dw_queries.query_text,
  ...
  array_agg(DISTINCT dw_queries.db_username) AS db_username_list,
  array_agg(DISTINCT dw_queries.query_remote_ip) AS query_remote_ip_list,
  array_agg(DISTINCT dw_queries.database_name) AS database_name_list
FROM
  dw_queries
WHERE
  false
GROUP BY
  ...
  dw_queries.query_normalized_md5,
  dw_queries.query_text,
  (dw_queries.tags ->> 'warehouse_name' :: text),
  dw_queries.query_started_at
ORDER BY
  dw_queries.query_started_at DESC;
