...
CREATE OR REPLACE VIEW "public"."spend_groupings_type" AS 
 SELECT s.tenant AS tenant_id,
  ...
  WHERE false
  GROUP BY s.tenant, s.cloud_account, ca.account_name, ca.cloud_provider, s.cloud_resource_id, cr.name, cr.service_name, cr.region, cr.type, s.date;
