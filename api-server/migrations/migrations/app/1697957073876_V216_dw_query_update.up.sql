...
drop view if exists dw_query_groupings_type CASCADE;
...
CREATE OR REPLACE VIEW "public"."dw_query_groupings_type" AS 
 SELECT 
  ...
  GROUP BY 
   ...
   dw_queries.query_started_at
  ORDER BY dw_queries.query_started_at DESC;

CREATE OR REPLACE FUNCTION public.dw_query_groupings(group_by text[] DEFAULT '{}'::text[], "where" json DEFAULT NULL::json, date_unit text DEFAULT 'day'::text, "limit" integer DEFAULT 100, "offset" integer DEFAULT 0, hasura_session json DEFAULT '{}'::json)
