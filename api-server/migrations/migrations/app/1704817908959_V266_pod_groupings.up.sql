-- public.k8s_pod_groupings_type source

CREATE OR REPLACE VIEW public.k8s_pod_groupings_type
AS SELECT cr.tenant AS tenant_id,
    cr.account AS account_id,
    crm.tags ->> 'controllerKind'::text AS workload_type,
    crm.tags ->> 'controller'::text AS workload_name,
    crm.tags ->> 'namespace'::text AS namespace_name,
    crm.tags ->> 'name'::text AS pod_name,
    crm.tags ->> 'node'::text AS node_name,
    crm."timestamp",
    sum(
        CASE
            WHEN crm.metric = 'totalCost'::text THEN crm.value
    ...
    sum(
        CASE
            WHEN crm.metric = 'networkReceiveBytes'::text THEN crm.value
            ELSE NULL::double precision
        END) AS sum_ingress,
    sum(
        CASE
            WHEN crm.metric = 'networkTransferBytes'::text THEN crm.value
            ELSE NULL::double precision
        END) AS sum_egress
   FROM cloud_resource_metrics crm
     LEFT JOIN cloud_resourses cr ON cr.id = crm.cloud_resource_id
  WHERE false
  GROUP BY cr.tenant, cr.account, (crm.tags ->> 'controllerKind'::text), (crm.tags ->> 'controller'::text), (crm.tags ->> 'namespace'::text), (crm.tags ->> 'name'::text), (crm.tags ->> 'node'::text), crm."timestamp"
  ORDER BY crm."timestamp" DESC;


CREATE OR REPLACE FUNCTION public.k8s_pod_groupings(group_by text[] DEFAULT '{}'::text[], "where" json DEFAULT NULL::json, date_unit text DEFAULT 'date'::text, "limit" integer DEFAULT 100, "offset" integer DEFAULT 0, sort_by text DEFAULT 'timestamp'::text, sort_order text DEFAULT 'asc'::text, hasura_session json DEFAULT '{}'::json)
...
SELECT
  (
    CASE
      WHEN 'tenant_id' = ANY(group_by) THEN cr.tenant
      ELSE NULL
    END
  ) AS tenant_id,
  (
    CASE
      WHEN 'account_id' = ANY(group_by) THEN cr.account
      ELSE NULL
    END
  ) AS account_id,
  (
...
where
  (
    ...
    OR (
      cr."tenant" = ("hasura_session" ->> 'x-hasura-user-tenant-id') :: uuid
    )
  )
  AND (
    "where" #>> '{account_id,_eq}' IS NULL
    OR (
      cr."account" = ("where" #>> '{account_id,_eq}') :: uuid
    )
  )
  AND (
