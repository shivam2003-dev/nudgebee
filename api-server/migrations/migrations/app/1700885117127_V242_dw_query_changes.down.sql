...
-- Could not auto-generate a down migration.
-- Please write an appropriate down migration for the SQL below:
-- DROP VIEW "public"."dw_query_groupings_type";

CREATE OR REPLACE FUNCTION public.dw_query_groupings(group_by text[] DEFAULT '{}'::text[], "where" json DEFAULT NULL::json, date_unit text DEFAULT 'day'::text, date_unit_bin integer DEFAULT 1, "limit" integer DEFAULT 100, "offset" integer DEFAULT 0, sort_by text DEFAULT 'query_started_at'::text, sort_order text DEFAULT 'asc'::text, hasura_session json DEFAULT '{}'::json)
...
SELECT
  ...
  (
    CASE
      WHEN 'resource_id' = ANY(group_by) THEN resource_id
      ELSE NULL
    END
  ) AS resource_id,
  (
    CASE
      WHEN 'database_name' = ANY(group_by) THEN database_name
      ELSE NULL
    END
  ) AS database_name,
  (
    CASE
      WHEN 'db_username' = ANY(group_by) THEN db_username
      ELSE NULL
    END
  ) AS db_username,
  (
    CASE
      WHEN 'query_remote_ip' = ANY(group_by) THEN query_remote_ip
      ELSE NULL
    END
  ) AS query_remote_ip,
  (
    CASE
      WHEN 'query_type' = ANY(group_by) THEN query_type
      ELSE NULL
    END
  ) AS query_type,
  (
    CASE
      WHEN 'query_started_at' = ANY(group_by) THEN date_bin(
  ...
  (
    CASE
      WHEN 'query_status' = ANY(group_by) THEN query_status
      ELSE NULL
    END
  ) AS query_status,
  (
    CASE
      WHEN 'warehouse_name' = ANY(group_by) THEN tags ->> 'warehouse_name'
      ELSE NULL
    END
  ) AS warehouse_name,
  (
    CASE
      WHEN 'query_normalized' = ANY(group_by) THEN query_normalized
      ELSE NULL
    END
  ) AS query_normalized,
  (
    CASE
      WHEN 'query_normalized_md5' = ANY(group_by) THEN query_normalized_md5
      ELSE NULL
    END
  ) AS query_normalized_md5,
  (
    CASE
      WHEN 'query_text' = ANY(group_by) THEN query_text
      ELSE NULL
    END
  ) AS query_text,
  avg(dw_queries.query_exec_duration_micro) AS avg_query_exec_duration_micro,
  ...
  count(*) AS query_count,
  array_agg(distinct db_username) as db_username_list,
  array_agg(distinct query_remote_ip) as query_remote_ip_list,
  array_agg(distinct database_name) as database_name_list
FROM
  dw_queries
WHERE
  (
    ...
    AND (
      ...
      AND (
        ...
        AND (
          ...
          AND (
            "where" #>> '{query_remote_ip,_eq}' IS NULL OR ("query_remote_ip" = ("where" #>> '{query_remote_ip,_eq}')))
            AND (
                              ...
                              AND (
                                ...
                                AND (
                                  ...
                                  ORDER BY
                                    ...
                                    (
                                      case
                                        when sort_by = 'query_count'
                                        and sort_order = 'asc' then count(*)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'query_count'
                                        and sort_order = 'desc' then count(*)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_rpu'
                                        and sort_order = 'asc' then sum(dw_queries.rpu)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_rpu'
                                        and sort_order = 'desc' then sum(dw_queries.rpu)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_query_returned_rows'
                                        and sort_order = 'asc' then sum(dw_queries.query_returned_rows)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_query_returned_rows'
                                        and sort_order = 'desc' then sum(dw_queries.query_returned_rows)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_partitions_scanned'
                                        and sort_order = 'asc' then sum(dw_queries.partitions_scanned)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_partitions_scanned'
                                        and sort_order = 'desc' then sum(dw_queries.partitions_scanned)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_bytes_spilled_remotely'
                                        and sort_order = 'asc' then sum(dw_queries.bytes_spilled_remotely)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_bytes_spilled_remotely'
                                        and sort_order = 'desc' then sum(dw_queries.bytes_spilled_remotely)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_bill'
                                        and sort_order = 'asc' then sum(dw_queries.bill)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_bill'
                                        and sort_order = 'desc' then sum(dw_queries.bill)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_query_returned_bytes'
                                        and sort_order = 'asc' then sum(dw_queries.query_returned_bytes)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_query_returned_bytes'
                                        and sort_order = 'desc' then sum(dw_queries.query_returned_bytes)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_query_planning_duration_micro'
                                        and sort_order = 'asc' then sum(dw_queries.query_planning_duration_micro)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_query_planning_duration_micro'
                                        and sort_order = 'desc' then sum(dw_queries.query_planning_duration_micro)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_bytes_scanned'
                                        and sort_order = 'asc' then sum(dw_queries.bytes_scanned)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_bytes_scanned'
                                        and sort_order = 'desc' then sum(dw_queries.bytes_scanned)
                                      end
                                    ) desc,
                                    (
                                      case
                                        when sort_by = 'sum_query_exec_duration_micro'
                                        and sort_order = 'asc' then sum(dw_queries.query_exec_duration_micro)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'sum_query_exec_duration_micro'
                                        and sort_order = 'desc' then sum(dw_queries.query_exec_duration_micro)
                                      end
                                    ) desc,
                                    (
                                      case
                                    ...
                                    (
                                      case
                                        when sort_by = 'max_query_exec_duration_micro'
                                        and sort_order = 'asc' then max(dw_queries.query_exec_duration_micro)
                                      end
                                    ) asc,
                                    (
                                      case
                                        when sort_by = 'max_query_exec_duration_micro'
                                        and sort_order = 'desc' then max(dw_queries.query_exec_duration_micro)
                                      end
                                    ) desc
                                  LIMIT
                                    "limit" OFFSET "offset";
...
alter table "public"."dw_queries" alter column "bill_total_duration_micro" set not null;

alter table "public"."dw_queries" alter column "query_exec_duration_micro" set not null;

ALTER TABLE "public"."dw_queries" ALTER COLUMN "query_returned_bytes" TYPE integer;

ALTER TABLE "public"."dw_queries" ALTER COLUMN "query_returned_rows" TYPE integer;

ALTER TABLE "public"."dw_queries" ALTER COLUMN "query_planning_duration_micro" TYPE integer;

ALTER TABLE "public"."dw_queries" ALTER COLUMN "bill_total_duration_micro" TYPE integer;

ALTER TABLE "public"."dw_queries" ALTER COLUMN "query_exec_duration_micro" TYPE integer;
...
AS $function$
  SELECT
    ...
    (CASE WHEN 'query_remote_ip' = ANY(group_by) THEN query_remote_ip ELSE NULL END) AS query_remote_ip,
    (CASE WHEN 'query_type' = ANY(group_by) THEN query_type ELSE NULL END) AS query_type,
    (CASE WHEN 'query_started_at' = ANY(group_by) THEN date_bin((date_unit_bin || ' ' || date_unit)::interval, query_started_at::timestamp,'2001-01-01'::timestamp) ELSE NULL END) AS query_started_at,
  ...
  GROUP BY
    ...
    (CASE WHEN 'query_remote_ip' = ANY(group_by) THEN query_remote_ip END),
    ...
    (CASE WHEN 'query_started_at' = ANY(group_by) THEN date_bin((date_unit_bin || ' ' ||date_unit)::interval, query_started_at::timestamp, '2001-01-01'::timestamp )  END)
  ORDER BY 
    (case when sort_by = 'query_started_at' and sort_order = 'asc' then max(date_bin((date_unit_bin || ' ' ||date_unit)::interval, query_started_at::timestamp, '2001-01-01'::timestamp) ) end) asc,
    (case when sort_by = 'query_started_at' and sort_order = 'desc' then max(date_bin((date_unit_bin || ' ' ||date_unit)::interval, query_started_at::timestamp, '2001-01-01'::timestamp) ) end) desc,
    ...
    (case when sort_by = 'sum_bytes_spilled_remotely' and sort_order = 'asc' then sum(dw_queries.bytes_spilled_remotely) end) asc,
    (case when sort_by = 'sum_bytes_spilled_remotely' and sort_order = 'desc' then sum(dw_queries.bytes_spilled_remotely) end) desc,
    (case when sort_by = 'max_query_exec_duration_micro' and sort_order = 'asc' then max(dw_queries.query_exec_duration_micro) end) asc,
    (case when sort_by = 'max_query_exec_duration_micro' and sort_order = 'desc' then max(dw_queries.query_exec_duration_micro) end) desc
